...
noisy_projection = itk.threshold_image_filter(noisy_projection, lower=1., outside_value=1.)
noisy_projection = itk.multiply_image_filter(noisy_projection, constant=1./I0)
noisy_projection = itk.log_image_filter(noisy_projection)
noisy_projection = itk.multiply_image_filter(noisy_projection, constant=-1./mu)
...
plt.imshow(itk.array_view_from_image(projection)[0,:,:], cmap=plt.cm.gray, origin='lower', clim=[0,140])
plt.xlabel('u pixel index')
plt.ylabel('v pixel index')
plt.subplot(132)
plt.title('Noisy projection')
plt.imshow(itk.array_view_from_image(noisy_projection)[0,:,:], cmap=plt.cm.gray, origin='lower', clim=[0,140])
plt.xlabel('u pixel index')
plt.subplot(133)
