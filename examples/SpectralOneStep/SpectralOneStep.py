...
spectrum.SetOrigin([0.0, origin_proj, origin_proj])
...
for i, m in zip(range(nmat), materials):
    ...
    mat_proj = rtk.constant_image_source(
        origin=[origin_proj, origin_proj, 0.0],
        size=[npix, npix, nproj],
...
counts.SetOrigin([origin_proj, origin_proj, 0.0])
...
def callback():  # write the result before the end of the reconstruction
    ...
    itk.image_file_writer(
        mechlem.GetOutput(),
        f"decomposed_iteration{iteration_number//nsubsets:03d}_subset{iteration_number%nsubsets:01d}.mha",
        ttype=ImageType,
    )
...
# GPU to CPU
itk.image_file_writer(mechlem.GetOutput(), file_name="decomposed.mha", ttype=ImageType)
