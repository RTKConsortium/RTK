...
for i, m in zip(range(nmat), materials):
    mat_ref = rtk.constant_image_source(
        origin=[origin] * 3, size=[npix] * 3, spacing=[spacing] * 3, ttype=ImageType
    )
...
mat_basis = rtk.constant_image_source(
    size=[nmat, energies.size], spacing=[1.0] * 2, ttype=itk.Image[itk.F, 2]
)
...
decomposed_init.SetOrigin([origin_proj, origin_proj, 0.0])
...
decomposed_ref.SetOrigin([origin_proj, origin_proj, 0.0])
...
counts_forward.SetOrigin([origin_proj, origin_proj, 0.0])
counts_forward.SetSpacing([spacing_proj] * 3)
counts_forward = rtk.spectral_forward_model_image_filter(
    input_decomposed_projections=decomposed_ref,
...
decomposed = rtk.simplex_spectral_projections_decomposition_image_filter(
    input_decomposed_projections=decomposed_init,
...
for m in range(nmat):
    decomp_mat_proj = itk.image_from_array(
        itk.array_from_image(decomposed[0])[:, :, :, m].copy()
    )
    decomp_mat_proj.CopyInformation(decomposed[0])
    mat_recon = rtk.constant_image_source(
        origin=[origin] * 3, size=[npix] * 3, spacing=[spacing] * 3, ttype=ImageType
    )
    mat_recon = itk.fdk_cone_beam_reconstruction_filter(
        mat_recon, decomp_mat_proj, geometry=geometry
    )
