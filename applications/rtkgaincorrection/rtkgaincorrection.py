...
def build_parser():
    parser = rtk.RTKArgumentParser(
        description="Polynomial gain correction projections"
    )
    ...
    parser.add_argument("--calibDir", "-c", help="Directory containing the calibration files", type=str, required=True)
    ...
    parser.add_argument("--K", help="Normalization coefficient", type=float, default=1.0)
    parser.add_argument("--bufferSize", help="Number of projections computed at the same time", type=int, default=4)
...
def process(args_info: argparse.Namespace):
    ...
    if hasattr(itk, 'CudaImage'):
        ...
        gainfilter = rtk.PolynomialGainCorrectionImageFilter[InputImageType, OutputImageType].New()
    ...
    pasteFilter = itk.PasteImageFilter[OutputImageType, OutputImageType].New()
    ...
    for bid in range(Nbuffers):
        ...
        print(f"Processing buffer no {bid} starting at image {bufferIdx} of size {currentBufferSize}")
        ...
        index[2] = bufferIdx
        desiredRegion.SetSize(itk.MakeSize(*size))
        desiredRegion.SetIndex(itk.MakeIndex(*index))
        ...
        if first:
            ...
            constantImageSource.SetSize(itk.Size[3](sizeInput[0], sizeInput[1], sizeInput[2]))
        ...
        pasteFilter.SetDestinationIndex(itk.Index[3](current_idx[0], current_idx[1], current_idx[2]))
        pasteFilter.Update()
