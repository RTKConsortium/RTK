...
def main():
  ...
  parser.add_argument('--show_trajectory', '-t', help='Show source trajectory', action='store_true')
  ...
  # Add bounding box
  box = pv.Cube(bounds=[min_bounds, max_bounds, min_bounds, max_bounds, min_bounds, max_bounds])
  ...
  if not args_info.hide_volume:
    ...
    if args_info.input:
      ...
      volume_sphere = pv.Sphere(radius= 0.1 * geometry.GetSourceToIsocenterDistances()[0], center=[0, 0, 0])
  ...
  def update(frame):
    ...
    if SDD == 0:
      rot_matrix = np.asarray(geometry.GetRotationMatrix(frame))
      direction = (np.array([-rot_matrix[2][0], -rot_matrix[2][1], -rot_matrix[2][2]]) * 2.0 * SID)
    ...
    if not actors:
      actors["detector"] = plotter.add_mesh(detector_poly, texture=texture, opacity=0.5)
      actors["central_line"] = plotter.add_mesh(central_line, color="black", line_width=2)
      actors["text"] = plotter.add_text(info_text, position="upper_right", font_size=10, color="white")
    ...
    if SDD == 0:
      ...
      if "lines" not in actors:
        ...
        for i, (src_corner, det_corner) in enumerate(
          ...
          line = pv.Line(src_corner, det_corner)
          actors["lines"][i].GetMapper(
          ).GetInput().points = line.points
