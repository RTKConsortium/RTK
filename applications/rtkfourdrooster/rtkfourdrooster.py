...
def build_parser():
    ...
    # Frames (same semantics as C++: override 4th dimension of constant source)
    parser.add_argument(
        "--frames",
        help="Number of reconstructed frames (time instants)",
        type=int,
        default=10,
    )
...
def process(args_info: argparse.Namespace):
    ...
    reorder = rtk.ReorderProjectionsImageFilter[ProjectionStackType].New()
    reorder.SetInput(reader.GetOutput())
    reorder.SetInputGeometry(geometry)
    reorder.SetInputSignal(signal)
    reorder.Update()

    # Release original projections stack
    ...
    if args_info.motionmask:
        motionMask = itk.imread(args_info.motionmask, VolumeType)
    ...
    if args_info.dvf:
        ...
        dvf = itk.imread(args_info.dvf, DVFSequenceImageType)
        if hasattr(itk, "CudaImage"):
        ...
        if args_info.idvf:
            ...
            idvf = itk.imread(args_info.idvf, DVFSequenceImageType)
            if hasattr(itk, "CudaImage"):
    ...
    itk.imwrite(rooster.GetOutput(), args_info.output)
