...
def process(args_info: argparse.Namespace):
    ...
    if args_info.hardware == "cuda":
        # Use the filter instead of cuda_image_from_image to Keep the conversion filter
        # alive (assign it to a variable) and pass its GetOutput() to downstream filters
        cudaProjections = itk.CudaImageFromImageFilter[OutputImageType].New()
        cudaProjections.SetInput(selector.GetOutput())

        ddf = rtk.CudaDisplacedDetectorImageFilter.New()
        ddf.SetInput(cudaProjections.GetOutput())
