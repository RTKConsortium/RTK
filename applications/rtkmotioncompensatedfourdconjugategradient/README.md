# Motion-compensated 4D conjugate gradient : unregularized 3D + time reconstruction with known motion
...
RTK provides several tools to reconstruct a 3D + time image using explicit motion estimation. Motion-compensated 4D conjugate gradient is one of them.
...
This example is illustrated with a dataset of a patient used in the [MA-ROOSTER paper](https://www.creatis.insa-lyon.fr/~srit/biblio/mory2016a.pdf). You can [download the projections](https://data.kitware.com/api/v1/item/5be97d688d777f2179a28e39/download), which also contain the RTK geometry file.

## Respiratory signal

The algorithm requires that we associate each projection image with the instant of the respiratory cycle at which it has been acquired. The cphase.txt is already zipped together with the projections, but you can visit [this page](../rtkamsterdamshroud/README.md) to learn how to generate it from the projections using the Amsterdam Shroud.

## 4D Displacement Vector Field

The forward and inverse 4D Displacement Vector Fields for this patient can be downloaded [here](https://data.kitware.com/api/v1/item/5be989e68d777f2179a29e95/download). Please refer to the tutorial on [Motion-Compensated FDK](../rtkfdk/README.md) if you wish to learn how to obtain a valid 4D Displacement Vector Field (DVF).
