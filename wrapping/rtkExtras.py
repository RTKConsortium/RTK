...
def make_application_func(app_name):
    ...
    python_api_example = f"{app_name}({', '.join([f'{arg.dest}=\"{arg.dest}\"' for arg in parser._actions if arg.required])})"

    # Get the help text and remove the 'usage:' line(s)
    help_lines = parser.format_help().splitlines()
    cli_help = "\n".join(
        line for line in help_lines if line.strip().startswith("options:")
    )
...
"""
    application_func.__doc__ = parser.description + usage_examples + cli_help
