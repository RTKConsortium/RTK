...
JosephBackProjectionImageFilter<TInputImage, TOutputImage>::GenerateData()
{
  ...
  for (unsigned int pix = 0; pix < buffReg.GetNumberOfPixels(); pix++, itIn->Next())
  {
    ...
    if (box->IsIntersectedByRay(pixelPosition, dirVox, infDist, supDist) &&
        supDist >= 0. && // check if detector after the source
                                                                                  ...
                                                                                  const int              oy)
{
  ...
  CoordinateType lxc = 1. - lx;
  CoordinateType lyc = 1. - ly;
  return (m_InterpolationWeightMultiplication(0, stepLengthInVoxel, lxc * lyc, pxiyi, idx) +
          m_InterpolationWeightMultiplication(0, stepLengthInVoxel, lx * lyc, pxsyi, idx) +
          m_InterpolationWeightMultiplication(0, stepLengthInVoxel, lxc * ly, pxiys, idx) +
          m_InterpolationWeightMultiplication(0, stepLengthInVoxel, lx * ly, pxsys, idx));
}
  ...
  const CoordinateType   maxy)
{
  ...
  result += m_InterpolationWeightMultiplication(0, stepLengthInVoxel, lxc * lyc, pxiyi, idx + offset_xi + offset_yi);
  result += m_InterpolationWeightMultiplication(0, stepLengthInVoxel, lxc * ly, pxiys, idx + offset_xi + offset_ys);
  result += m_InterpolationWeightMultiplication(0, stepLengthInVoxel, lx * lyc, pxsyi, idx + offset_xs + offset_yi);
  result += m_InterpolationWeightMultiplication(0, stepLengthInVoxel, lx * ly, pxsys, idx + offset_xs + offset_ys);
