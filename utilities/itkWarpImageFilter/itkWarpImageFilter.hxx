...
::EvaluateDisplacementAtPhysicalPoint(const PointType & point, DisplacementType &output)
{
  ...
  for ( dim = 0; dim < ImageDimension; dim++ )
    {
    baseIndex[dim] = Math::Floor< IndexValueType, double >(index[dim]);

    if ( ( baseIndex[dim] >=  m_StartIndex[dim] ) && (baseIndex[dim] <  m_EndIndex[dim] ) )
      distance[dim] = index[dim] - static_cast< double >( baseIndex[dim] );
    else // If the point is outside the DVF, assume that there is no movement
      skip = true;
    }
