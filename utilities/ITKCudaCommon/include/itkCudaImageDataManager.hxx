...
CudaImageDataManager<ImageType>::MakeCPUBufferUpToDate()
{
  if (m_Image)
  {
     ...
     */
    if ((m_IsCPUBufferDirty || (gpu_time_stamp > cpu_time_stamp)) && m_GPUBuffer.GetPointer() != NULL &&
        m_CPUBuffer != NULL)
    {
      cudaError_t errid;
#ifdef VERBOSE
...
CudaImageDataManager<ImageType>::MakeGPUBufferUpToDate()
{
  if (m_Image)
  {
     ...
     */
    if ((m_IsGPUBufferDirty || (gpu_time_stamp < cpu_time_stamp)) && m_CPUBuffer != NULL &&
        m_GPUBuffer.GetPointer() != NULL)
    {
      cudaError_t errid;
#ifdef VERBOSE
