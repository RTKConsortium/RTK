...
public:
  ...
  bool
  LoadProgramFromFile(const char * filename);
  bool
  LoadProgramFromString(const char * str);

  int
  CreateKernel(const char * kernelName);

  int
  CreateKernel(const char * kernelName, const std::type_info &);

  bool
  PushKernelArg(int kernelIdx, const void * argVal);

  void
  ClearKernelArgs(int kernelIdx);

  bool
  SetKernelArg(int kernelIdx, int argIdx, size_t argSize, const void * argVal);

  bool
  SetKernelArgWithImage(int kernelIdx, int argIdx, CudaDataManager::Pointer manager);

  template <class TPixel, unsigned int VImageDimension>
  bool
  SetKernelArgWithImage(int kernelIdx, int argIdx, SmartPointer<CudaImage<TPixel, VImageDimension>> image)
  {
  ...
  bool
  SetKernelArgWithImageAndBufferedRegion(int kernelIdx, int & argIdx, TCudaImageDataManager * manager)
  {
  ...
  bool
  LaunchKernel1D(int kernelIdx, size_t globalWorkSize, size_t localWorkSize, unsigned int sharedMemBytes = 0);
...
protected:
  ...
  bool
  GetKernelParams(int kernelIdx, std::vector<void *> & params);
