...
void
CudaUnaryFunctorImageFilter<TInputImage, TOutputImage, TFunction, TParentImageFilter>::GenerateOutputInformation()
{
...
template <class TInputImage, class TOutputImage, class TFunction, class TParentImageFilter>
void
CudaUnaryFunctorImageFilter<TInputImage, TOutputImage, TFunction, TParentImageFilter>::GPUGenerateData()
{
  ...
  int argidx =
    (this->GetFunctor()).SetCudaKernelArguments(this->m_CudaKernelManager, m_UnaryFunctorImageFilterCudaKernelHandle);
  // arguments set up
  this->m_CudaKernelManager->SetKernelArgWithImage(
    m_UnaryFunctorImageFilterCudaKernelHandle, argidx++, inPtr->GetCudaDataManager());
  this->m_CudaKernelManager->SetKernelArgWithImage(
    m_UnaryFunctorImageFilterCudaKernelHandle, argidx++, otPtr->GetCudaDataManager());
  for (int i = 0; i < (int)TInputImage::ImageDimension; i++)
  {
    this->m_CudaKernelManager->SetKernelArg(
      m_UnaryFunctorImageFilterCudaKernelHandle, argidx++, sizeof(int), &(imgSize[i]));
  }
