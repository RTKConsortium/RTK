...
template <class TElement>
void
CudaReduction<TElement>::PrintSelf(std::ostream & os, Indent indent) const
{
...
template <class TElement>
unsigned int
CudaReduction<TElement>::NextPow2(unsigned int x)
{
  --x;
  x |= x >> 1;
  x |= x >> 2;
...
template <class TElement>
bool
CudaReduction<TElement>::isPow2(unsigned int x)
{
  return ((x & (x - 1)) == 0);
}
...
template <class TElement>
void
CudaReduction<TElement>::GetNumBlocksAndThreads(int   whichKernel,
                                                int   n,
                                                int   maxBlocks,
                                                int   maxThreads,
                                                int & blocks,
...
template <class TElement>
unsigned int
CudaReduction<TElement>::GetReductionKernel(int whichKernel, int blockSize, int isPowOf2)
{
...
template <class TElement>
void
CudaReduction<TElement>::AllocateGPUInputBuffer(TElement * h_idata)
{
...
template <class TElement>
TElement
CudaReduction<TElement>::RandomTest()
{
...
template <class TElement>
void
CudaReduction<TElement>::InitializeKernel(unsigned int size)
{
  m_Size = size;
...
template <class TElement>
TElement
CudaReduction<TElement>::GPUGenerateData()
{
...
template <class TElement>
TElement
CudaReduction<TElement>::GPUReduce(int             n,
                                   int             numThreads,
                                   int             numBlocks,
                                   ...
                                   CudaDataPointer odata)
{
  ...
  int sharedMemSize = (numThreads <= 32) ? 2 * numThreads * sizeof(TElement) : numThreads * sizeof(TElement);

  // execute the kernel
  this->m_CudaKernelManager->LaunchKernel1D(m_ReduceCudaKernelHandle, globalSize, localSize, sharedMemSize);
...
template <class TElement>
TElement
CudaReduction<TElement>::CPUGenerateData(TElement * data, int size)
{
  TElement sum = 0;
  TElement c = 0;
  for (int i = 0; i < size; i++)
