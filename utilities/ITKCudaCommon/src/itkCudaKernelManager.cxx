...
bool
CudaKernelManager::LoadProgramFromFile(const char * filename)
{
  //
...
bool
CudaKernelManager::LoadProgramFromString(const char * str)
{
  //
  // Create CUDA module from cubin or PTX file
  //
...
int
CudaKernelManager::CreateKernel(const char * kernelName)
{
  CUfunction func;
...
int
CudaKernelManager::CreateKernel(const char * kernelName, const std::type_info & type)
{
  std::stringstream s;
...
bool
CudaKernelManager::PushKernelArg(int kernelIdx, const void * argVal)
{
...
bool
CudaKernelManager::SetKernelArg(int kernelIdx, int argIdx, size_t itkNotUsed(argSize), const void * argVal)
{
  if (kernelIdx < 0 || kernelIdx >= (int)m_KernelContainer.size())
    return false;
...
bool
CudaKernelManager::SetKernelArgWithImage(int kernelIdx, int argIdx, CudaDataManager::Pointer manager)
{
  if (kernelIdx < 0 || kernelIdx >= (int)m_KernelContainer.size())
    return false;
...
bool
CudaKernelManager::CheckArgumentReady(int kernelIdx)
{
  if (kernelIdx < 0 || kernelIdx >= (int)m_KernelContainer.size())
    return false;
...
void
CudaKernelManager::ClearKernelArgs(int kernelIdx)
{
  if (kernelIdx < 0 || kernelIdx >= (int)m_KernelContainer.size())
    return;
...
void
CudaKernelManager::ResetArguments(int kernelIdx)
{
  if (kernelIdx < 0 || kernelIdx >= (int)m_KernelContainer.size())
    return;
...
bool
CudaKernelManager::GetKernelParams(int kernelIdx, std::vector<void *> & params)
{
  if (kernelIdx < 0 || kernelIdx >= (int)m_KernelContainer.size())
    return false;
...
bool
CudaKernelManager::LaunchKernel1D(int          kernelIdx,
...
bool
CudaKernelManager::LaunchKernel2D(int          kernelIdx,
...
bool
CudaKernelManager::LaunchKernel3D(int          kernelIdx,
                                  size_t       globalWorkSizeX,
                                  size_t       globalWorkSizeY,
...
bool
CudaKernelManager::LaunchKernel(int          kernelIdx,
                                int          itkNotUsed(dim),
                                size_t *     globalWorkSize,
                                size_t *     localWorkSize,
                                unsigned int sharedMemBytes)
{
  ...
  std::cout << " globalWorkSize: " << globalWorkSize[0] << ", " << globalWorkSize[1] << ", " << globalWorkSize[2]
            << std::endl;
  std::cout << " localWorkSize: " << localWorkSize[0] << ", " << localWorkSize[1] << ", " << localWorkSize[2]
            << std::endl;
