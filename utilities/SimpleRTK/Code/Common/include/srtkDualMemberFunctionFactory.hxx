 ...
 *
 *=========================================================================*/
#ifndef __srtkDualMemberFunctionFactory_hxx
#define __srtkDualMemberFunctionFactory_hxx

#include "srtkDualMemberFunctionFactory.h"

#include "srtkPixelIDTokens.h"
...
::Register( MemberFunctionType pfunc,  TImageType1*, TImageType2*  )
{
  ...
  srtkStaticAssert( TImageType1::ImageDimension == 2 || TImageType1::ImageDimension == 3,
                    "Image Dimension out of range" );
  srtkStaticAssert( int(TImageType1::ImageDimension) == int(TImageType2::ImageDimension),
                    "Image Dimensions do not match" );
  srtkStaticAssert( IsInstantiated<TImageType1>::Value,
                    "invalid pixel type for argument one");
  srtkStaticAssert( IsInstantiated<TImageType2>::Value,
...
::GetMemberFunction( PixelIDValueType pixelID1, PixelIDValueType pixelID2, unsigned int imageDimension  )
{
  if ( pixelID1 >= typelist::Length< InstantiatedPixelIDTypeList >::Result || pixelID1 < 0 )
    {
    srtkExceptionMacro ( << "unexpected error pixelID1 is out of range " << pixelID1 << " "  << typeid(ObjectType).name() );
    }

  if ( pixelID2 >= typelist::Length< InstantiatedPixelIDTypeList >::Result || pixelID2 < 0 )
    {
    srtkExceptionMacro ( << "unexpected error pixelID2 is out of range " << pixelID2 << " "  << typeid(ObjectType).name() );
    }
  ...
  switch ( imageDimension )
    ...
    case 2:
      ...
      break;
    default:
      srtkExceptionMacro ( << "Image dimension " << imageDimension << " is not supported" );
    }
}
...
#endif //  __srtkDualMemberFunctionFactory_hxx
