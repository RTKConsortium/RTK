...
namespace simple
{
  ...
  /** \class Image
   * \brief The main Image class for SimpleRTK
   */
  class SRTKCommon_EXPORT Image
  {
  public:
    typedef Image              Self;
    ...
    /** \brief Construct an SimpleRTK Image from an pointer to an ITK
     * image
     *
     * The SimpleRTK image will add a reference to the underlying the
     * ITK image and hold a pointer to the image. If the image is
     * manipulated directly from the ITK interface, SimpleRTK may be
    ...
    explicit Image( itk::SmartPointer<TImageType> image )
      : m_PimpleImage( NULL )
      {
        srtkStaticAssert( ImageTypeToPixelIDValue<TImageType>::Result != (int)srtkUnknown,
                          "invalid pixel type" );
     ...
     * follows standard SimpleRTK conventions for it's length. The
  ...
  private:
   ...
   /** Method called by certain constructors to convert ITK images
     ...
     * the external SimpleRTK interface. Template parameters have been
