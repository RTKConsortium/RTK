 ...
 *
 *=========================================================================*/
#ifndef __srtkTransform_h
#define __srtkTransform_h
...
// differently.
#if ( ( SRTK_ITK_VERSION_MAJOR == 4 ) && ( SRTK_ITK_VERSION_MINOR < 5 ) )
class TransformBase;
#else
...
enum TransformEnum { srtkIdentity,
                     srtkTranslation,
                     srtkScale,
                     srtkScaleLogarithmic,
                     srtkEuler,
                     srtkSimilarity,
                     srtkQuaternionRigid,
                     srtkVersor,
 ...
 *
 */
class SRTKCommon_EXPORT Transform
{
public:
  ...
  /** \brief Construct a SimpleRTK Transform from a pointer to an ITK
   * composite transform.
   *
   */
  ...
  explicit Transform( itk::CompositeTransform< double, NDimension >* compositeTransform )
    {
      srtkStaticAssert( NDimension == 2 || NDimension == 3, "Only 2D and 3D transforms are supported" );
      if ( compositeTransform == NULL )
        {
        srtkExceptionMacro( "Unable to construct a null transform!" );
        }
      this->InternalInitialization<NDimension>( srtkComposite, compositeTransform );
    }
...
// read
SRTKCommon_EXPORT Transform ReadTransform( const std::string &filename );

// write
SRTKCommon_EXPORT void WriteTransform( const Transform &transform, const std::string &filename);

}
}

#endif // __srtkTransform_h
