 ...
 *
 *=========================================================================*/
#ifndef __srtkMemberFunctionFactory_hxx
#define __srtkMemberFunctionFactory_hxx
...
::Register( typename MemberFunctionFactory::MemberFunctionType pfunc,  TImageType*  )
{
  ...
  srtkStaticAssert( TImageType::ImageDimension == 2 || TImageType::ImageDimension == 3,
                    "Image Dimension out of range" );
  srtkStaticAssert( IsInstantiated<TImageType>::Value,
                    "invalid pixel type");
...
::GetMemberFunction( PixelIDValueType pixelID, unsigned int imageDimension  )
{
  if ( pixelID >= typelist::Length< InstantiatedPixelIDTypeList >::Result || pixelID < 0 )
    {
    srtkExceptionMacro ( << "unexpected error pixelID is out of range " << pixelID << " "  << typeid(ObjectType).name() );
    }
...
#endif //  __srtkMemberFunctionFactory_h
