...
public:
  ...
  virtual itk::TransformBase::Pointer GetTransformBase( void ) = 0;
  virtual itk::TransformBase::ConstPointer GetTransformBase( void ) const = 0;
...
public:
  ...
  virtual itk::TransformBase::Pointer GetTransformBase( void ) { return this->m_Transform.GetPointer(); }
  virtual itk::TransformBase::ConstPointer GetTransformBase( void ) const { return this->m_Transform.GetPointer(); }
  ...
  virtual PimpleTransformBase* AddTransform( Transform &t )
    {
      if ( t.GetDimension() != TransformType::InputSpaceDimension )
        {
        srtkExceptionMacro( "Transform argument has dimension " << t.GetDimension()
  ...
  virtual std::vector< double > TransformPoint( const std::vector< double > &pt ) const
    {
      ...
      typename TransformType::OutputPointType opt =
        this->m_Transform->TransformPoint( srtkSTLVectorToITK< typename TransformType::InputPointType >(pt));

      return srtkITKVectorToSTL<double>( opt );
    }
...
Transform::Transform( )
    ...
    : m_PimpleTransform( NULL )
  {
    if ( dimensions == 2 )
      ...
      {
      srtkExceptionMacro("Invalid dimension for transform");
      }
  }
...
void Transform::MakeUniqueForWrite( void )
{
  ...
  void  Transform::InternalInitialization(  TransformEnum type, itk::TransformBase *base )
  {
    switch( type )
      ...
      case srtkSimilarity:
        ...
        break;
      case srtkQuaternionRigid:
        if( VDimension != 3)
          {
          srtkExceptionMacro( "A srtkQuaternionRigid Transform only works for 3D!");
          }
      ...
      case srtkVersor:
        if( VDimension != 3)
          {
          srtkExceptionMacro( "A srtkVersor Transform only works for 3D!");
          }

        m_PimpleTransform = new PimpleTransform<itk::VersorTransform< double > >();
        break;
      case srtkVersorRigid:
        if( VDimension != 3)
          {
          srtkExceptionMacro( "A srtkVersorRigid Transform only works for 3D!");
          }
      ...
      case srtkComposite:
        ...
        if ( !base )
          ...
          if ( !compositeTransform )
            {
            srtkExceptionMacro("Unexpectedly unable to convert to CompositeTransform" );
            }
          }
  ...
  Transform ReadTransform( const std::string &filename )
  {
    ...
    if ( list->empty() )
      {
      srtkExceptionMacro( "Read transform file: \"" << filename << "\", but there appears to be not transform in the file!" );
      }
    ...
    srtkExceptionMacro( "Unable to transform with InputSpaceDimension: " <<  list->front()->GetInputSpaceDimension()
  ...
  {
    rtk::simple::WriteTransform( *this, filename );
  }
