  ...
  Image& Image::operator=( const Image &img )
  {
      ...
      : m_PimpleImage( NULL )
    {
      if ( size.size() == 2 )
        ...
        {
        srtkExceptionMacro("Unsuported number of dimesions specified!");
        }
    }
    ...
    void Image::CopyInformation( const Image & srcImage )
    {
      ...
      if ( this->GetDimension() != srcImage.GetDimension() )
        {
        srtkExceptionMacro( "Source Image for information does not match this image's dimension." );
        }
      ...
      if ( this->GetSize() != srcImage.GetSize() )
        {
        srtkExceptionMacro( "Source image size of " << srcImage.GetSize()
