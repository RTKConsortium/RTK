 ...
 *
 *=========================================================================*/
#ifndef __srtkPimpleImageBase_hxx
#define __srtkPimpleImageBase_hxx

#include "srtkPimpleImageBase.h"
...
///
  ...
  public:
    ...
    PimpleImage ( ImageType* image )
      : m_Image( image )
      {
        srtkStaticAssert( ImageType::ImageDimension == 3 || ImageType::ImageDimension == 2,
                          "Image Dimension out of range" );
        srtkStaticAssert( ImageTypeToPixelIDValue<ImageType>::Result != (int)srtkUnknown,
                          "invalid pixel type" );

        if ( image == NULL )
          {
          srtkExceptionMacro( << "Unable to initialize an image with NULL" );
          }

        if ( image->GetLargestPossibleRegion() != image->GetBufferedRegion() )
          {
          srtkExceptionMacro( << "The image has a LargestPossibleRegion of " << image->GetLargestPossibleRegion()
                              << " while the buffered region is " << image->GetBufferedRegion() << std::endl
                              << "SimpleRTK does not support streamming or unbuffered regions!" );
          }
        ...
        for ( unsigned int i = 0; i < ImageType::ImageDimension; ++i )
          ...
          if ( idx[i] != 0 )
            {
            srtkExceptionMacro( << "The image has a staring index of " << idx
                                << "SimpleRTK only supports images with a zero starting index!" );
            }
          }
      }
    ...
    DeepCopy( void ) const
      {
        srtkExceptionMacro( "This method is not implemented yet" );
    ...
    virtual std::vector<double> GetOrigin( void ) const
      {
        return srtkITKVectorToSTL<double>( this->m_Image->GetOrigin() );
      }

    // Set Origin
    virtual void SetOrigin( const std::vector<double> & orgn )
      {
        this->m_Image->SetOrigin( srtkSTLVectorToITK< typename ImageType::PointType> ( orgn ) );
      }

    // Get Spacing
    virtual std::vector<double> GetSpacing( void ) const
      {
        return srtkITKVectorToSTL<double>( this->m_Image->GetSpacing() );
      }

    // Set Spacing
    virtual void SetSpacing( const std::vector<double> &spc )
      {
        this->m_Image->SetSpacing( srtkSTLVectorToITK< typename ImageType::SpacingType> ( spc ) );
      }
    ...
    virtual void SetDirection( const std::vector< double > & in )
      ...
      if (in.size() != ImageType::ImageDimension*ImageType::ImageDimension)
        {
        srtkExceptionMacro("direction size mismatch");
        }
    ...
    virtual std::vector<int64_t> TransformPhysicalPointToIndex( const std::vector<double> &pt ) const
      {
        ...
        {
        srtkExceptionMacro("vector dimension mismatch");
        }


        typename ImageType::IndexType index;
        this->m_Image->TransformPhysicalPointToIndex( srtkSTLVectorToITK< typename ImageType::PointType> ( pt ), index);

        return srtkITKVectorToSTL<int64_t>( index );
      }
    ...
    virtual std::vector<double> TransformIndexToPhysicalPoint( const std::vector<int64_t> &idx ) const
      {
        ...
        return srtkITKVectorToSTL<double>( point );
      }
    ...
    virtual std::vector<double> TransformPhysicalPointToContinuousIndex( const std::vector<double> &pt ) const
      {
        ...
        this->m_Image->TransformPhysicalPointToContinuousIndex(srtkSTLVectorToITK< typename ImageType::PointType> ( pt ), index);

        return srtkITKVectorToSTL<double>( index );
      }
    ...
    virtual std::vector<unsigned int> GetSize( void ) const
      {
        ...
        return srtkITKVectorToSTL<unsigned int>( largestRegion.GetSize() );
      }
  ...
  protected:
    ...
    InternalGetPixel( const std::vector<uint32_t> &idx ) const
      {
        const IndexType itkIdx = srtkSTLVectorToITK<IndexType>( idx );
        if ( ! this->m_Image->GetLargestPossibleRegion().IsInside( itkIdx ) )
          {
          srtkExceptionMacro( "index out of bounds" );
          }
    ...
    InternalGetPixel( const std::vector<uint32_t> &idx ) const
      {
        Unused( idx );
        srtkExceptionMacro( << "The image is of type: " << GetPixelIDValueAsString( this->GetPixelID() )
    ...
    InternalGetBuffer( void )
      {
        srtkExceptionMacro( "This method is not supported for LabelMaps." )
      }
    ...
    template < typename TPixelIDType >
    typename DisableIf<nsstd::is_same<TPixelIDType, typename ImageTypeToPixelID<ImageType>::PixelIDType>::value,
                       typename itk::NumericTraits<typename PixelIDToImageType<TPixelIDType,2>::ImageType::PixelType>::ValueType *>::Type
    InternalGetBuffer( void )
      {
        srtkExceptionMacro( << "The image is of type: " << GetPixelIDValueAsString( this->GetPixelID() )
    ...
    InternalSetPixel( const std::vector<uint32_t> &idx, const std::vector<TPixelValueType> & v  ) const
      {
        ...
        if ( px.GetSize()  != v.size() )
          {
          srtkExceptionMacro(<<"Unable to convert vector to ITK pixel type\n"
...
#endif // __srtkPimpleImageBase_hxx
