...
// Create an ITK EventObject from the SimpleRTK enumerated type.
...
bool rm_pred(const rtk::simple::Command *cmd, const std::pair<EventEnum, Command*> &i) throw()
{
  return cmd == i.second;
}
...
void ProcessObject::onCommandDelete(const rtk::simple::Command *cmd) throw()
{
  if (this->m_ActiveProcess)
    ...
    std::cerr << "srtk::Fatal: Cannot delete Command during execution!" << std::endl;
    std::terminate();
