...
namespace rtk
{
  ...
  itksys::SystemTools::GetEnv ( "SRTK_SHOW_EXTENSION", Extension );
  ...
  if ( !itksys::SystemTools::GetEnv ( "TMP", TempDirectory )
    ...
    {
    srtkExceptionMacro ( << "Can not find temporary directory.  Tried TMP, TEMP, USERPROFILE, and WINDIR environment variables" );
    }
  ...
  static void ExecuteShow( const std::vector<std::string> & cmdLine )
  {
    ...
    switch (itksysProcess_GetState(kp))
      ...
      case itksysProcess_State_Exited:
        ...
        if ( exitValue != 0 )
          {
          srtkExceptionMacro (  << "Process returned " << exitValue << "." );
          }
        }
        break;
      ...
      case itksysProcess_State_Killed:
        ...
        srtkExceptionMacro (  << "Child was killed by parent." );
        break;
      ...
      case itksysProcess_State_Exception:
        ...
        itksysProcess_Delete( kp );
        srtkExceptionMacro (  << "Child terminated abnormally: " << exceptionString );;
        }
        break;
      ...
      case itksysProcess_State_Error:
        ...
        itksysProcess_Delete( kp );
        srtkExceptionMacro (  << "Error in administrating child process: [" << errorString << "]" );
        }
        break;
      ...
      default:
        itksysProcess_Delete( kp );
        srtkExceptionMacro (  << "Unexpected process state!" );
      };
  ...
  colorFlag = ( (image.GetNumberOfComponentsPerPixel() == 3)
                  && ((image.GetPixelIDValue()==srtkVectorUInt8) || (image.GetPixelIDValue()==srtkVectorUInt16)) );
  ...
  if (colorFlag)
    {
    itksys::SystemTools::GetEnv ( "SRTK_SHOW_COLOR_COMMAND", Command );
    if (!Command.length())
      {
      itksys::SystemTools::GetEnv ( "SRTK_SHOW_COMMAND", Command );
      }
    if (!Command.length())
      {
  ...
  itksys::SystemTools::GetEnv ( "SRTK_SHOW_3D_COMMAND", Command3D );
  if (!Command3D.length())
    {
    Command3D = Command;
    }
