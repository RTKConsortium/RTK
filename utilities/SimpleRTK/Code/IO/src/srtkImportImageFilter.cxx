...
std::string ImportImageFilter::ToString() const
{
  ...
  out << "rtk::simple::ImportImageFilter\n"
...
Image ImportImageFilter::Execute ()
{
  ...
  if (  this->m_NumberOfComponentsPerPixel == 0 || this->m_PixelIDValue == srtkUnknown )
    {
    srtkExceptionMacro( << "PixelType or NumberOfComponentsPerPixel are invalid!" );
    }


  if ( imageDimension != 3 && imageDimension != 2 )
    {
    srtkExceptionMacro(<< "The length of size is invalid! "
...
Image ImportImageFilter::ExecuteInternal( )
{
  ...
  typename ImageType::SizeType size = srtkSTLVectorToITK< typename ImageType::SizeType >( this->m_Size );
  region.SetSize(size);
