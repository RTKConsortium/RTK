...
TEST_F(Import,Required) {
  ...
  // Check has Self
  typedef rtk::simple::ImportImageFilter::Self ImportFilterType;
...
TEST_F(Import,BasicUsage) {
  ...
  srtk::ImportImageFilter importer;
  ...
  srtk::Image image = importer.Execute();
  ...
  EXPECT_EQ ( "a2178ce2d158a4a7c5a9ef3d03a33a6099b9c5be", srtk::Hash( image ) ) << " hash value for basic uin8_t";
  ...
  EXPECT_EQ ( "6f6eefc4c1833e3e29b0968023ead2141fdbee3f", srtk::Hash( image ) ) << " hash value for basic uin32_t";
}
...
TEST_F(Import,Direction) {
  ...
  srtk::Image img(0,0,srtk::srtkUInt8);
...
TEST_F(Import,ExhaustiveTypes) {
  ...
  importer.SetBufferAsUInt8( &uint8_buffer[0] );
  EXPECT_EQ ( "ac458b067c6b021c7e9358229b636e9d1e4cb154", srtk::Hash( importer.Execute() ) ) << " hash value for uint8";

  int8_buffer = std::vector< int8_t >( 16*16, -1 );
  importer.SetBufferAsInt8( &int8_buffer[0] );
  EXPECT_EQ ( "c744cac6af7621524fc3a2b0a9a135a32b33c81b", srtk::Hash( importer.Execute() ) ) << " hash value for int8";

  uint16_buffer = std::vector< uint16_t >( 16*16, 2 );
  importer.SetBufferAsUInt16( &uint16_buffer[0] );
  EXPECT_EQ ( "adbc44d359d3b3c032629bab34e3ee62c4563223", srtk::Hash( importer.Execute() ) ) << " hash value for uint16";

  int16_buffer = std::vector< int16_t >( 16*16, -3 );
  importer.SetBufferAsInt16( &int16_buffer[0] );
  EXPECT_EQ ( "8111171305e02db2276c7366d117598b1f6789ff", srtk::Hash( importer.Execute() ) ) << " hash value for int16";

  uint32_buffer = std::vector< uint32_t >( 16*16, 8 );
  importer.SetBufferAsUInt32( &uint32_buffer[0] );
  EXPECT_EQ ( "4b710847b34bc5e96520de8a1d7b97563077423c", srtk::Hash( importer.Execute() ) ) << " hash value for uint32";

  int32_buffer = std::vector< int32_t >( 16*16, -123 );
  importer.SetBufferAsInt32( &int32_buffer[0] );
  EXPECT_EQ ( "d5748f3800b837611dc94c089ac7926f2bbea973" , srtk::Hash( importer.Execute() ) ) << " hash value for int32";

  if ( srtk::srtkUInt64 != srtk::srtkUnknown )
    {
    uint64_buffer = std::vector< uint64_t >( 16*16, 8 );
    importer.SetBufferAsUInt64( &uint64_buffer[0] );
    EXPECT_EQ ( "95c3884597103a98cf6f1def52cab572c41f0eb5", srtk::Hash( importer.Execute() ) ) << " hash value for uint64";
    }
  ...
  if ( srtk::srtkInt64 != srtk::srtkUnknown )
    ...
    EXPECT_EQ ( "d0a23a11b2f39b46cfc09bd71fc4c9661b68a826" , srtk::Hash( importer.Execute() ) ) << " hash value for int64";
    }

  float_buffer = std::vector< float >( 16*16, 1.123 );
  importer.SetBufferAsFloat( &float_buffer[0] );
  EXPECT_EQ ( "8588f5624f56bb55693d54505388dc06b93d2f14", srtk::Hash( importer.Execute() ) ) << " hash value for float";

  double_buffer = std::vector< double >( 16*16, 321.123 );
  importer.SetBufferAsDouble( &double_buffer[0] );
  EXPECT_EQ ( "00393fe0b94729ff0f777ce848104697ce689c70", srtk::Hash( importer.Execute() ) ) << " hash value for double";


  uint8_buffer = std::vector< uint8_t >( 16*16*2, 1 );
  importer.SetBufferAsUInt8( &uint8_buffer[0], 2 );
  EXPECT_EQ ( "3ef8a08ec90e244fe2a8948b701eafcc1d065712", srtk::Hash( importer.Execute() ) ) << " hash value for vector of uint8";

  int8_buffer = std::vector< int8_t >( 16*16*3, -1 );
  importer.SetBufferAsInt8( &int8_buffer[0], 3 );
  EXPECT_EQ ( "fc1b469729f5a061d4b8b5eb1efba13a7b52d5a7", srtk::Hash( importer.Execute() ) ) << " hash value for vector of int8";

  uint16_buffer = std::vector< uint16_t >( 16*16*4, 2 );
  importer.SetBufferAsUInt16( &uint16_buffer[0], 4 );
  EXPECT_EQ ( "f449215906661bacee88ef886c38f3049d9a232b", srtk::Hash( importer.Execute() ) ) << " hash value for vector of uint16";

  int16_buffer = std::vector< int16_t >( 16*16*5, -3 );
  importer.SetBufferAsInt16( &int16_buffer[0], 5 );
  EXPECT_EQ ( "7bce93764d2393b987ed2b17281073ddb520af46", srtk::Hash( importer.Execute() ) ) << " hash value for vector of int16";

  uint32_buffer = std::vector< uint32_t >( 16*16*6, 8 );
  importer.SetBufferAsUInt32( &uint32_buffer[0], 6 );
  EXPECT_EQ ( "c558884e4bf07ece6a2644307aa3b4eb84be8a11", srtk::Hash( importer.Execute() ) ) << " hash value for vector of uint32";

  int32_buffer = std::vector< int32_t >( 16*16*7, -123 );
  importer.SetBufferAsInt32( &int32_buffer[0], 7 );
  EXPECT_EQ ( "c65dc5820f3691fc4b6b3b9aaba1f506ca3f697e" , srtk::Hash( importer.Execute() ) ) << " hash value for vector of int32";
  ...
  if ( srtk::srtkVectorUInt64 != srtk::srtkUnknown )
    ...
    EXPECT_EQ ( "cac37cb71f23cd41ed1e2b60d1e325ee8de4ca71", srtk::Hash( importer.Execute() ) ) << " hash value for vector of uint64";
    }
  ...
  if ( srtk::srtkVectorInt64 != srtk::srtkUnknown )
    ...
    EXPECT_EQ ( "91a61e519faf128747bf2d2bbd860d4f05d79ac6" , srtk::Hash( importer.Execute() ) ) << " hash value for vector of int64";
    }

  float_buffer = std::vector< float >( 16*16*8, 1.123 );
  importer.SetBufferAsFloat( &float_buffer[0], 8 );
  EXPECT_EQ ( "9fb1d83b9c9a5645e7b136761d6924ea7d859284", srtk::Hash( importer.Execute() ) ) << " hash value for vector of float";

  double_buffer = std::vector< double >( 16*16*9, 321.123 );
  importer.SetBufferAsDouble( &double_buffer[0], 9 );
  EXPECT_EQ ( "80514b97613fc659c9ceb45bb44c8a02fec9c4db", srtk::Hash( importer.Execute() ) ) << " hash value for vector of double";
...
TEST_F(Import,Procedual) {
  ...
  uint8_buffer = std::vector< uint8_t >( 16*16, 1 );
  img = srtk::ImportAsUInt8( &uint8_buffer[0],
                             size,
                             spacing2,
                             origin2,
                             direction2D );
  EXPECT_EQ ( "ac458b067c6b021c7e9358229b636e9d1e4cb154", srtk::Hash( img ) ) << " hash value for uint8";
  ...
  int8_buffer = std::vector< int8_t >( 16*16, -1 );
  img = srtk::ImportAsInt8( &int8_buffer[0],
                            size,
                            spacing2,
                            origin2,
                            direction2D );
  EXPECT_EQ ( "c744cac6af7621524fc3a2b0a9a135a32b33c81b", srtk::Hash( img ) ) << " hash value for int8";
  ...
  uint16_buffer = std::vector< uint16_t >( 16*16, 2 );
  img = srtk::ImportAsUInt16( &uint16_buffer[0],
                              size,
                              spacing2,
                              origin2,
                              direction2D );
  EXPECT_EQ ( "adbc44d359d3b3c032629bab34e3ee62c4563223", srtk::Hash( img ) ) << " hash value for uint16";
  ...
  int16_buffer = std::vector< int16_t >( 16*16, -3 );
  img = srtk::ImportAsInt16( &int16_buffer[0],
                             size,
                             spacing2,
                             origin2,
                             direction2D );
  EXPECT_EQ ( "8111171305e02db2276c7366d117598b1f6789ff", srtk::Hash( img ) ) << " hash value for int16";
  ...
  uint32_buffer = std::vector< uint32_t >( 16*16, 8 );
  img = srtk::ImportAsUInt32( &uint32_buffer[0],
                              size,
                              spacing2,
                              origin2,
                              direction2D );
  EXPECT_EQ ( "4b710847b34bc5e96520de8a1d7b97563077423c", srtk::Hash( img ) ) << " hash value for uint32";
  ...
  int32_buffer = std::vector< int32_t >( 16*16, -123 );
  img = srtk::ImportAsInt32( &int32_buffer[0],
                             size,
                             spacing2,
                             origin2,
                             direction2D );
  EXPECT_EQ ( "d5748f3800b837611dc94c089ac7926f2bbea973" , srtk::Hash( img ) ) << " hash value for int32";
  ...
  float_buffer = std::vector< float >( 16*16, 1.123 );
  img = srtk::ImportAsFloat( &float_buffer[0],
                             size,
                             spacing2,
                             origin2,
                             direction2D );
  EXPECT_EQ ( "8588f5624f56bb55693d54505388dc06b93d2f14", srtk::Hash( img ) ) << " hash value for float";
  ...
  double_buffer = std::vector< double >( 16*16, 321.123 );
  img = srtk::ImportAsDouble( &double_buffer[0],
                              size,
                              spacing2,
                              origin2,
                              direction2D );
  EXPECT_EQ ( "00393fe0b94729ff0f777ce848104697ce689c70", srtk::Hash( img ) ) << " hash value for double";
