...
std::vector<std::string> DataFinder::GetLuaExecutable ()
  {
  return this->FindExecutable ( "SimpleRTKLua" );
  }

std::vector<std::string> DataFinder::GetTclExecutable ()
  {
  return this->FindExecutable ( "SimpleRTKTclsh" );
  }
...
void ExternalProgramRunner::CheckImageHash( const std::string &fileName, const std::string &hash )
  ...
  rtk::simple::Image image = rtk::simple::ReadImage( fileName );
  EXPECT_EQ ( hash, rtk::simple::Hash( image ) );
  }
...
ProcessObjectCommand::ProcessObjectCommand(rtk::simple::ProcessObject &po)
  : m_Process(po)
{
}

ProgressUpdate::ProgressUpdate(rtk::simple::ProcessObject &po)
...
AbortAtCommand::AbortAtCommand(rtk::simple::ProcessObject &po, float abortAt)
  : ProcessObjectCommand(po),
    m_AbortAt(abortAt)
{
}
...
CountCommand::CountCommand(rtk::simple::ProcessObject &po)
  : ProcessObjectCommand(po),
    m_Count(0)
{
}
