...
testimA <- ReadImage(system.file("data/cthead1-Float.mha", package="SimpleRTK"))
testimB <- ReadImage(system.file("data/cthead1-Float.mha", package="SimpleRTK"))

# need to cast to signed types to compare with R arithmetic
testimInt <- ReadImage(system.file("data/cthead1.png", package="SimpleRTK"))
testimInt <- Cast(testimInt, 'srtkInt16')
...
powTest <- function()
  {
    ...
    # there isn't a SimpleRTK function for this
    #ImC <- 3.1^testimA
    #AC <- 3.1^testArrayA
    #if (!compareImArrayTol(ImC, AC)) {
    #  cat("failure in", fname, "\n")
...
modTest <- function()
  {
    ...
    ## for float in underlying srtk filter
    ImC <- testimInt %% 3
