...
#include "srtkAddImageFilter.h"
#include "srtkSubtractImageFilter.h"
#include "srtkMultiplyImageFilter.h"

#include "srtkImageOperators.h"

#include "srtkComplexToRealImageFilter.h"
#include "srtkComplexToImaginaryImageFilter.h"
#include "srtkRealAndImaginaryToComplexImageFilter.h"
...
using  rtk::simple::InstantiatedPixelIDTypeList;


class Image : public ::testing::Test {
public:
  typedef std::auto_ptr<rtk::simple::Image> srtkAutoImagePointer;
  ...
  virtual void SetUp() {
    ...
    fim->FillBuffer ( 0 );
    itkFloatImage = fim;
    floatImage = srtkAutoImagePointer( new rtk::simple::Image( fim.GetPointer() ) );

    itkFloatVectorImage = FloatVectorImageType::New();
    floatVectorImage = srtkAutoImagePointer( new rtk::simple::Image( itkFloatVectorImage.GetPointer() ) );

    itkFloatVector2DImage = FloatVector2DImageType::New();
    floatVector2DImage = srtkAutoImagePointer( new rtk::simple::Image( itkFloatVector2DImage ) );
...
TEST_F(Image,ImageDataType) {
  ...
  result = typelist::IndexOf< InstantiatedPixelIDTypeList, rtk::simple::BasicPixelID<short> >::Result;
  EXPECT_EQ( shortImage->GetPixelIDValue(), result);
  EXPECT_EQ( shortImage->GetPixelID(), result);

  result = typelist::IndexOf< InstantiatedPixelIDTypeList, rtk::simple::BasicPixelID<float> >::Result;
  EXPECT_EQ( floatImage->GetPixelIDValue(), result );
  EXPECT_EQ( floatImage->GetPixelID(), result );

  result = typelist::IndexOf< InstantiatedPixelIDTypeList, rtk::simple::VectorPixelID<float> >::Result;
  ...
  result = rtk::simple::PixelIDToPixelIDValue< rtk::simple::BasicPixelID<short> >::Result;
  EXPECT_EQ( shortImage->GetPixelIDValue(), result);
  EXPECT_EQ( shortImage->GetPixelID(), result);

  result = rtk::simple::PixelIDToPixelIDValue< rtk::simple::BasicPixelID<float> >::Result;
  EXPECT_EQ( floatImage->GetPixelIDValue(), result );
  EXPECT_EQ( floatImage->GetPixelID(), result );

  result = rtk::simple::PixelIDToPixelIDValue< rtk::simple::VectorPixelID<float> >::Result;
  EXPECT_EQ( floatVectorImage->GetPixelIDValue(), result );
  EXPECT_EQ( floatVectorImage->GetPixelID(), result );
  ...
  result = typelist::IndexOf< InstantiatedPixelIDTypeList,  rtk::simple::ImageTypeToPixelID<ShortImageType>::PixelIDType >::Result;
  EXPECT_EQ( shortImage->GetPixelIDValue(), result );
  EXPECT_EQ( shortImage->GetPixelID(), result );

  result = typelist::IndexOf< InstantiatedPixelIDTypeList,  rtk::simple::ImageTypeToPixelID<FloatVectorImageType>::PixelIDType >::Result;
  EXPECT_EQ( floatVectorImage->GetPixelIDValue(), result );
  EXPECT_EQ( floatVectorImage->GetPixelID(), result );


  result = rtk::simple::ImageTypeToPixelIDValue<ShortImageType>::Result;
  EXPECT_EQ( shortImage->GetPixelIDValue(), result );
  EXPECT_EQ( shortImage->GetPixelID(), result );

  result = rtk::simple::ImageTypeToPixelIDValue<FloatVectorImageType>::Result;
  EXPECT_EQ( floatVectorImage->GetPixelIDValue(), result );
  EXPECT_EQ( floatVectorImage->GetPixelID(), result );
...
TEST_F(Image,Constructors) {
  ...
  {
  rtk::simple::Image image;
  EXPECT_EQ ( 0u, image.GetWidth() );
  EXPECT_EQ ( 0u, image.GetHeight() );
  EXPECT_EQ ( 0u, image.GetDepth() );
  }

  rtk::simple::Image image ( 64, 65, 66, rtk::simple::srtkUInt8 );
  EXPECT_EQ ( "08183e1b0c50fd2cf6f070b58e218443fb7d5317", hasher.SetHashFunction ( rtk::simple::HashImageFilter::SHA1 ).Execute ( image ) ) << " SHA1 hash value srtkUInt8";
  result = typelist::IndexOf< InstantiatedPixelIDTypeList, rtk::simple::BasicPixelID<unsigned char> >::Result;
  ...
  image = rtk::simple::Image ( 64, 65, 66, rtk::simple::srtkInt16 );
  EXPECT_EQ ( "645b71695b94923c868e16b943d8acf8f6788617", hasher.SetHashFunction ( rtk::simple::HashImageFilter::SHA1 ).Execute ( image ) ) << " SHA1 hash value srtkUInt16";
  result = typelist::IndexOf< InstantiatedPixelIDTypeList, rtk::simple::BasicPixelID<short> >::Result;
  EXPECT_EQ ( image.GetPixelIDValue(), result );
  ...
  image = rtk::simple::Image ( 64, 65, rtk::simple::srtkUInt16 );
  EXPECT_EQ ( "e3c464cc1b73df3f48bacf238a80f88b5ab0d3e6", hasher.SetHashFunction ( rtk::simple::HashImageFilter::SHA1 ).Execute ( image ) ) << " SHA1 hash value srtkUInt16";
  result = typelist::IndexOf< InstantiatedPixelIDTypeList, rtk::simple::BasicPixelID<unsigned short> >::Result;
  EXPECT_EQ ( image.GetPixelIDValue(), result );
  ...
  image = rtk::simple::Image( s2d, rtk::simple::srtkVectorUInt8 );

  EXPECT_EQ ( image.GetDimension(), 2u );
  EXPECT_EQ ( 2u, image.GetNumberOfComponentsPerPixel() );
  image = rtk::simple::Image( s3d, rtk::simple::srtkVectorFloat32 );
  EXPECT_EQ ( image.GetDimension(), 3u );
  EXPECT_EQ ( 3u, image.GetNumberOfComponentsPerPixel() );

  image = rtk::simple::Image( s2d, rtk::simple::srtkVectorInt16, 5 );
  EXPECT_EQ ( image.GetDimension(), 2u );
  EXPECT_EQ ( 5u, image.GetNumberOfComponentsPerPixel() );
  image = rtk::simple::Image( s3d, rtk::simple::srtkVectorFloat64, 10 );
  EXPECT_EQ ( image.GetDimension(), 3u );
  ...
  ASSERT_ANY_THROW( rtk::simple::Image( s1d, rtk::simple::srtkVectorFloat64 ) );
  ASSERT_ANY_THROW( rtk::simple::Image( s4d, rtk::simple::srtkVectorFloat64 ) );

  // check for error with bad pixelID
  ASSERT_ANY_THROW( rtk::simple::Image( s2d, rtk::simple::srtkUnknown ) );
  ASSERT_ANY_THROW( rtk::simple::Image( s2d, rtk::simple::PixelIDValueEnum(-100) ) );
  ...
  ASSERT_ANY_THROW( rtk::simple::Image( s2d, rtk::simple::srtkInt16, 10 ) );
  ASSERT_ANY_THROW( rtk::simple::Image( s2d, rtk::simple::srtkLabelUInt8, 10 ) );
  ...
  image = rtk::simple::Image ( 64, 65, 66, rtk::simple::srtkLabelUInt8 );
  EXPECT_EQ ( 64u, image.GetWidth() );
  EXPECT_EQ ( 65u, image.GetHeight() );
  EXPECT_EQ ( 66u, image.GetDepth() );
  EXPECT_EQ ( 1u, image.GetNumberOfComponentsPerPixel() );
  ...
  image = rtk::simple::Image ( 64, 65, 66, rtk::simple::srtkLabelUInt16 );

  image = rtk::simple::Image ( 64, 65, 66, rtk::simple::srtkLabelUInt32 );

  image = rtk::simple::Image ( 64, 65, 66, rtk::simple::srtkVectorUInt8 );
  ...
  image = rtk::simple::Image ( 64, 65, 66, rtk::simple::srtkVectorUInt16 );
  EXPECT_EQ ( 3u, image.GetNumberOfComponentsPerPixel() );
}

TEST_F(Image,Hash) {
  rtk::simple::HashImageFilter hasher;
  EXPECT_EQ ( "a998ea8b4999b4db9cbad033a52fe6d654211ff9", hasher.SetHashFunction ( rtk::simple::HashImageFilter::SHA1 ).Execute ( *shortImage ) ) << " SHA1 hash value";
  EXPECT_EQ ( "8cdd56962c5b3aabbfac56cd4dce1c7e", hasher.SetHashFunction ( rtk::simple::HashImageFilter::MD5 ).Execute ( *shortImage ) ) << " MD5 hash value";

  EXPECT_EQ ( "3b6bfcb1922bf8b29b171062ad722c82f8aa3f50", hasher.SetHashFunction ( rtk::simple::HashImageFilter::SHA1 ).Execute ( *floatImage ) ) << " SHA1 hash value";
  EXPECT_EQ ( "e5eba8af943d7911220c9f2fb9b5b9c8", hasher.SetHashFunction ( rtk::simple::HashImageFilter::MD5 ).Execute ( *floatImage ) ) << " MD5 hash value";
}
...
TEST_F(Image, CopyInformation)
{
  ...
  srtk::Image img1( 10, 20, srtk::srtkFloat32 );
  srtk::Image img3d( 10, 10, 10, srtk::srtkUInt32 );


  // number if dimension are different
  EXPECT_ANY_THROW( img1.CopyInformation( img3d ) );

  srtk::Image img2( 10, 10, srtk::srtkUInt16 );
  ...
  img2 = srtk::Image( img1.GetSize(), srtk::srtkFloat64);
...
TEST_F(Image, CopyOnWrite)
{
  ...
  srtk::Image img( 10, 10, srtk::srtkInt16 );
  EXPECT_EQ(static_cast<const srtk::Image *>(&img)->GetITKBase()->GetReferenceCount(), 1 )
    << " Reference Count for just constructed Image";

  // use the image from the fixture to test some copy constructor
  EXPECT_EQ(static_cast<const srtk::Image *>(shortImage.get())->GetITKBase()->GetReferenceCount(), 2 )
    << " Reference Count for shared shortImage initial";
  srtk::Image img0 = *shortImage;
  EXPECT_EQ(static_cast<const srtk::Image *>(shortImage.get())->GetITKBase()->GetReferenceCount(), 3 )
    << " Reference Count for shared shortImage copy";
  srtk::Image imgCopy = img0;
  EXPECT_EQ(static_cast<const srtk::Image *>(shortImage.get())->GetITKBase()->GetReferenceCount(), 4 )
  ...
  EXPECT_EQ(static_cast<const srtk::Image *>(&imgCopy)->GetITKBase()->GetReferenceCount(), 1 )
    << " Reference Count for copy after set origin";
  EXPECT_EQ(static_cast<const srtk::Image *>(&img0)->GetITKBase()->GetReferenceCount(), 3 )
    << " Reference Count for shared after set origin";

  // check shallow copy on assignment
  imgCopy = img0;
  EXPECT_EQ(static_cast<const srtk::Image *>(&imgCopy)->GetITKBase()->GetReferenceCount(), 4 )
    << " Reference Count for copy after assigment";
  EXPECT_EQ(static_cast<const srtk::Image *>(&img0)->GetITKBase()->GetReferenceCount(), 4 )
  ...
  EXPECT_EQ( srtk::Hash( imgCopy ), srtk::Hash( img0 ) ) << "Hash for shared and copy after set spacing";
}
...
TEST_F(Image,Operators)
{
  ...
  srtk::Image imgA = *floatImage;
  srtk::Image imgB = *floatImage;
  srtk::Image imgC = *floatImage;
...
TEST_F(Image,SetPixel)
{
  ...
  srtk::Image img = srtk::Image( 10, 10, srtk::srtkFloat64 );
  ...
  img = srtk::Image( 10, 10, srtk::srtkVectorFloat64 );
  ...
  img = srtk::Image( 10, 10, srtk::srtkLabelUInt8 );
  ...
  img = srtk::Image( 10, 10, srtk::srtkFloat64 );

  // Check that out of bounds throw
  uint32_t xOOB[] = {10,0,0};
  uint32_t yOOB[] = {0,10,0};
  uint32_t zOOB[] = {0,0,10};
  ASSERT_THROW( img.SetPixelAsDouble(  std::vector<uint32_t>( xOOB, xOOB+3 ), 0.0 ), rtk::simple::GenericException ) << "x out of bounds";
  ASSERT_THROW( img.SetPixelAsDouble(  std::vector<uint32_t>( yOOB, yOOB+3 ), 0.0 ), rtk::simple::GenericException ) << "y out of bounds";
...
TEST_F(Image,GetPixel)
{
  ...
  srtk::Image img = srtk::Image( 10, 10, srtk::srtkInt8 );
  ...
  img = srtk::Image( 10, 10, srtk::srtkUInt8 );
  ...
  img = srtk::Image( 10, 10, 10, srtk::srtkUInt8 );
  ...
  img = srtk::Image( 10, 10, srtk::srtkInt16 );
  ...
  img = srtk::Image( 10, 10, srtk::srtkUInt16 );
  ...
  img = srtk::Image( 10, 10, srtk::srtkInt32 );
  ...
  img = srtk::Image( 10, 10, srtk::srtkUInt32 );
  ...
  if ( srtk::srtkInt64 != srtk::srtkUnknown )
    {
    img = srtk::Image( 10, 10, srtk::srtkInt64 );
 ...
 if ( srtk::srtkUInt64 != srtk::srtkUnknown )
  ...
  img = srtk::Image( 10, 10, srtk::srtkFloat32 );
  ...
  img = srtk::Image( 10, 10, srtk::srtkComplexFloat32 );
  ...
  img = srtk::Image( 10, 10, srtk::srtkComplexFloat64 );
  ...
  uint32_t yOOB[] = {0,10,0};
  uint32_t zOOB[] = {0,0,10};
  ASSERT_THROW( img.GetPixelAsDouble(  std::vector<uint32_t>( xOOB, xOOB+3 ) ), rtk::simple::GenericException ) << "x out of bounds";
  ASSERT_THROW( img.GetPixelAsDouble(  std::vector<uint32_t>( yOOB, yOOB+3 ) ), rtk::simple::GenericException ) << "y out of bounds";
...
TEST_F(Image,GetPixelVector)
{
  ...
  srtk::Image img = srtk::Image( 10, 10, srtk::srtkVectorInt8 );
  ...
  img = srtk::Image( 10, 10, srtk::srtkVectorUInt8 );
  ...
  img = srtk::Image( 10, 10, 10, srtk::srtkVectorUInt8 );
  ...
  img = srtk::Image( 10, 10, srtk::srtkVectorInt16 );
  ...
  img = srtk::Image( 10, 10, srtk::srtkVectorUInt16 );
  ...
  img = srtk::Image( 10, 10, srtk::srtkVectorInt32 );
  ...
  img = srtk::Image( 10, 10, srtk::srtkVectorUInt32 );
  ...
  if ( srtk::srtkVectorInt64 != srtk::srtkUnknown )
    {
    img = srtk::Image( 10, 10, srtk::srtkVectorInt64 );
  ...
  img = srtk::Image( 10, 10, srtk::srtkVectorFloat32 );
  ...
  uint32_t yOOB[] = {0,10,0};
  uint32_t zOOB[] = {0,0,10};
  ASSERT_THROW( img.GetPixelAsVectorFloat64(  std::vector<uint32_t>( xOOB, xOOB+3 ) ), rtk::simple::GenericException ) << "x out of bounds";
  ASSERT_THROW( img.GetPixelAsVectorFloat64(  std::vector<uint32_t>( yOOB, yOOB+3 ) ), rtk::simple::GenericException ) << "y out of bounds";
...
TEST_F(Image, GetBuffer)
{
  ...
  srtk::Image img = srtk::Image( 10, 10, srtk::srtkUInt8 );
  ...
  img = srtk::Image( 10, 10, srtk::srtkInt8 );
...
TEST_F(Image, GetBufferVector)
{
  ...
  srtk::Image img = srtk::Image( 10, 10, srtk::srtkVectorUInt8 );
  ...
  img = srtk::Image( 10, 10, srtk::srtkVectorInt8 );
  EXPECT_EQ( img.GetBufferAsInt8()[99], 0 ) << " Get last element in buffer ";
  ASSERT_ANY_THROW( img.GetBufferAsUInt8() ) << " Get with wrong type";
  ASSERT_ANY_THROW( img.GetBufferAsInt16() ) << " Get with wrong type";
  ASSERT_ANY_THROW( img.GetBufferAsUInt16() ) << " Get with wrong type";
...
{
  srtk::Image img = srtk::Image( 10,10, 10, srtk::srtkFloat32 );
...
TEST_F(Image,Mandelbrot)
{
  ...
  srtk::Image real = srtk::Image( xs, ys, srtk::srtkFloat32 );
  srtk::Image imagine = srtk::Image( xs, ys, srtk::srtkFloat32 );
  ...
  srtk::Image C = srtk::RealAndImaginaryToComplex( real, imagine );
  std::cout << "Generated C" << std::endl;

  // initial image filled with 0s
  srtk::Image img( xs, ys, srtk::srtkComplexFloat32 );
  ...
  srtk::Image R = srtk::ComplexToReal( img );
  srtk::Image I = srtk::ComplexToImaginary( img );
  img = R*R + I*I;

  srtk::WriteImage( img, "mandelbrot.nrrd" );
}
