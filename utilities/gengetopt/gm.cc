    ...
    foropt
    {
      if (opt->mode_value)
      {
        modeOptionMap[opt->mode_value].push_back(
          std::make_pair("args_info->" + string(opt->var_arg) + "_given", string("\"--") + opt->long_opt + "\""));
      }
    }
  }
...
static void
_generate_option_arg(ostream & stream, unsigned int indent, struct gengetopt_option * opt);
  ...
  , tab_indentation(0)
{
  ...
  const string my_ifndefname = to_upper(strip_path(stripped_header_file_name));
  set_ifndefname(canonize_names(my_ifndefname.c_str()));
  header_gen_class::set_parser_name(parser_function_name);
  const string my_package_var_name = to_upper(parser_function_name) + "_PACKAGE";
  const string my_version_var_name = to_upper(parser_function_name) + "_VERSION";
  ...
  if (gengetopt_package)
    set_package_var_val(uppersand + gengetopt_package + uppersand);
  else
    set_package_var_val("PACKAGE");

  if (gengetopt_version)
    set_version_var_val(uppersand + gengetopt_version + uppersand);
  else
  ...
  if (gen_gengetopt_version)
    c_source_gen_class::set_generator_version("version " VERSION);
  ...
  if (!gen_strdup)
  {
    foropt if (opt->type != ARG_FLAG || opt->type != ARG_NO)
    {
      gen_strdup = true;
      break;
    }
...
void
CmdlineParserCreator::set_has_arg_types()
{
  struct gengetopt_option * opt;
...
CmdlineParserCreator::generate_header_file()
{
  ...
  ofstream * output_file = open_fstream(header_file.c_str());
 ...
 */
static const string
from_value_to_enum(const string & name, const string & val)
{
  return name + "_arg_" + canonize_enum(val);
}

void
CmdlineParserCreator::generate_enum_types(ostream & stream, unsigned int indent)
{
  struct gengetopt_option * opt;
  FIX_UNUSED(indent);
  ...
  foropt
  {
    ...
    if (opt->type == ARG_ENUM)
    {
      ...
      for (AcceptedValues::const_iterator it = opt->acceptedvalues->begin(); it != opt->acceptedvalues->end(); ++it)
      {
        enum_values << ", ";
...
void
CmdlineParserCreator::generate_option_arg(ostream & stream, unsigned int indent)
{
  struct gengetopt_option * opt;

  foropt { _generate_option_arg(stream, indent, opt); }
}
...
void
CmdlineParserCreator::generate_option_given(ostream & stream, unsigned int indent)
{
  struct gengetopt_option * opt;
  string                    indent_str(indent, ' ');
  bool                      first = true;
...
void
CmdlineParserCreator::generate_option_values_decl(ostream & stream, unsigned int indent)
{
  struct gengetopt_option * opt;
  bool                      first = true;
  FIX_UNUSED(indent);
  ...
  foropt
  {
    if (opt->acceptedvalues)
    {
      ...
      stream << "extern const char *" << OPTION_VALUES_NAME(opt->var_arg) << "[];  /**< @brief Possible values for "
             << opt->long_opt << ". */\n";
    }
  }
...
void
CmdlineParserCreator::generate_option_values(ostream & stream, unsigned int indent)
{
  struct gengetopt_option * opt;
  bool                      first = true;
  FIX_UNUSED(indent);
  ...
  foropt
  {
    if (opt->acceptedvalues)
    {
      ...
      stream << "const char *" << OPTION_VALUES_NAME(opt->var_arg) << "[] = {" << opt->acceptedvalues->toString(false)
...
static void
generate_option_usage_string(gengetopt_option * opt, ostream & usage)
{
  const char * type_str;
...
CmdlineParserCreator::generate_usage_string(bool use_config_package)
{
  ...
  if (long_help)
  {
    ...
    foropt if (opt->required && !opt->hidden && !opt->mode_value) { generate_option_usage_string(opt, usage); }
...
CmdlineParserCreator::generate_help_option_print_from_lists(ostream &           stream,
                                                            unsigned int        indent,
                                                            OptionHelpList *    full_option_list,
...
void
CmdlineParserCreator::generate_help_option_print(ostream & stream, unsigned int indent)
{
  ...
  if (!c_source_gen_class::has_hidden && !c_source_gen_class::has_details)
  {
    ...
    for (OptionHelpList::const_iterator it = option_list->begin(); it != option_list->end(); ++it)
    {
  ...
  else
  {
    ...
    generate_help_option_print_from_lists(stream,
                                          indent,
                                          full_option_list,
                                          option_list,
...
void
CmdlineParserCreator::generate_full_help_option_print(ostream & stream, unsigned int indent)
{
...
void
CmdlineParserCreator::generate_detailed_help_option_print(ostream & stream, unsigned int indent)
{
...
void
CmdlineParserCreator::generate_custom_getopt(ostream & stream, unsigned int indent)
{
...
CmdlineParserCreator::generate_help_option_list(bool generate_hidden, bool generate_details)
{
  ...
  foropt
  {
    ...
    // hidden options
    if (opt->section && (!curr_section || strcmp(curr_section, opt->section)) && (!opt->hidden || generate_hidden))
    {
    ...
    if (opt->group_value && (!prev_group || strcmp(opt->group_value, prev_group) != 0))
    {
      string group_string = "\\n Group: ";
      string wrapped_desc;
    ...
    if (opt->mode_value && (!prev_mode || strcmp(opt->mode_value, prev_mode) != 0))
    {
      string mode_string = "\\n Mode: ";
      string wrapped_desc;
    ...
    if (!opt->hidden || generate_hidden)
    {
      ...
      else
      {
        ...
        option_stream << "--" << opt->long_opt << (arg_optional ? "[" : "") << "=" << type_str
                      << (arg_optional ? "]" : "");
      ...
      if ((option_string.size() >= MAX_STARTING_COLUMN) || (desc_col <= option_string.size()))
      {
      ...
      generate_help_desc_print(stream,
                               desc_col,
                               opt_desc,
                               def_val,
...
void
generate_counter_init(const Collection & collection, const string & name, ostream & stream, unsigned int indent)
{
  string                              indent_str(indent, ' ');
  ...
  {
    stream << indent_str;
    stream << ARGS_STRUCT << "->" << canonize_name(idx->first) << "_" << name << "_counter = 0 ;";
    stream << endl;
  }
}

void
CmdlineParserCreator::generate_given_init(ostream & stream, unsigned int indent)
{
  struct gengetopt_option * opt;
  string                    indent_str(indent, ' ');
...
CmdlineParserCreator::generate_reset_groups(ostream & stream, unsigned int indent)
{
  ...
  for (groups_collection_t::const_iterator idx = gengetopt_groups.begin(); idx != end; ++idx)
  {
    body.str("");
...
void
CmdlineParserCreator::free_option(struct gengetopt_option * opt, ostream & stream, unsigned int indent)
{
  if (opt->type == ARG_NO)
    return;
  ...
  if (opt->type != ARG_FLAG)
  {
    if (opt->multiple)
    {
      ...
      free_multiple.generate_free_multiple(stream, indent);
    }
    else
    {
...
CmdlineParserCreator::generate_check_modes(ostream & stream, unsigned int indent)
{
  ...
  for (ModeOptionMap::const_iterator map_it = modeOptionMap.begin(); map_it != modeOptionMap.end(); ++map_it)
  {
    ...
    for (map_it2 = map_it; map_it2 != modeOptionMap.end(); ++map_it2)
    {
      ...
      std::for_each(
        map_it1->second.begin(), map_it1->second.end(), pair_print_f<OptionValueElem>(mode1_given, mode1_options));
      std::for_each(
        map_it2->second.begin(), map_it2->second.end(), pair_print_f<OptionValueElem>(mode2_given, mode2_options));
...
CmdlineParserCreator::generate_clear_arg(ostream & stream, unsigned int indent)
{
  ...
  foropt
  {
    ...
    else if (opt->type == ARG_STRING)
    {
      clear_arg.set_has_arg(true);
      if (opt->default_given)
        clear_arg.set_value("gengetopt_strdup (\"" + string(opt->default_string) + "\")");
    }
...
void
CmdlineParserCreator::generate_long_option_struct(ostream & stream, unsigned int indent)
{
  string                    indent_str(indent, ' ');
  struct gengetopt_option * opt;
  ...
  foropt
  {
    ...
    stream << "{ \"" << opt->long_opt << "\",\t"
           << (opt->type == ARG_NO || opt->type == ARG_FLAG ? 0 : (opt->arg_is_optional ? 2 : 1)) << ", NULL, ";
...
CmdlineParserCreator::generate_getopt_string()
{
  ...
  foropt if (opt->short_opt)
  {
    built_getopt_string << opt->short_opt << (opt->type == ARG_NO || opt->type == ARG_FLAG ? "" : ":");
    built_getopt_string << (opt->arg_is_optional ? ":" : "");
  }
...
void
CmdlineParserCreator::generate_handle_no_short_option(ostream & stream, unsigned int indent)
{
  handle_options(stream, indent, false);
}

void
CmdlineParserCreator::generate_handle_option(ostream & stream, unsigned int indent)
{
...
CmdlineParserCreator::handle_options(ostream & stream, unsigned int indent, bool has_short)
{
  ...
  foropt
  {
    ...
    if ((has_short && opt->short_opt) || (!has_short && !opt->short_opt))
    {
      ...
      if (!no_help && ((opt->short_opt == HELP_SHORT_OPT && strcmp(opt->long_opt, HELP_LONG_OPT) == 0) ||
                       strcmp(opt->long_opt, HELP_LONG_OPT) == 0 || strcmp(opt->long_opt, FULL_HELP_LONG_OPT) == 0 ||
                       strcmp(opt->long_opt, DETAILED_HELP_LONG_OPT) == 0))
      {
        ...
        if (no_handle_help)
        {
          ...
          // and to return 0
          const string final_instructions = parser_function_name + string("_free (&local_args_info);\nreturn 0;");
...
void
CmdlineParserCreator::generate_handle_group(ostream & stream, unsigned int indent)
{
...
void
CmdlineParserCreator::generate_handle_required(ostream & stream, unsigned int indent)
{
  struct gengetopt_option * opt;
  ...
  foropt if (opt->required || opt->multiple)
  {
    if (opt->mode_value)
    {
      opt_gen.set_mode_condition("args_info->" + canonize_name(opt->mode_value) + "_mode_counter && ");
    }
    else
    {
...
void
CmdlineParserCreator::generate_handle_dependencies(ostream & stream, unsigned int indent)
{
  struct gengetopt_option *  opt;
...
template <typename Collection>
void
generate_counters(const Collection & collection, const string & name, ostream & stream, unsigned int indent)
{
...
void
CmdlineParserCreator::generate_group_counters(ostream & stream, unsigned int indent)
{
  generate_counters(gengetopt_groups, "group", stream, indent);
}

void
CmdlineParserCreator::generate_mode_counters(ostream & stream, unsigned int indent)
{
...
void
CmdlineParserCreator::generate_free(ostream & stream, unsigned int indent)
{
  struct gengetopt_option * opt;

  foropt { free_option(opt, stream, indent); }
}

void
CmdlineParserCreator::generate_list_free(ostream & stream, unsigned int indent)
{
  struct gengetopt_option * opt;
...
CmdlineParserCreator::generate_file_save_loop(ostream & stream, unsigned int indent)
{
  ...
  foropt
  {
    if (opt->multiple)
    {
      ...
      file_save_multiple.set_values((opt->acceptedvalues ? OPTION_VALUES_NAME(opt->var_arg) : "0"));
    ...
    else
    {
      ...
      file_save.set_values((opt->acceptedvalues ? OPTION_VALUES_NAME(opt->var_arg) : "0"));
