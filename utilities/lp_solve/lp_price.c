...
int CMP_CALLMODEL
compareImprovementVar(const pricerec * current, const pricerec * candidate)
{
  int     result = COMP_PREFERNONE;
  lprec * lp = current->lp;
  REAL    testvalue, margin = PREC_IMPROVEGAP;
  int     currentcolno, currentvarno = current->varno, candidatecolno, candidatevarno = candidate->varno;
  ...
  if (lp->_piv_rule_ != PRICER_FIRSTINDEX)
  {
    ...
    if (result == COMP_PREFERNONE)
    {
      if (candidatecolno > 0)
        testvalue =
          mat_collength(lp->matA, candidatecolno) + (is_obj_in_basis(lp) && (lp->obj[candidatecolno] != 0) ? 1 : 0);
      else
        testvalue = 1;
      if (currentcolno > 0)
        testvalue -=
          mat_collength(lp->matA, currentcolno) + (is_obj_in_basis(lp) && (lp->obj[currentcolno] != 0) ? 1 : 0);
      else
        testvalue -= 1;
      if (testvalue > 0)
        result = COMP_PREFERINCUMBENT;
...
int CMP_CALLMODEL
compareSubstitutionVar(const pricerec * current, const pricerec * candidate)
{
  int     result = COMP_PREFERNONE;
  lprec * lp = current->lp;
  REAL    testvalue = candidate->theta, margin = current->theta;
  ...
  if (result == COMP_PREFERNONE)
  {
    REAL currentpivot = fabs(current->pivot), candidatepivot = fabs(candidate->pivot);
...
int CMP_CALLMODEL
compareBoundFlipVar(const pricerec * current, const pricerec * candidate)
{
  REAL    testvalue, margin;
  int     result = COMP_PREFERNONE;
  lprec * lp = current->lp;
  MYBOOL  candbetter;
  int     currentvarno = current->varno, candidatevarno = candidate->varno;
  ...
  if (result == COMP_PREFERNONE)
  {
    ...
    if (result == COMP_PREFERNONE)
      result = compareREAL(&(lp->upbo[currentvarno]), &(lp->upbo[candidatevarno]));
  }
...
STATIC MYBOOL
validImprovementVar(pricerec * candidate)
{
...
STATIC MYBOOL
validSubstitutionVar(pricerec * candidate)
{
  ...
  if (candidate->varno <= 0)
    ...
    return ((MYBOOL)((theta < lp->infinite) && (fabs(candidate->pivot) >= candidate->epspivot)));
}

int CMP_CALLMODEL
compareImprovementQS(const UNIONTYPE QSORTrec * current, const UNIONTYPE QSORTrec * candidate)
{
  return (compareImprovementVar((pricerec *)current->pvoidint2.ptr, (pricerec *)candidate->pvoidint2.ptr));
}
int CMP_CALLMODEL
compareSubstitutionQS(const UNIONTYPE QSORTrec * current, const UNIONTYPE QSORTrec * candidate)
{
...
STATIC int
addCandidateVar(pricerec * candidate, multirec * multi, findCompare_func findCompare, MYBOOL allowSortedExpand)
{
  int        insertpos, delta = 1;
  pricerec * targetrec;

  /* Find the insertion point (if any) */
  if ((multi->freeList[0] == 0) || (multi->sorted && allowSortedExpand) ||
      (candidate->isdual && (multi->used == 1) &&
        ...
        multi_truncatingvar(multi, ((pricerec *)(multi->sortedList[0].pvoidreal.ptr))->varno))))
  {
    ...
    if (((insertpos >= multi->size) && (multi->freeList[0] == 0)) ||
        ((insertpos == multi->used) && (!allowSortedExpand || (multi->step_last >= multi->epszero))))
      return (-1);
...
STATIC MYBOOL
findImprovementVar(pricerec * current, pricerec * candidate, MYBOOL collectMP, int * candidatecount)
...
STATIC MYBOOL
collectMinorVar(pricerec * candidate, multirec * longsteps, MYBOOL isphase2, MYBOOL isbatch)
{
  int inspos;
  ...
  if (!isbatch && !longsteps->sorted && (longsteps->used > 1) &&
      ((longsteps->freeList[0] == 0) || multi_truncatingvar(longsteps, candidate->varno) ||
       (longsteps->step_last >= longsteps->epszero)))
  {
    longsteps->sorted =
      QS_execute(longsteps->sortedList, longsteps->used, (findCompare_func *)compareSubstitutionQS, &inspos);
  ...
  return ((MYBOOL)(inspos >= 0) && ((isbatch == TRUE) || multi_recompute(longsteps, inspos, isphase2, TRUE)));
}

STATIC MYBOOL
findSubstitutionVar(pricerec * current, pricerec * candidate, int * candidatecount)
...
STATIC partialrec *
partial_createBlocks(lprec * lp, MYBOOL isrow)
{
...
STATIC int
partial_countBlocks(lprec * lp, MYBOOL isrow)
{
...
STATIC int
partial_activeBlocks(lprec * lp, MYBOOL isrow)
{
  partialrec * blockdata = IF(isrow, lp->rowblocks, lp->colblocks);
...
STATIC void
partial_freeBlocks(partialrec ** blockdata)
{
...
STATIC void
makePriceLoop(lprec * lp, int * start, int * end, int * delta)
{
  int offset = is_piv_mode(lp, PRICE_LOOPLEFT);

  if ((offset) || (((lp->total_iter + offset) % 2 == 0) && is_piv_mode(lp, PRICE_LOOPALTERNATE)))
  {
...
STATIC MYBOOL
serious_facterror(lprec * lp, REAL * bvector, int maxcols, REAL tolerance)
{
  int      i, j, ib, ie, nz, nc;
...
STATIC void
update_reducedcosts(lprec * lp, MYBOOL isdual, int leave_nr, int enter_nr, REAL * prow, REAL * drow)
{
                     ...
                     int     roundmode)
{
  ...
  if (isdual)
  {
    bsolve_xA2(lp,
               ...
               prow,
               epsvalue,
               nzprow, /* Calculate net sensitivity given a leaving variable */
               0,
               drow,
               epsvalue,
               nzdrow, /* Calculate the net objective function values */
               roundmode);
  }
  else
  {
...
STATIC MYBOOL
verify_stability(lprec * lp, MYBOOL isprimal, REAL xfeas, REAL sfeas, int nfeas)
{
  MYBOOL testOK = TRUE;
  return (testOK);
...
STATIC int
find_rowReplacement(lprec * lp, int rownr, REAL * prow, int * nzprow)
   ...
   Basically, the function is a specialized coldual(). */
{
  ...
  compute_reducedcosts(lp, TRUE, rownr, NULL, TRUE, prow, nzprow, NULL, NULL, MAT_ROUNDDEFAULT);
  ...
  {
    if (!lp->is_basic[i] && !is_fixedvar(lp, i) && (fabs(prow[i]) > bestvalue))
    {
      bestindex = i;
...
#endif
      compute_reducedcosts(lp,
                           FALSE,
                           0,
                           coltarget,
                           (MYBOOL)((nloop <= 1) || (partialloop > 1)),
                           NULL,
                           NULL,
                           drow,
                           nzdrow,
  ...
  if (lp->multivars != NULL)
  {
    if (collectMP)
    {
      if (!lp->multivars->sorted)
        lp->multivars->sorted =
          QS_execute(lp->multivars->sortedList, lp->multivars->used, (findCompare_func *)compareImprovementQS, NULL);
...
STATIC int
rowprim(lprec * lp, int colnr, LREAL * theta, REAL * pcol, int * nzpcol, MYBOOL forceoutEQ, REAL * xviol)
{
...
Retry:
  ...
  for (; Hpass <= 2; Hpass++)
  {
    ...
    for (; ii * iz <= iy; ii += iz)
    {
      ...
      /*i =*/compute_theta(
        lp, i, &candidate.theta, isupper, my_if(lp->upbo[lp->var_basic[i]] < lp->epsprimal, Heps / 10, Heps), TRUE);
      ...
      else if (findSubstitutionVar(&current, &candidate, NULL))
        break;
  ...
  else if (current.theta >= lp->infinite)
  {
    report(lp,
           IMPORTANT,
           "rowprim: Numeric instability pcol[%d] = %g, rhs[%d] = %g, upbo = %g\n",
           current.varno,
           savef,
           current.varno,
           lp->rhs[current.varno],
...
STATIC int
rowdual(lprec * lp, REAL * rhvec, MYBOOL forceoutEQ, MYBOOL updateinfeas, REAL * xviol)
{
  int      k, i, iy, iz, ii, ninfeas;
  REAL     rh;
  REAL     up, lo = 0, epsvalue, sinfeas, xinfeas;
  pricerec current, candidate;
  MYBOOL   collectMP = FALSE;
...
STATIC void
longdual_testset(lprec * lp, int which, int rownr, REAL * prow, int * nzprow, REAL * drow, int * nzdrow)
{
  (void)nzdrow;
  int  i, j;
  REAL F = lp->infinite;
  ...
  { /* Maros Example-1 - presorted in correct order */
    j = 1;
    i = lp->rows + j;
    lp->upbo[i] = 0;
    lp->is_lower[i] = TRUE;
        ...
        REAL *  xviol)
{
  ...
  *candidatecount = 0;
  ...
  if (!skipupdate)
  {
    ...
    /* Recompute from scratch only at the beginning, otherwise update */
    if ((lp->current_iter > 0) && (refactRecent(lp) < AUTOMATIC))
      compute_reducedcosts(lp, TRUE, row_nr, NULL, TRUE, prow, nzprow, NULL, NULL, MAT_ROUNDDEFAULT);
    else
#endif
      compute_reducedcosts(lp, TRUE, row_nr, NULL, TRUE, prow, nzprow, drow, nzdrow, MAT_ROUNDDEFAULT);
  }
  ...
  { /* Check if the leaving variable is >= its upper bound */
    ...
    if (g == 1)
    {
      if (viol >= lp->infinite)
      {
        report(lp,
               IMPORTANT,
               "coldual: Large basic solution value %g at iter %.0f indicates numerical instability\n",
               lp->rhs[row_nr],
      ...
      if (skipupdate)
        ...
        report(lp,
               SEVERE,
               "coldual: Leaving variable %d does not violate bounds at iter %.0f\n",
               row_nr,
               (double)get_total_iter(lp));
      return (-1);
    }
  ...
  for (; ix * iz <= iy; ix += iz)
  {
    ...
    if (dolongsteps)
    {
      if (isbatch && (ix == iy))
        isbatch = AUTOMATIC;
      if (collectMinorVar(&candidate, lp->longsteps, (MYBOOL)(dolongsteps == AUTOMATIC), isbatch) && lp->spx_trace)
  ...
  if (lp->spx_trace)
    report(lp,
           NORMAL,
           "coldual: Entering column %d, reduced cost %g, pivot value %g, bound swaps %d\n",
           i,
           drow[i],
           prow[i],
           multi_used(lp->longsteps));
...
INLINE REAL
normalizeEdge(lprec * lp, int item, REAL edge, MYBOOL isdual)
{
#if 1
...
STATIC int
partial_findBlocks(lprec * lp, MYBOOL autodefine, MYBOOL isrow)
{
...
STATIC int
partial_blockStart(lprec * lp, MYBOOL isrow)
{
  partialrec * blockdata;
...
STATIC int
partial_blockEnd(lprec * lp, MYBOOL isrow)
{
  partialrec * blockdata;
...
STATIC int
partial_blockNextPos(lprec * lp, int block, MYBOOL isrow)
{
  partialrec * blockdata;
...
STATIC MYBOOL
partial_blockStep(lprec * lp, MYBOOL isrow)
{
  partialrec * blockdata;
...
STATIC MYBOOL
partial_isVarActive(lprec * lp, int varno, MYBOOL isrow)
{
  partialrec * blockdata;
...
STATIC multirec *
multi_create(lprec * lp, MYBOOL truncinf)
{
  multirec * multi;
...
STATIC MYBOOL
multi_mustupdate(multirec * multi)
{
  return ((MYBOOL)((multi != NULL) && (multi->used < multi->limit)));
}
STATIC MYBOOL
multi_resize(multirec * multi, int blocksize, int blockdiv, MYBOOL doVlist, MYBOOL doIset)
{
  MYBOOL ok = TRUE;
  ...
  if ((blocksize > 1) && (blockdiv > 0))
  {
    ...
    multi->sortedList =
      (UNIONTYPE QSORTrec *)realloc(multi->sortedList, (multi->size + 1) * sizeof(*(multi->sortedList)));
...
STATIC void
multi_valueInit(multirec * multi, REAL step_base, REAL obj_base)
{
  ...
  if (step_base > 0)
    report(multi->lp,
           SEVERE,
           "multi_valueInit: Positive constraint violation %g provided at iteration %6.0f\n",
           step_base,
           (double)get_total_iter(multi->lp));
#endif
}

STATIC REAL *
multi_valueList(multirec * multi)
{
  return (multi->valueList);
}

STATIC int *
multi_indexSet(multirec * multi, MYBOOL regenerate)
{
  if (regenerate)
...
STATIC int
multi_getvar(multirec * multi, int item)
{
#ifdef Paranoia
...
STATIC MYBOOL
multi_recompute(multirec * multi, int index, MYBOOL isphase2, MYBOOL fullupdate)
{
  int        i, n;
  ...
  while ((index <= n) && (multi->step_last < multi->epszero))
  {
    ...
    if (lp->spx_trace && (multi->step_last > lp->infinite))
      report(lp,
             SEVERE,
             "multi_recompute: A very large step-size %g was generated at iteration %6.0f\n",
             multi->step_last,
             (double)get_total_iter(lp));
#endif
    index++;
...
STATIC MYBOOL
multi_truncatingvar(multirec * multi, int varnr)
{
  return (multi->truncinf && is_infinite(multi->lp, multi->lp->upbo[varnr]));
}

STATIC MYBOOL
multi_removevar(multirec * multi, int varnr)
{
  int   i = 1;
...
STATIC int
multi_enteringvar(multirec * multi, pricerec * current, int priority)
{
  lprec *   lp = multi->lp;
  ...
  if (multi->objcheck && (lp->solutioncount > 0) && bb_better(lp, OF_WORKING | OF_PROJECTED, OF_TEST_WE))
  {
...
Redo:
  ...
  for (i = multi->used - 1; i >= 0; i--)
  {
    ...
    score =
      pow(1.0 + score, b1) * pow(1.0 + log(bound / multi->maxbound + 1), b2) * pow(1.0 + (REAL)i / multi->used, b3);
    if (score > bestscore)
    {
      bestscore = score;
      bestindex = i;
...
/*    if(lp->upbo[colnr] >= lp->infinite) */
  ...
  if (lp->spx_trace && (fabs(score) > 1 / lp->epsprimal))
    report(
      lp, IMPORTANT, "multi_enteringvar: A very large Theta %g was generated (pivot %g)\n", score, bestcand->pivot);
...
STATIC REAL
multi_enteringtheta(multirec * multi)
{
  return (multi->step_base);
}

STATIC int
multi_populateSet(multirec * multi, int ** list, int excludenr)
{
  int n = 0;
  if (list == NULL)
    list = &(multi->indexSet);
  if ((multi->used > 0) && ((*list != NULL) || allocINT(multi->lp, list, multi->size + 1, FALSE)))
  {
    int i, colnr;
