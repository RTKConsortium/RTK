...
#endif
    /* Do validation */
    if (!result || ((BLAS_dscal == NULL) || (BLAS_dcopy == NULL) || (BLAS_daxpy == NULL) || (BLAS_dswap == NULL) ||
                    (BLAS_ddot == NULL) || (BLAS_idamax == NULL) || (BLAS_dload == NULL) || (BLAS_dnormi == NULL)))
    {
      load_BLAS(NULL);
      result = FALSE;
    }
...
MYBOOL
unload_BLAS(void) { return (load_BLAS(NULL)); }
...
void BLAS_CALLMODEL
my_daxpy(int * _n, REAL * _da, REAL * dx, int * _incx, REAL * dy, int * _incy)
{
...
/* CPU intensive loop; option to do pointer arithmetic */
  ...
  {
    REAL *xptr, *yptr;
    for (i = 1, xptr = dx + ix, yptr = dy + iy; i <= n; i++, xptr += incx, yptr += incy)
      (*yptr) += rda * (*xptr);
    return;
  }
#else
...
/* ************************************************************************ */
void
dcopy(int n, REAL * dx, int incx, REAL * dy, int incy)
{
  dx++;
  dy++;
...
void BLAS_CALLMODEL
my_dcopy(int * _n, REAL * dx, int * _incx, REAL * dy, int * _incy)
{
...
void BLAS_CALLMODEL
my_dscal(int * _n, REAL * _da, REAL * dx, int * _incx)
{
...
REAL BLAS_CALLMODEL
my_ddot(int * _n, REAL * dx, int * _incx, REAL * dy, int * _incy)
{
...
void
dswap(int n, REAL * dx, int incx, REAL * dy, int incy)
{
  dx++;
  dy++;
...
void BLAS_CALLMODEL
my_dswap(int * _n, REAL * dx, int * _incx, REAL * dy, int * _incy)
{
  int i, ix, iy;
#if !defined DOFASTMATH
  int  m, mp1, ns;
...
void
dload(int n, REAL da, REAL * dx, int incx)
{
  dx++;
...
void BLAS_CALLMODEL
my_dload(int * _n, REAL * _da, REAL * dx, int * _incx)
{
...
int BLAS_CALLMODEL
my_idamax(int * _n, REAL * x, int * _is)
{
  REAL xmax, xtest;
  int  i, imax = 0;
#if !defined DOFASTMATH
...
int
submat(int nrowb, int row, int col)
{
  return (nrowb * (col - 1) + subvec(row));
}

int
posmat(int nrowb, int row, int col)
{
...
void
randomdens(int n, REAL * x, REAL r1, REAL r2, REAL densty, int * seeds)
{
  /* ------------------------------------------------------------------
...
void
ddrand(int n, REAL * x, int incx, int * seeds)
{
  ...
  for (ix = 1; ix <= 1 + (n - 1) * incx; ix = ix + incx)
  {
    ...
    if (seeds[1] < 0)
      seeds[1] = seeds[1] + 30269;
    if (seeds[2] < 0)
      seeds[2] = seeds[2] + 30307;
    if (seeds[3] < 0)
      seeds[3] = seeds[3] + 30323;

    x[ix] = ((REAL)seeds[1]) / 30269.0 + ((REAL)seeds[2]) / 30307.0 + ((REAL)seeds[3]) / 30323.0;
    xix = (int)x[ix];
    x[ix] = fabs(x[ix] - xix);
  }
}
