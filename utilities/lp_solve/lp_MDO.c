...
STATIC MYBOOL
includeMDO(MYBOOL * usedpos, int item)
{
...
STATIC int
prepareMDO(lprec * lp, MYBOOL * usedpos, int * colorder, int * data, int * rowmap)
   ...
   row index base is 0 (which suits lp_solve fine). */
{
  ...
  for (j = 1; j <= ncols; j++)
  {
    ...
    else
    {
      ...
      for (; i < ii; i++, value += matValueStep, rownr += matRowColStep)
      {
...
STATIC MYBOOL
verifyMDO(lprec * lp, int * col_end, int * row_nr, int rowmax, int colmax)
{
  int i, j, n, err = 0;
...
void *
mdo_calloc(size_t size, size_t count)
{
