...
REAL
scaled_value(lprec * lp, REAL value, int index)
{
...
REAL
unscaled_value(lprec * lp, REAL value, int index)
{
  if (fabs(value) < lp->infinite)
  {
    if (lp->scaling_used)
...
STATIC REAL
scaled_mat(lprec * lp, REAL value, int rownr, int colnr)
{
  if (lp->scaling_used)
...
STATIC REAL
unscaled_mat(lprec * lp, REAL value, int rownr, int colnr)
{
  if (lp->scaling_used)
    value /= lp->scalars[rownr] * lp->scalars[lp->rows + colnr];
  return (value);
...
REAL
CurtisReidMeasure(lprec * lp, MYBOOL _Advanced, REAL * FRowScale, REAL * FColScale)
{
  int      i, nz;
  ...
  nz = get_nonzeros(lp);
  for (i = 0; i < nz; i++, value += matValueStep, rownr += matRowColStep, colnr += matRowColStep)
  {
    absvalue = fabs(*value);
    if (absvalue > 0)
    {
...
int
CurtisReidScales(lprec * lp, MYBOOL _Advanced, REAL * FRowScale, REAL * FColScale)
{
  int      i, row, col, ent, nz;
  REAL *   RowScalem2, *ColScalem2, *RowSum, *ColSum, *residual_even, *residual_odd;
  REAL     sk, qk, ek, skm1, qkm1, ekm1, qkm2, qkqkm1, ekm2, ekekm1, absvalue, logvalue, StopTolerance;
  ...
  for (col = 1; col <= colMax; col++)
  {
    ...
    ent = mat->col_end[col];
    for (; i < ent; i++, rownr += matRowColStep)
    {
  ...
  while (sk > StopTolerance)
  {
    ...
    ekekm1 = ek * ekm1;
    if ((Result % 2) == 0)
    { /* pass is even; construct RowScale[pass+1] */
      if (Result != 0)
      {
        for (row = 0; row <= lp->rows; row++)
          RowScalem2[row] = FRowScale[row];
        if (qkqkm1 != 0)
        {
          ...
          for (row = 0; row <= lp->rows; row++)
            FRowScale[row] += (residual_odd[row] / (qkqkm1 * (REAL)RowCount[row]) - RowScalem2[row] * ekekm1 / qkqkm1);
        }
      }
    }
    else
    { /* pass is odd; construct ColScale[pass+1] */
      for (col = 1; col <= colMax; col++)
      ...
      if (qkqkm1 != 0)
      {
        ...
        for (col = 1; col <= colMax; col++)
          FColScale[col] += (residual_even[col] / ((REAL)ColCount[col] * qkqkm1) - ColScalem2[col] * ekekm1 / qkqkm1);
      }
    }
    ...
    { /* even */
      ...
      rownr = &(COL_MAT_ROWNR(0));
      colnr = &(COL_MAT_COLNR(0));
      for (i = 0; i < nz; i++, rownr += matRowColStep, colnr += matRowColStep)
      {
  ...
  {
    if ((Result % 2) == 0)
    { /* pass is even, compute RowScale */
      for (row = 0; row <= lp->rows; row++)
        FRowScale[row] *= (1.0 + ekekm1 / qkm1);
      for (row = 0; row <= lp->rows; row++)
        FRowScale[row] += (residual_odd[row] / (qkm1 * (REAL)RowCount[row]) - RowScalem2[row] * ekekm1 / qkm1);
    }
    else
    { /* pass is odd, compute ColScale */
      for (col = 1; col <= colMax; col++)
        FColScale[col] *= (1 + ekekm1 / qkm1);
      for (col = 1; col <= colMax; col++)
        FColScale[col] += (residual_even[col] / ((REAL)ColCount[col] * qkm1) - ColScalem2[col] * ekekm1 / qkm1);
    }
  }
...
STATIC MYBOOL
scaleCR(lprec * lp, REAL * scaledelta)
{
  ...
  if (Result > 0)
  {
    ...
    /* Do the scaling*/
    if (scale_updaterows(lp, scalechange, TRUE) || scale_updatecolumns(lp, &scalechange[lp->rows], TRUE))
...
STATIC MYBOOL
transform_for_scale(lprec * lp, REAL * value)
{
  MYBOOL Accept = TRUE;
  *value = fabs(*value);
#ifdef Paranoia
...
STATIC void
accumulate_for_scale(lprec * lp, REAL * min, REAL * max, REAL value)
{
...
STATIC REAL
minmax_to_scale(lprec * lp, REAL min, REAL max, int itemcount)
{
  REAL scale;
...
STATIC MYBOOL
scale_updatecolumns(lprec * lp, REAL * scalechange, MYBOOL updateonly)
{
  int i, j;
...
STATIC MYBOOL
scale_updaterows(lprec * lp, REAL * scalechange, MYBOOL updateonly)
{
  int i;
...
STATIC MYBOOL
scale_columns(lprec * lp, REAL * scaledelta)
{
  int      i, j, colMax, nz;
  REAL *   scalechange;
  REAL *   value;
  ...
  value = &(COL_MAT_VALUE(0));
  colnr = &(COL_MAT_COLNR(0));
  for (i = 0; i < nz; i++, value += matValueStep, colnr += matRowColStep)
  {
...
STATIC MYBOOL
scale_rows(lprec * lp, REAL * scaledelta)
{
  int      i, j, nz, colMax;
  REAL *   scalechange;
  REAL *   value;
  ...
  value = &(COL_MAT_VALUE(0));
  rownr = &(COL_MAT_ROWNR(0));
  for (i = 0; i < nz; i++, value += matValueStep, rownr += matRowColStep)
  {
...
STATIC REAL
scale(lprec * lp, REAL * scaledelta)
{
  ...
  for (j = 1; j <= lp->columns; j++)
  {
    ...
    rownr = &(COL_MAT_ROWNR(i));
    nz = mat->col_end[j];
    for (; i < nz; i++, value += matValueStep, rownr += matRowColStep)
    {
  ...
  for (j = 1; j <= lp->columns; j++)
  {
    if (is_int(lp, j) && !is_integerscaling(lp))
...
STATIC MYBOOL
finalize_scaling(lprec * lp, REAL * scaledelta)
{
  int i;
...
auto_scale(lprec * lp)
{
  ...
  if (lp->scaling_used && ((((lp->scalemode & SCALE_DYNUPDATE) == 0)) || (lp->bb_level > 0)))
...
unscale_columns(lprec * lp)
{
  ...
  rownr = &(COL_MAT_ROWNR(0));
  colnr = &(COL_MAT_COLNR(0));
  for (j = 0; j < nz; j++, value += matValueStep, rownr += matRowColStep, colnr += matRowColStep)
  {
