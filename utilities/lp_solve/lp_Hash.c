...
STATIC hashtable *
create_hash_table(int size, int base)
{
  int         i;
  int         HashPrimes[] = { 29,      229,     883,     1671,    2791,    4801,    8629,    10007,   15289,
                               25303,   34843,   65269,   99709,   129403,  147673,  166669,  201403,  222163,
                               242729,  261431,  303491,  320237,  402761,  501131,  602309,  701507,  800999,
                               900551,  1000619, 1100837, 1200359, 1300021, 1400017, 1500007, 1750009, 2000003,
                               2500009, 3000017, 4000037, 5000011, 6000011, 7000003, 8000009, 9000011, 9999991 };
  hashtable * ht;
...
STATIC int
hashval(const char * string, int size)
{
...
STATIC hashelem *
findhash(const char * name, hashtable * ht)
{
  hashelem * h_tab_p;
  for (h_tab_p = ht->table[hashval(name, ht->size)]; h_tab_p != NULL; h_tab_p = h_tab_p->next)
...
STATIC hashelem *
puthash(const char * name, int index, hashelem ** list, hashtable * ht)
{
  hashelem * hp = NULL;
  int        hashindex;
...
STATIC void
drophash(const char * name, hashelem ** list, hashtable * ht)
{
  hashelem *hp, *hp1, *hp2;
  int       hashindex;
...
STATIC hashtable *
copy_hash_table(hashtable * ht, hashelem ** list, int newsize)
{
  hashtable * copy;
...
STATIC int
find_row(lprec * lp, const char * name, MYBOOL Unconstrained_rows_found)
{
  hashelem * hp;
...
STATIC int
find_var(lprec * lp, const char * name, MYBOOL verbose)
{
  hashelem * hp;
