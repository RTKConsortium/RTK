...
STATIC int
presolve_setstatusex(presolverec * psdata, int status, int lineno, const char * filename)
{
  if ((status == INFEASIBLE) || (status == UNBOUNDED))
  {
    report(psdata->lp,
           ...
           (status == INFEASIBLE ? "INFEASIBLE" : "UNBOUNDED"),
           lineno,
...
STATIC MYBOOL
presolve_statuscheck(presolverec * psdata, int * status)
{
  if (*status == RUNNING)
  {
    lprec * lp = psdata->lp;
...
STATIC MYBOOL
inc_presolve_space(lprec * lp, int delta, MYBOOL isrows)
{
  int               i, ii, oldrowcolalloc, rowcolsum, oldrowalloc, oldcolalloc;
...
STATIC MYBOOL
presolve_setOrig(lprec * lp, int orig_rows, int orig_cols)
{
  presolveundorec * psundo = lp->presolve_undo;
...
STATIC MYBOOL
presolve_fillUndo(lprec * lp, int orig_rows, int orig_cols, MYBOOL setOrig)
{
  int               i;
  presolveundorec * psundo = lp->presolve_undo;
...
STATIC MYBOOL
presolve_rebuildUndo(lprec * lp, MYBOOL isprimal)
{
...
STATIC void
presolve_storeDualUndo(presolverec * psdata, int rownr, int colnr)
{
  lprec *  lp = psdata->lp;
  ...
  for (ix = presolve_nextrow(psdata, colnr, &item); ix >= 0; ix = presolve_nextrow(psdata, colnr, &item))
  {
    ...
    if (!firstdone)
      firstdone =
        addUndoPresolve(lp, FALSE, rownr, get_mat(lp, 0, colnr) / Aij, get_mat_byindex(lp, ix, FALSE, TRUE) / Aij, iix);
    else
...
/* ----------------------------------------------------------------------------- */
STATIC MYBOOL
presolve_SOScheck(presolverec * psdata)
{
  MYBOOL   status = TRUE;
  lprec *  lp = psdata->lp;
  ...
  for (colnr = 1; colnr <= lp->columns; colnr++)
  {
    ...
    for (; k < nk; k++)
    {
      if (!SOS_is_member(lp->SOS, lp->SOS->membership[k], colnr))
      {
        ...
        report(lp,
               IMPORTANT,
               "presolve_SOScheck: E - Sparse array did not indicate column index %d as member of SOS %d\n",
               colnr,
               lp->SOS->membership[k]);
      }
    }
...
INLINE REAL
presolve_roundrhs(lprec * lp, REAL value, MYBOOL isGE)
{
...
INLINE REAL
presolve_roundval(lprec * lp, REAL value)
{
#ifdef DoPresolveRounding
...
INLINE REAL
presolve_sumplumin(lprec * lp, int item, psrec * ps, MYBOOL doUpper)
{
  REAL *plu = (doUpper ? ps->pluupper : ps->plulower), *neg = (doUpper ? ps->negupper : ps->neglower);
...
INLINE void
presolve_range(lprec * lp, int rownr, psrec * ps, REAL * loValue, REAL * hiValue)
{
...
STATIC void
presolve_rangeorig(lprec * lp, int rownr, psrec * ps, REAL * loValue, REAL * hiValue, REAL delta)
{
...
STATIC MYBOOL
presolve_rowfeasible(presolverec * psdata, int rownr, MYBOOL userowmap)
{
  lprec * lp = psdata->lp;
  MYBOOL  status = TRUE;
  ...
  while ((status == TRUE) && (rownr != 0))
  {
    ...
    if (value < LHS - lp->epssolution)
    {
      ...
      report(lp,
             NORMAL,
             "presolve_rowfeasible: Lower bound infeasibility in %s row %s (%g << %g)\n",
             get_str_constr_type(lp, contype),
             get_row_name(lp, rownr),
             value,
             LHS);
      if (rownr != origrownr)
        report(
          lp, NORMAL, "        ...           Input row base used for testing was %s\n", get_row_name(lp, origrownr));
      status = FALSE;
    }
    ...
    if (value > RHS + lp->epssolution)
    {
      ...
      report(lp,
             NORMAL,
             "presolve_rowfeasible: Upper bound infeasibility in %s row %s (%g >> %g)\n",
             get_str_constr_type(lp, contype),
             get_row_name(lp, rownr),
             value,
             RHS);
...
STATIC MYBOOL
presolve_debugmap(presolverec * psdata, const char * caption)
{
  lprec *  lp = psdata->lp;
  MATrec * mat = lp->matA;
  ...
  for (colnr = 1; colnr <= lp->columns; colnr++)
  {
    ...
    for (jx = 1; jx <= je; jx++, rows++)
    {
      if ((*rows < 0) || (*rows > nz))
      {
        report(lp,
               SEVERE,
               "presolve_debugmap: NZ index %d for column %d out of range (index %d<=%d)\n",
               *rows,
               colnr,
               jx,
               je);
...
STATIC MYBOOL
presolve_validate(presolverec * psdata, MYBOOL forceupdate)
{
...
STATIC MYBOOL
presolve_rowtallies(presolverec * psdata, int rownr, int * plu, int * neg, int * pluneg)
{
  REAL     value;
  lprec *  lp = psdata->lp;
  MATrec * mat = lp->matA;
...
STATIC MYBOOL
presolve_debugrowtallies(presolverec * psdata)
{
  lprec * lp = psdata->lp;
  int     i, plu, neg, pluneg, nerr = 0;

  for (i = 1; i <= lp->rows; i++)
    if (isActiveLink(psdata->rows->varmap, i) && presolve_rowtallies(psdata, i, &plu, &neg, &pluneg))
    {
      if ((psdata->rows->plucount[i] != plu) || (psdata->rows->negcount[i] != neg) ||
...
STATIC int
presolve_debugcheck(lprec * lp, LLrec * rowmap, LLrec * colmap)
{
  int i, j, errc = 0;
...
STATIC MYBOOL
presolve_candeletevar(presolverec * psdata, int colnr)
{
  lprec * lp = psdata->lp;
...
STATIC int
presolve_rowlengthex(presolverec * psdata, int rownr)
{
  ...
  if (j1 != j2)
  {
    report(psdata->lp,
           SEVERE,
           "presolve_rowlengthex: Expected row length %d, but found %d in row %s\n",
           j2,
           j1,
           get_row_name(psdata->lp, rownr));
    j1 = -j1;
  }
#endif
...
STATIC int
presolve_rowlengthdebug(presolverec * psdata)
{
  int rownr, n = 0;

  for (rownr = firstActiveLink(psdata->rows->varmap); rownr != 0; rownr = nextActiveLink(psdata->rows->varmap, rownr))
    n += presolve_rowlengthex(psdata, rownr);
  return (n);
}

INLINE int
presolve_nextrecord(psrec * ps, int recnr, int * previtem)
{
...
INLINE int
presolve_nextcol(presolverec * psdata, int rownr, int * previtem)
...
INLINE int
presolve_lastcol(presolverec * psdata, int rownr)
{
  return (presolve_nextrecord(psdata->rows, rownr, NULL));
}
INLINE int
presolve_nextrow(presolverec * psdata, int colnr, int * previtem)
...
INLINE int
presolve_lastrow(presolverec * psdata, int colnr)
{
  return (presolve_nextrecord(psdata->cols, colnr, NULL));
}

INLINE void
presolve_adjustrhs(presolverec * psdata, int rownr, REAL fixdelta, REAL epsvalue)
{
  lprec * lp = psdata->lp;
...
STATIC int
presolve_shrink(presolverec * psdata, int * nConRemove, int * nVarRemove)
{
  SOSgroup * SOS = psdata->lp->SOS;
  int        status = RUNNING, countR = 0, countC = 0, i, ix, n, *list;
  REAL       fixValue;
...
STATIC void
presolve_rowremove(presolverec * psdata, int rownr, MYBOOL allowcoldelete)
{
  lprec *  lp = psdata->lp;
  MATrec * mat = lp->matA;
...
STATIC int
presolve_colremove(presolverec * psdata, int colnr, MYBOOL allowrowdelete)
{
  lprec * lp = psdata->lp;
...
STATIC int
presolve_redundantSOS(presolverec * psdata, int * nb, int * nSum)
{
  lprec *  lp = psdata->lp;
  int      i, ii, k, kk, j, nrows = lp->rows, *fixed = NULL, iBoundTighten = 0, status = RUNNING;
  SOSrec * SOS;
...
STATIC MYBOOL
presolve_fixSOS1(presolverec * psdata, int colnr, REAL fixvalue, int * nr, int * nv)
{
  (void)nr;
  lprec *  lp = psdata->lp;
  int      i, k, j;
...
STATIC void
presolve_setEQ(presolverec * psdata, int rownr)
{
  lprec * lp = psdata->lp;
...
STATIC MYBOOL
presolve_singletonbounds(presolverec * psdata, int rownr, int colnr, REAL * lobound, REAL * upbound, REAL * aval)
{
  lprec * lp = psdata->lp;
  ...
  if (!isneg)
  {
    ...
    if (!isneg)
      report(lp,
             NORMAL,
             "presolve_singletonbounds: Singleton variable %s in row %s infeasibility (%g << %g)\n",
             get_col_name(lp, colnr),
             get_row_name(lp, rownr),
             *lobound,
             *upbound);
  }
...
STATIC MYBOOL
presolve_altsingletonvalid(presolverec * psdata, int rownr, int colnr, REAL reflotest, REAL refuptest)
{
  lprec * lp = psdata->lp;
  ...
  if (epsvalue > PRESOLVE_BOUNDSLACK)
  {
    report(lp,
           NORMAL,
           "presolve_altsingletonvalid: Singleton variable %s in row %s infeasible (%g)\n",
           get_col_name(lp, colnr),
           get_row_name(lp, rownr),
           MAX(reflotest - coeff_bu, coeff_bl - refuptest));
    return (FALSE);
  }
  else
    return (TRUE);
...
STATIC MYBOOL
presolve_multibounds(presolverec * psdata,
                     int           rownr,
                     int           colnr,
                     REAL *        lobound,
                     REAL *        upbound,
...
STATIC MYBOOL
isnz_origobj(lprec * lp, int colnr)
{
  return ((MYBOOL)(lp->orig_obj[colnr] != 0));
}

STATIC MYBOOL
presolve_testrow(presolverec * psdata, int lastrow)
{
  (void)lastrow;
...
STATIC MYBOOL
presolve_coltighten(presolverec * psdata, int colnr, REAL LOnew, REAL UPnew, int * count)
{
  lprec *  lp = psdata->lp;
  ...
  if ((UPnew < lp->infinite) && (UPnew + margin < UPold))
  {
    ...
    if (UPold < lp->infinite)
    {
      ...
      for (; elmnr < elmend; elmnr++, rownr += matRowColStep, value += matValueStep)
      {
        k = *rownr;
        if (!isActiveLink(psdata->rows->varmap, k))
          continue;
  ...
  if (newcount > oldcount)
  {
    ...
    if (LOnew > UPnew)
    {
      ...
      else
      {
        report(lp,
               NORMAL,
               "presolve_coltighten: Found column %s with LB %g > UB %g\n",
               get_col_name(lp, colnr),
               LOnew,
               UPnew);
        return (FALSE);
    ...
    if (lp->spx_trace || (lp->verbose > DETAILED))
      report(lp,
             NORMAL,
             "presolve_coltighten: Replaced bounds on column %s to [%g ... %g]\n",
             get_col_name(lp, colnr),
             LOnew,
             UPnew);
...
STATIC int
presolve_rowtighten(presolverec * psdata, int rownr, int * tally, MYBOOL intsonly)
{
  lprec *  lp = psdata->lp;
  MYBOOL   rowbinds;
  int      item = 0, jx, jjx, ix, idxn = 0, *idxbound = NULL, status = RUNNING;
  REAL *   newbound = NULL, RHlo = get_rh_lower(lp, rownr), RHup = get_rh_upper(lp, rownr), VARlo, VARup, Aval;
  MATrec * mat = lp->matA;
  ...
  for (jx = presolve_nextcol(psdata, rownr, &item); jx >= 0; jx = presolve_nextcol(psdata, rownr, &item))
  {
  ...
  while (ix < idxn)
  {
    ...
    if (is_unbounded(lp, jx) || (intsonly && !is_int(lp, jx)))
      continue;
...
STATIC void
set_dv_bounds(presolverec * psdata, int rownr, REAL lowbo, REAL upbo)
{
  psdata->dv_lobo[rownr] = lowbo;
  psdata->dv_upbo[rownr] = upbo;
}
STATIC REAL
get_dv_lower(presolverec * psdata, int rownr)
{
  return (psdata->dv_lobo[rownr]);
}

STATIC REAL
get_dv_upper(presolverec * psdata, int rownr)
{
  return (psdata->dv_upbo[rownr]);
}

STATIC MYBOOL
presolve_rowfix(presolverec * psdata, int rownr, REAL newvalue, MYBOOL remove, int * tally)
{
  lprec *  lp = psdata->lp;
  int      i, ix, ie;
  MYBOOL   isneg, lofinite, upfinite, doupdate = FALSE, chsign = is_chsign(lp, rownr);
  REAL     lobound, upbound, lovalue, upvalue, Value, fixvalue, fixprod, mult;
  MATrec * mat = lp->matA;
  ...
  for (; ix < ie; ix++)
  {
    ...
    if (remove && ((i == 0) || (ps->next[i][0] == 1)) && !psdata->forceupdate)
    {
      ...
      if ((upvalue < Value) || (lovalue > Value))
      {
        report(lp,
               IMPORTANT,
               "presolve: Row %s (%g << %g) infeasibility in column %s (OF=%g)\n",
               get_row_name(lp, rownr),
               lovalue,
               upvalue,
               get_col_name(lp, i),
...
STATIC int
presolve_colsingleton(presolverec * psdata, int i, int j, int * count)
{
  lprec * lp = psdata->lp;
...
STATIC MYBOOL
presolve_colfix(presolverec * psdata, int colnr, REAL newvalue, MYBOOL remove, int * tally)
{
  lprec *  lp = psdata->lp;
  int      i, ix, ie;
  MYBOOL   isneg, lofinite, upfinite, doupdate = FALSE, doOF = TRUE;
  REAL     lobound, upbound, lovalue, upvalue, Value, fixvalue, mult;
  MATrec * mat = lp->matA;
  ...
  for (; doOF || (ix < ie); ix++, rownr += matRowColStep, value += matValueStep)
  {
  ...
  Restart:
    ...
    if (remove && ((i == 0) || (ps->next[i][0] == 1)) && !psdata->forceupdate)
    {
      if (i == 0)
      {
        ...
        report(lp,
               DETAILED,
               "presolve_colfix: Objective determined by presolve as %18g\n",
...
#endif
        if ((upvalue < get_rh_lower(lp, i) - Value) || (lovalue > get_rh_upper(lp, i) + Value))
        {
          report(lp,
                 NORMAL,
                 "presolve_colfix: Variable %s (%g << %g) infeasibility in row %s (%g << %g)\n",
                 get_col_name(lp, colnr),
                 lovalue,
                 upvalue,
                 get_row_name(lp, i),
...
STATIC int
presolve_rowfixzero(presolverec * psdata, int rownr, int * nv)
{
  lprec *  lp = psdata->lp;
  MATrec * mat = lp->matA;
...
STATIC MYBOOL
presolve_colfixdual(presolverec * psdata, int colnr, REAL * fixValue, int * status)
{
  lprec *  lp = psdata->lp;
  ...
  if (((loX < 0) && (upX > 0)) || (fabs(upX - loX) < lp->epsvalue) || SOS_is_member_of_type(lp->SOS, colnr, SOSn))
    return (FALSE);
  isMI = (MYBOOL)(upX <= 0);
  ...
  for (; (ix < ie) && isDualFREE; ix++, rownr += matRowColStep, value += matValueStep)
  {
    i = *rownr;
    if (!isActiveLink(psdata->rows->varmap, i))
      continue;
    ...
    {
      REAL val = my_chsign(is_chsign(lp, i), *value), loR = get_rh_lower(lp, i), upR = get_rh_upper(lp, i);
      ...
      isDualFREE =
        my_infinite(lp, get_rh_range(lp, i)) ||                                          /* Explicitly free */
...
#else
STATIC MYBOOL
presolve_probefix01(presolverec * psdata, int colnr, REAL * fixvalue)
{
  lprec *  lp = psdata->lp;
  int      i, ix, item;
  ...
  for (ix = presolve_nextrow(psdata, colnr, &item); (ix >= 0); ix = presolve_nextrow(psdata, colnr, &item))
  {
    ...
    *fixvalue = COL_MAT_VALUE(ix);
    ...
    if (!my_infinite(lp, range) && (upLim + *fixvalue < lp->orig_rhs[i] - range - tolgap))
    {
      if (*fixvalue > 0)
        presolve_setstatus(psdata, INFEASIBLE);
      *fixvalue = 0;
    ...
    if (((*fixvalue < 0) && (upLim + *fixvalue >= loLim - tolgap) && (upLim > lp->orig_rhs[i] + tolgap)) ||
        ((*fixvalue > 0) && (loLim + *fixvalue <= upLim + tolgap) && (loLim < lp->orig_rhs[i] - range - tolgap) &&
         !my_infinite(lp, range)))
    {
      *fixvalue = 1;
      break;
    }
...
STATIC int
presolve_probetighten01(presolverec * psdata, int colnr)
{
  lprec *  lp = psdata->lp;
  MYBOOL   chsign;
  int      i, ix, item, n = 0;
...
STATIC int
presolve_mergerows(presolverec * psdata, int * nRows, int * nSum)
{
  lprec *  lp = psdata->lp;
  MYBOOL   candelete;
  int      status = RUNNING, item1, item2, firstix, RT1, RT2, i, ix, iix, j, jjx, n = 0;
...
#endif
    firstix = ix;
    for (RT1 = 0; (ix > 0) && (RT1 < RT2) && (status == RUNNING); ix = prevActiveLink(psdata->rows->varmap, ix), RT1++)
    {
      candelete = FALSE;
      ...
      jjx = presolve_nextcol(psdata, i, &item2);
      for (; (jjx >= 0) && (Value1 == bound); jjx = presolve_nextcol(psdata, i, &item2))
      {
        iix = presolve_nextcol(psdata, ix, &item1);
        if (ROW_MAT_COLNR(iix) != ROW_MAT_COLNR(jjx))
          break;
      ...
      if (jjx < 0)
      {
        ...
        else
        {
          ...
          if (!candelete)
          {
            report(lp,
                   NORMAL,
                   "presolve: Range infeasibility found involving rows %s and %s\n",
                   get_row_name(lp, ix),
                   get_row_name(lp, i));
          }
        }
...
STATIC MYBOOL
presolve_reduceGCD(presolverec * psdata, int * nn, int * nb, int * nsum)
{
  lprec *  lp = psdata->lp;
  MYBOOL   status = TRUE;
...
STATIC int
presolve_knapsack(presolverec * psdata, int * nn)
{
  lprec *  lp = psdata->lp;
  int      m, n, i, ix, j, jx, colnr, *rownr = NULL, status = RUNNING;
...
STATIC MYBOOL
presolve_invalideq2(lprec * lp, presolverec * psdata)
{
  int      jx, jjx, i = 0, item;
  MATrec * mat = lp->matA;
  MYBOOL   error = FALSE;
...
int BFP_CALLMODEL
presolve_getcolumnEQ(lprec * lp, int colnr, REAL nzvalues[], int nzrows[], int mapin[])
{
  int      i, ib, ie, nn = 0;
  MATrec * mat = lp->matA;
...
STATIC int
presolve_singularities(presolverec * psdata, int * nn, int * nr, int * nv, int * nSum)
{
  (void)nv;
  lprec * lp = psdata->lp;
...
STATIC int
presolve_elimeq2(presolverec * psdata, int * nn, int * nr, int * nc, int * nSum)
{
  lprec * lp = psdata->lp;
  int     n, i, jx, jjx, k, item, *plucount, *negcount, colplu, colneg,
    iCoeffChanged = 0, iRowsRemoved = 0, iVarsFixed = 0, nrows = lp->rows, status = RUNNING, *colindex = NULL;
  MYBOOL freshupdate;
  REAL   Coeff1, Coeff2, Value1, Value2, lobound, upbound, bound, test, product, *colvalue = NULL, *delvalue = NULL,
                                                                               *colitem;
  ...
  if ((EQ2 == NULL) || !allocREAL(lp, &colvalue, nrows + 1, FALSE) || !allocREAL(lp, &delvalue, nrows + 1, FALSE))
    goto Finish;
  for (i = firstActiveLink(psdata->EQmap); i > 0; i = nextActiveLink(psdata->EQmap, i))
  {
    if (presolve_rowlength(psdata, i) == 2)
  ...
  for (i = firstActiveLink(EQ2); i > 0; i = nextActiveLink(EQ2, i))
  {
    ...
    if (jx < 0)
      report(lp,
             SEVERE,
             "presolve_elimeq2: No qualifying %dst column was found in row %s (ostensible length %d)\n",
             1,
             get_row_name(lp, i),
             presolve_rowlength(psdata, i));
#endif
    ...
    if (jjx < 0)
      report(lp,
             SEVERE,
             "presolve_elimeq2: No qualifying %dnd column was found in row %s (ostensible length %d)\n",
             2,
             get_row_name(lp, i),
             presolve_rowlength(psdata, i));
#endif
    ...
    if ((fabs(Coeff1) < psdata->epspivot * mat->colmax[jx]) && ((fabs(Coeff1) != 1) && (fabs(Coeff2) != 1)) &&
    ...
    if (k == 0)
    {
      ...
      if (k == 0)
      {
        if ((fabs(Coeff2) < psdata->epspivot * mat->colmax[jjx]) && (fabs(Coeff1) > psdata->epspivot * mat->colmax[jx]))
          k += 64;
      ...
      if (k == 0)
      {
        ...
        if ((Value2 * lp->orig_obj[jx] < 0) && (Value2 * lp->orig_obj[jjx] > 0)) /* Seek increased dual feasibility */
          k += 256;
#endif
    ...
    for (k = 0; k <= nrows; k++, colitem++)
    {
      ...
      bound = delvalue[k];
      if ((k == i) || (bound == 0) || ((k > 0) && !isActiveLink(psdata->rows->varmap, k)))
        continue;
    ...
    if (item == 0)
    {
      ...
      report(lp,
             DETAILED,
             "presolve_elimeq2: Was able to remove variables %d and %d in row %s\n",
             jx,
             jjx,
             get_row_name(lp, i));
#endif
...
STATIC MYBOOL
presolve_impliedfree(lprec * lp, presolverec * psdata, int colnr)
{
  int      i, ix, ie;
  REAL     Tlower, Tupper;
...
STATIC MYBOOL
presolve_impliedcolfix(presolverec * psdata, int rownr, int colnr, MYBOOL isfree)
{
  lprec *  lp = psdata->lp;
  MYBOOL   signflip, undoadded = FALSE;
  MATrec * mat = lp->matA;
  int      jx, i, ib, ie = mat->row_end[rownr];
  REAL     varLo = 0, varHi = 0, varRange, conRange = 0, matValue = 0, dual, RHS = lp->orig_rhs[rownr], pivot,
       matAij = mat_getitem(mat, rownr, colnr), *vecOF = lp->orig_obj;
  ...
  if (is_int(lp, colnr))
  {
    ...
    for (ib = presolve_nextcol(psdata, rownr, &i); i != 0; ib = presolve_nextcol(psdata, rownr, &i))
    {
      ...
      else if ((pivot > dual + psdata->epsvalue) || ((pivot > 0) && (fabs(fmod(dual, pivot)) > psdata->epsvalue)))
        return (FALSE);
    }
  }
  ...
  else
  {
    ...
    if (isfree)
    {
      ...
      signflip = (MYBOOL)((dual > 0) && !my_infinite(lp, conRange));
    }
    else
    {
  ...
  i = 0;
  for (ib = presolve_nextcol(psdata, rownr, &i); ib >= 0; ib = presolve_nextcol(psdata, rownr, &i))
  {
...
STATIC psrec *
presolve_initpsrec(lprec * lp, int size)
{
...
STATIC presolverec *
presolve_init(lprec * lp)
{
  int           k, i, ix, ixx, colnr, ncols = lp->columns, nrows = lp->rows;
  REAL          hold;
  MATrec *      mat = lp->matA;
...
STATIC void
presolve_free(presolverec ** psdata)
{
...
STATIC int
presolve_makefree(presolverec * psdata)
{
  lprec *  lp = psdata->lp;
  int      i, ix, j, nn = 0;
  ...
  for (i = firstActiveLink(psdata->rows->varmap); i != 0; i = nextActiveLink(psdata->rows->varmap, i))
  {
    ...
    if (presolve_rowlength(psdata, i) > 1)
    {
      if ((is_constr_type(lp, i, GE) && (upsum <= uprhs)) || (is_constr_type(lp, i, LE) && (losum >= lorhs)))
...
STATIC MYBOOL
presolve_updatesums(presolverec * psdata)
{
  lprec * lp = psdata->lp;
  int     j;
  ...
  for (j = firstActiveLink(psdata->rows->varmap); j != 0; j = nextActiveLink(psdata->rows->varmap, j))
  {
...
STATIC MYBOOL
presolve_finalize(presolverec * psdata)
{
  lprec * lp = psdata->lp;
  ...
  if ((n > 0) && (ke > 0))
  {
    ...
    mat_colcompact(lp->matA, lp->presolve_undo->orig_rows, lp->presolve_undo->orig_columns);
    compactvars = TRUE;
  }
  ...
  if (compactvars)
    varmap_compact(lp, lp->presolve_undo->orig_rows, lp->presolve_undo->orig_columns);
...
STATIC MYBOOL
presolve_debugdump(lprec * lp, presolverec * psdata, const char * filename, MYBOOL doappend)
{
  FILE * output = stdout;
  int    size;
  MYBOOL ok;
  ...
  fprintf(output,
          "Model size:     %d rows (%d equalities, %d less than), %d columns\n",
          psdata->rows->varmap->count,
          psdata->EQmap->count,
          psdata->LTmap->count,
          psdata->cols->varmap->count);
...
int CMP_CALLMODEL
compRedundant(const UNIONTYPE QSORTrec * current, const UNIONTYPE QSORTrec * candidate)
{
  int start1 = (int)(current->int4.intpar1), start2 = (int)(candidate->int4.intpar1),
...
int CMP_CALLMODEL
compSparsity(const UNIONTYPE QSORTrec * current, const UNIONTYPE QSORTrec * candidate)
{
  int start1 = (int)(current->int4.intpar1), start2 = (int)(candidate->int4.intpar1),
      result = CMP_COMPARE(start1, start2);
...
int CMP_CALLMODEL
compAggregate(const UNIONTYPE QSORTrec * current, const UNIONTYPE QSORTrec * candidate)
{
  int     index1 = (int)(current->pvoidint2.intval), index2 = (int)(candidate->pvoidint2.intval);
  lprec * lp = (lprec *)current->pvoidint2.ptr;
  REAL    value1 = lp->orig_obj[index1], value2 = lp->orig_obj[index2];
...
STATIC int
presolve_rowdominance(presolverec * psdata, int * nCoeffChanged, int * nRowsRemoved, int * nVarsFixed, int * nSum)
{
  lprec *  lp = psdata->lp;
  MATrec * mat = lp->matA;
  int      i, ii, ib, ie, n, jb, je, jx, *coldel = NULL, status = RUNNING, item, iCoeffChanged = 0, iRowRemoved = 0,
                                    iVarFixed = 0;
  ...
  for (i = firstActiveLink(psdata->EQmap); i != 0; i = nextActiveLink(psdata->EQmap, i))
  {
    ...
    {
      item = 0;
      for (jb = presolve_nextcol(psdata, i, &item); jb >= 0; jb = presolve_nextcol(psdata, i, &item))
      {
        jx = ROW_MAT_COLNR(jb);
  ...
  if (!allocREAL(lp, &rowvalues, lp->columns + 1, TRUE) || !allocINT(lp, &coldel, lp->columns + 1, FALSE))
    goto Finish;
  ...
  for (ib = 0; ib < n; ib++)
  {
    ...
    for (ie = ib + 1; ie < n; ie++)
    {
      ...
      if ((lp->orig_rhs[i] == 0) && (lp->orig_rhs[ii] == 0))
        ...
        continue;
      item = 0;
      for (jb = presolve_nextcol(psdata, ii, &item); jb >= 0; jb = presolve_nextcol(psdata, ii, &item))
      {
        jx = ROW_MAT_COLNR(jb);
        if (rowvalues[jx] == 0)
          break;
      ...
      if (jb < 0)
      {
        ...
        for (jb = presolve_nextcol(psdata, i, &item); jb >= 0; jb = presolve_nextcol(psdata, i, &item))
        {
          ...
          if (mat_findelm(mat, ii, jx) <= 0)
          {
            ...
            if ((lp->orig_lowbo[lp->rows + jx] < 0) && (lp->orig_upbo[lp->rows + jx] > 0))
            {
              coldel[0] = -1;
              break;
            }
            ...
            else if ((lp->orig_lowbo[lp->rows + jx] > 0) || (lp->orig_upbo[lp->rows + jx] < 0))
            {
              report(lp,
                     DETAILED,
                     "presolve_rowdominate: Column %s is infeasible due to conflict in rows %s and %s\n",
                     get_col_name(lp, jx),
                     get_row_name(lp, i),
                     get_row_name(lp, ii));
              coldel[0] = -1;
...
STATIC int
presolve_coldominance01(presolverec * psdata, NATURAL * nConRemoved, NATURAL * nVarsFixed, NATURAL * nSum)
/* The current version of this routine eliminates binary variables
   that are dominated via set coverage or unit knapsack constraints */
{
  (void)nConRemoved;
  lprec *              lp = psdata->lp;
  MATrec *             mat = lp->matA;
  NATURAL              i, ib, ie, jx, item, item2, n = lp->int_vars, iVarFixed = 0, nrows = lp->rows, *coldel = NULL;
  int                  jb, jj, ii, status = RUNNING;
  REAL                 rhsval = 0.0, *colvalues = NULL, *colobj = NULL;
  LLrec *              sets = NULL;
  ...
  for (i = firstActiveLink(psdata->cols->varmap); i != 0; i = nextActiveLink(psdata->cols->varmap, i))
    if (is_binary(lp, i) && !SOS_is_member(lp->SOS, 0, i))
    {
      ...
      item = 0;
      for (jb = presolve_nextrow(psdata, i, &item); jb >= 0; jb = presolve_nextrow(psdata, i, &item))
      {
        jx = COL_MAT_ROWNR(jb);
  ...
  /* Let us start from the top of the list, going forward and looking
    for the longest possible dominated column */
  if (!allocREAL(lp, &colvalues, nrows + 1, TRUE) || !allocREAL(lp, &colobj, n + 1, FALSE) ||
...
#  endif
      ...
      item = 0;
      item2 = 0;
      for (jb = presolve_nextrow(psdata, ii, &item), jj = presolve_nextrow(psdata, i, &item2); jb >= 0;
           jb = presolve_nextrow(psdata, ii, &item), jj = presolve_nextrow(psdata, i, &item2))
      {
        jx = COL_MAT_ROWNR(jb);
        if (jx != COL_MAT_ROWNR(jj))
          break;
...
STATIC int
presolve_aggregate(presolverec * psdata, int * nConRemoved, int * nVarsFixed, int * nSum)
/* This routine combines compatible or identical columns */
{
  ...
  MATrec * mat = lp->matA;
  MYBOOL   first;
  int      i, ii, ib, ie, ix, n, jb, je, jx, jj, item, item2, *coldel = NULL, status = RUNNING, iVarFixed = 0;
  ...
  if (!allocREAL(lp, &colvalues, lp->rows + 1, TRUE) || !allocINT(lp, &coldel, lp->columns + 1, FALSE))
    goto Finish;
      ...
      while ((status == RUNNING) && (jb < coldel[0]))
      {
               ...
               (fabs(lp->orig_obj[ix = QSagg[je].pvoidint2.intval] - of) < psdata->epsvalue))
        {
          ...
          if (is_int(lp, ix) == isint)
          {
            ...
            if (my_infinite(lp, lp->orig_upbo[lp->rows + ii]))
            { /* Case 1 (recipe.mps) */
            ...
            else if (my_infinite(lp, lp->orig_lowbo[lp->rows + ii]))
            { /* Case 2 */
              /* Do nothing */
            }
...
STATIC int
presolve_makesparser(presolverec * psdata, int * nCoeffChanged, int * nConRemove, int * nVarFixed, int * nSum)
{
  (void)nVarFixed;
  lprec *  lp = psdata->lp;
  MATrec * mat = lp->matA;
  ...
  for (ix = firstActiveLink(EQlist); ix != 0;)
  {
    ...
    for (ib = 1; ib < ix; ib++)
    {
             ...
             (fabs(test - ratio) < psdata->epsvalue))
      {
        ...
        jx = 0;
        jjx = ROW_MAT_COLNR(jjx);
        for (k = presolve_nextcol(psdata, i, &item); (jx < jjx) && (item > 0); k = presolve_nextcol(psdata, i, &item))
        {
          jx = ROW_MAT_COLNR(k);
      ...
      if ((itemEQ == 0) && (nzidx[0] > 0) && (fabs(test - ratio) < psdata->epsvalue))
      {
        ...
        if (presolve_rowlength(psdata, i) == presolve_rowlength(psdata, ii))
        {
          ...
          else
          {
            ...
            if ((value + psdata->epsvalue < valueEQ) || (value - get_rh_range(lp, i) - psdata->epsvalue > valueEQ))
            {
              report(
                lp, NORMAL, "presolve_sparser: Infeasibility of relatively equal RHS values for %d and %d\n", i, ii);
              status = presolve_setstatus(psdata, INFEASIBLE);
              goto Finish;
            }
            presolve_rowremove(psdata, i, TRUE);
...
STATIC int
presolve_SOS1(presolverec * psdata, int * nCoeffChanged, int * nConRemove, int * nVarFixed, int * nSOS, int * nSum)
{
  (void)nVarFixed;
  lprec *  lp = psdata->lp;
  MYBOOL   candelete, SOS_GUBactive = FALSE;
  int      iCoeffChanged = 0, iConRemove = 0, iSOS = 0, i, ix, iix, j, jx, jjx, status = RUNNING;
  REAL     Value1;
  MATrec * mat = lp->matA;
  (void)nVarFixed;
...
STATIC int
presolve_boundconflict(presolverec * psdata, int baserowno, int colno)
{
  REAL     Value1, Value2;
  lprec *  lp = psdata->lp;
  MATrec * mat = lp->matA;
  int      ix, item = 0, status = RUNNING;
  ...
  Value1 = get_rh_upper(lp, baserowno), Value2 = get_rh_lower(lp, baserowno);

  if (presolve_singletonbounds(psdata, baserowno, colno, &Value2, &Value1, NULL))
  {
    int iix;
    item = 0;
    for (ix = presolve_nextrow(psdata, colno, &item); ix >= 0; ix = presolve_nextrow(psdata, colno, &item))
    {
      iix = COL_MAT_ROWNR(ix);
      if ((iix != baserowno) && (presolve_rowlength(psdata, iix) == 1) &&
...
STATIC int
presolve_columns(presolverec * psdata,
                 int *         nCoeffChanged,
                 int *         nConRemove,
                 int *         nVarFixed,
                 int *         nBoundTighten,
...
{
  lprec * lp = psdata->lp;
  MYBOOL  candelete, isOFNZ, unbounded, probefix = is_presolve(lp, PRESOLVE_PROBEFIX),
#if 0
           probereduce = is_presolve(lp, PRESOLVE_PROBEREDUCE),
#endif
                                       colfixdual = is_presolve(lp, PRESOLVE_COLFIXDUAL);
  int  iCoeffChanged = 0, iConRemove = 0, iVarFixed = 0, iBoundTighten = 0, status = RUNNING, ix, j, countNZ, item;
  REAL Value1;
  ...
  for (j = firstActiveLink(psdata->cols->varmap); (j != 0) && (status == RUNNING);)
  {
    ...
    else if ((countNZ == 0) && isOFNZ)
    {
      ...
      else
      {
        ...
        report(
          lp, DETAILED, "presolve_columns: Eliminated trivial variable %s fixed at %g\n", get_col_name(lp, j), Value1);
        candelete = TRUE;
      }
    }
    ...
    else if (colfixdual && presolve_colfixdual(psdata, j, &Value1, &status))
    {
      ...
      else
      {
        ...
        report(lp,
               DETAILED,
               "presolve_columns: Eliminated dual-zero variable %s fixed at %g\n",
               get_col_name(lp, j),
               Value1);
        candelete = TRUE;
      }
    ...
    else if (probefix && is_binary(lp, j) && presolve_probefix01(psdata, j, &Value1))
    {
...
STATIC int
presolve_freeandslacks(presolverec * psdata, int * nCoeffChanged, int * nConRemove, int * nVarFixed, int * nSum)
{
  lprec * lp = psdata->lp;
  MYBOOL  isOFNZ, unbounded, impliedfree = is_presolve(lp, PRESOLVE_IMPLIEDFREE),
                            impliedslack = is_presolve(lp, PRESOLVE_IMPLIEDSLK);
  int      iCoeffChanged = 0, iConRemove = 0, iVarFixed = 0, status = RUNNING, i, ix, j, countNZ;
  ...
  if (impliedfree || impliedslack)
    for (j = firstActiveLink(psdata->cols->varmap); j != 0;)
    {
      ...
      if ((presolve_collength(psdata, j) != 1) || is_int(lp, j) || is_semicont(lp, j) ||
      ...
      if (impliedfree && unbounded && presolve_impliedcolfix(psdata, i, j, TRUE))
      {
        report(lp,
               DETAILED,
               "presolve_freeandslacks: Eliminated free variable %s and row %s\n",
               get_col_name(lp, j),
               get_row_name(lp, i));
        presolve_rowremove(psdata, i, TRUE);
        iConRemove++;
      ...
      else if (impliedslack && (countNZ > 1) && is_constr_type(lp, i, EQ) &&
               presolve_impliedcolfix(psdata, i, j, FALSE))
      {
        report(lp,
               DETAILED,
               "presolve_freeandslacks: Eliminated implied slack variable %s via row %s\n",
               get_col_name(lp, j),
               get_row_name(lp, i));
        psdata->forceupdate = TRUE;
        j = presolve_colremove(psdata, j, TRUE);
      ...
      else if (impliedslack && !isOFNZ && my_infinite(lp, coeff_bu) && /* Consider removing this test */
#if 0                                                                  /* Force zero-bounded implicit slack  */
             (coeff_bl == 0)) &&
#else
               !my_infinite(lp, coeff_bl) &&
#endif
               (countNZ > 1) && !is_constr_type(lp, i, EQ))
      {
        REAL *target, ValueA = COL_MAT_VALUE(presolve_lastrow(psdata, j));
#if 0
      ...
      if(!presolve_singletonbounds(psdata, i, j, &coeff_bl, &coeff_bu, &ValueA)) {
        ...
        report(lp,
               DETAILED,
               "presolve_freeandslacks: Eliminated duplicate slack variable %s via row %s\n",
               get_col_name(lp, j),
               get_row_name(lp, i));
        j = presolve_colremove(psdata, j, TRUE);
      }
...
STATIC int
presolve_preparerows(presolverec * psdata, int * nBoundTighten, int * nSum)
{
  lprec *  lp = psdata->lp;
  MYBOOL   impliedfree = is_presolve(lp, PRESOLVE_IMPLIEDFREE), tightenbounds = is_presolve(lp, PRESOLVE_BOUNDS);
  ...
  for (i = lastActiveLink(psdata->rows->varmap); i > 0; i = prevActiveLink(psdata->rows->varmap, i))
  {
    ...
    if (impliedfree && (j > 1) && mat_validate(mat))
    {
      ...
      lorhs = get_rh_lower(lp, i);
      uprhs = get_rh_upper(lp, i);
      if ((losum > MIN(upsum, uprhs) + epsvalue) || (upsum < MAX(losum, lorhs) - epsvalue))
      {
        report(lp,
               NORMAL,
               "presolve_preparerows: Variable bound / constraint value infeasibility in row %s.\n",
               get_row_name(lp, i));
        status = presolve_setstatus(psdata, INFEASIBLE);
        break;
      }
...
STATIC int
presolve_rows(presolverec * psdata,
              int *         nCoeffChanged,
              int *         nConRemove,
              int *         nVarFixed,
              int *         nBoundTighten,
              int *         nSum)
{
  lprec *  lp = psdata->lp;
  MYBOOL   candelete;
  int      iCoeffChanged = 0, iConRemove = 0, iVarFixed = 0, iBoundTighten = 0, status = RUNNING, i, ix, j, jx, item;
...
presolve(lprec * lp)
{
  int status = RUNNING, i, j = 0, jx = 0, jjx = 0, k, oSum, iCoeffChanged = 0, iConRemove = 0, iVarFixed = 0,
      iBoundTighten = 0, iSOS = 0, iSum = 0, nCoeffChanged = 0, nConRemove = 0, nVarFixed = 0, nBoundTighten = 0,
      nSOS = 0, nSum = 0;
  ...
  if (lp->wasPresolved)
  {
    ...
    if ((lp->solvecount > 1) && (lp->bb_level < 1) && ((lp->scalemode & SCALE_DYNUPDATE) != 0))
      auto_scale(lp);
    if (!lp->basis_valid)
    {
      crash_basis(lp);
...
#  endif
  ...
  else
  {
    ...
    for (i = 1; i <= SOS_count(lp); i++)
    {
      ...
      if (k > 0)
      {
        ...
        report(lp,
               NORMAL,
               "presolve: Found SOS %d (type %d) to be range-infeasible on variable %d\n",
               i,
               SOS_get_type(lp->SOS, i),
               k);
    ...
    do
    {
      ...
      do
      {
        ...
        do
        {
          ...
          if (presolve_statuscheck(psdata, &status) && is_presolve(lp, PRESOLVE_ROWS))
          ...
          if (presolve_statuscheck(psdata, &status) && is_presolve(lp, PRESOLVE_COLS))
        ...
        if (presolve_statuscheck(psdata, &status) && (psdata->outerloops == 1) &&
            (psdata->middleloops <= MAX_PSMERGELOOPS) && is_presolve(lp, PRESOLVE_MERGEROWS))
          status = presolve_mergerows(psdata, &iConRemove, &iSum);

        /* Eliminate dominated rows */
        if (presolve_statuscheck(psdata, &status) && is_presolve(lp, PRESOLVE_ROWDOMINATE))
        ...
        if (presolve_statuscheck(psdata, &status) && (MIP_count(lp) > 0) && is_presolve(lp, PRESOLVE_SOS))
        ...
        if (presolve_statuscheck(psdata, &status) && (lp->int_vars > 1) && is_presolve(lp, PRESOLVE_COLDOMINATE))
      ...
      if (presolve_statuscheck(psdata, &status) && is_presolve(lp, PRESOLVE_ELIMEQ2))
      {
        jjx = 0;
        do
        {
      ...
      if (presolve_statuscheck(psdata, &status) && (psdata->EQmap->count > 0) && is_presolve(lp, PRESOLVE_SPARSER))
      ...
      if (presolve_statuscheck(psdata, &status) && (psdata->INTmap->count > 0) && is_presolve(lp, PRESOLVE_REDUCEGCD))
      ...
      if (presolve_statuscheck(psdata, &status) && is_presolve(lp, PRESOLVE_KNAPSACK))
      {
        i = iCoeffChanged;
      ...
      if (iSum > 0)
        report(lp,
               NORMAL,
               "Presolve O:%d -> Reduced rows:%5d, cols:%5d --- changed bnds:%5d, Ab:%5d.\n",
               psdata->outerloops,
               iConRemove,
               iVarFixed,
               iBoundTighten,
    ...
    } while (presolve_statuscheck(psdata, &status) && (psdata->forceupdate || (oSum < nSum)) &&
    ...
    if (nSum > 0)
      report(lp,
             i,
             "PRESOLVE             Elimination loops performed.......... O%d:M%d:I%d\n",
             psdata->outerloops,
             psdata->middleloops,
             psdata->innerloops);
    if (nVarFixed)
  ...
  if ((lp->usermessage != NULL) && ((lp->do_presolve & PRESOLVE_LASTMASKMODE) != 0) && (lp->msgmask & MSG_PRESOLVE))
...
postsolve(lprec * lp, int status)
{
  ...
  if (lp->lag_status != RUNNING)
  {
    ...
    {
      itemp = check_solution(lp, lp->columns, lp->best_solution, lp->orig_upbo, lp->orig_lowbo, lp->epssolution);
    ...
    else if (status != PRESOLVED)
    {
      report(lp,
             NORMAL,
             "lp_solve unsuccessful after %.0f iter and a last best value of %g\n",
