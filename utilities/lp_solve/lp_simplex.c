...
STATIC void
stallMonitor_update(lprec * lp, REAL newOF)
{
  int         newpos;
...
STATIC MYBOOL
stallMonitor_creepingObj(lprec * lp)
{
  OBJmonrec * monitor = lp->monitor;

  if (monitor->countstep > 1)
  {
    REAL deltaOF = (monitor->objstep[monitor->currentstep] - monitor->objstep[monitor->startstep]) / monitor->countstep;
    deltaOF /= MAX(1, (monitor->idxstep[monitor->currentstep] - monitor->idxstep[monitor->startstep]));
...
STATIC MYBOOL
stallMonitor_shortSteps(lprec * lp)
{
  OBJmonrec * monitor = lp->monitor;
  ...
  {
    REAL deltaOF =
      MAX(1, (monitor->idxstep[monitor->currentstep] - monitor->idxstep[monitor->startstep])) / monitor->countstep;
...
STATIC MYBOOL
stallMonitor_create(lprec * lp, MYBOOL isdual, const char * funcname)
{
  ...
  if (MAX_STALLCOUNT <= 1)
    ...
    monitor->limitstall[FALSE] = MAX(MAX_STALLCOUNT, (int)pow((REAL)(lp->rows + lp->columns) / 2, 0.667));
#if 1
  ...
  if (MAX_RULESWITCH <= 0)
    ...
    monitor->limitruleswitches = MAX(MAX_RULESWITCH, lp->rows / MAX_RULESWITCH);
...
STATIC MYBOOL
stallMonitor_check(lprec * lp, int rownr, int colnr, int lastnr, MYBOOL minit, MYBOOL approved, MYBOOL * forceoutEQ)
{
  OBJmonrec * monitor = lp->monitor;
  ...
  if (lp->spx_trace && (lastnr > 0))
    report(lp,
           NORMAL,
           "%s: Objective at iter %10.0f is " RESULTVALUEMASK " (%4d: %4d %s- %4d)\n",
           monitor->spxfunc,
           (double)get_total_iter(lp),
           monitor->thisobj,
           rownr,
           lastnr,
           my_if(minit == ITERATE_MAJORMAJOR, "<", "|"),
           colnr);
  ...
  if (isStalled || isCreeping)
  {
    ...
    { /* Obvious cycling */
      ...
      if (!approved && !is_anti_degen(lp, ANTIDEGEN_STALLING))
      {
        ...
        report(lp,
               msglevel,
               "%s: Stalling at iter %10.0f; no alternative strategy left.\n",
               monitor->spxfunc,
               (double)get_total_iter(lp));
        acceptance = FALSE;
        return (acceptance);
      ...
      if (approved && (monitor->pivrule != altrule) && (monitor->pivrule == monitor->oldpivrule))
      {
        ...
        report(lp,
               msglevel,
               "%s: Stalling at iter %10.0f; changed to '%s' rule.\n",
               monitor->spxfunc,
               (double)get_total_iter(lp),
      ...
      else
      {
        report(lp,
               msglevel,
               "%s: Stalling at iter %10.0f; proceed to bound relaxation.\n",
               monitor->spxfunc,
               (double)get_total_iter(lp));
        acceptance = FALSE;
        lp->spx_status = DEGENERATE;
...
STATIC MYBOOL
add_artificial(lprec * lp, int forrownr, REAL * nzarray, int * idxarray)
...
STATIC int
get_artificialRow(lprec * lp, int colnr)
{
  MATrec * mat = lp->matA;
...
STATIC int
findAnti_artificial(lprec * lp, int colnr)
...
STATIC int
findBasicArtificial(lprec * lp, int before)
{
...
STATIC void
eliminate_artificials(lprec * lp, REAL * prow)
{
...
STATIC int
primloop(lprec * lp, MYBOOL primalfeasible, REAL primaloffset)
{
  (void)primaloffset;
  MYBOOL primal = TRUE, bfpfinal = FALSE, changedphase = FALSE, forceoutEQ = AUTOMATIC, primalphase1, pricerCanChange,
         minit, stallaccept, pendingunbounded;
  int   i, j, k, colnr = 0, rownr = 0, lastnr = 0, candidatecount = 0, minitcount = 0, ok = TRUE;
  LREAL theta = 0.0;
  REAL  epsvalue, xviolated = 0.0, cviolated = 0.0, *prow = NULL, *pcol = NULL, *drow = lp->drow;
  int * workINT = NULL, *nzdrow = lp->nzdrow;
  (void)primaloffset;
  ...
  ok = allocREAL(lp, &(lp->bsolveVal), lp->rows + 1, FALSE) && allocREAL(lp, &prow, lp->sum + 1, TRUE) &&
       allocREAL(lp, &pcol, lp->rows + 1, TRUE);
  if (is_piv_mode(lp, PRICE_MULTIPLE) && (lp->multiblockdiv > 1))
  {
    lp->multivars = multi_create(lp, FALSE);
    ok &= (lp->multivars != NULL) && multi_resize(lp->multivars, lp->sum / lp->multiblockdiv, 2, FALSE, TRUE);
  }
  if (!ok)
    goto Finish;
...
#  endif
      ...
      if ((lp->bb_level <= 1) || (lp->improve & IMPROVE_BBSIMPLEX) /* || (lp->bb_rule & NODE_RCOSTFIXING) */)
      { /* NODE_RCOSTFIXING fix */
        ...
        if (i > 0)
        {
          ...
          if (lp->total_iter == 0)
            report(lp,
                   DETAILED,
                   "primloop: Lost primal feasibility at iter  %10.0f: will try to recover.\n",
                   (double)get_total_iter(lp));
        }
      }
    }
    ...
    if (is_action(lp->spx_action, ACTION_ITERATE))
    {
      ...
      if ((minit == ITERATE_MAJORMAJOR) && (lastnr > lp->sum - abs(lp->P1extraDim)))
      {
        ...
        if (lp->is_basic[lastnr] || !lp->is_basic[colnr])
          report(lp,
                 SEVERE,
                 "primloop: Invalid basis indicator for variable %d at iter %10.0f.\n",
                 lastnr,
                 (double)get_total_iter(lp));
#endif
...
STATIC int
dualloop(lprec * lp, MYBOOL dualfeasible, int dualinfeasibles[], REAL dualoffset)
{
  MYBOOL primal = FALSE, inP1extra, dualphase1 = FALSE, changedphase = TRUE, pricerCanChange, minit, stallaccept,
         longsteps, forceoutEQ = FALSE, bfpfinal = FALSE;
  ...
  int * boundswaps = NULL;
  LREAL theta = 0.0;
  REAL  epsvalue, xviolated, cviolated, *prow = NULL, *pcol = NULL, *drow = lp->drow;
  int * nzprow = NULL, *workINT = NULL, *nzdrow = lp->nzdrow;

  if (lp->spx_trace)
    report(lp, DETAILED, "Entered dual simplex algorithm with feasibility %s.\n", my_boolstr(dualfeasible));

  /* Allocate work arrays */
  ok = allocREAL(lp, &prow, lp->sum + 1, TRUE) && allocINT(lp, &nzprow, lp->sum + 1, FALSE) &&
       allocREAL(lp, &pcol, lp->rows + 1, TRUE);
  if (!ok)
    goto Finish;
  ...
  if (longsteps)
  {
    ...
    ok = (lp->longsteps != NULL) && multi_resize(lp->longsteps, MIN(lp->boundedvars + 2, 11), 1, TRUE, TRUE);
    if (!ok)
      goto Finish;
  ...
  /* Check if we have equality slacks in the basis and we should try to
     ...
     forceoutEQ = FALSE :    Only eliminate assured "good" violated
                  ...
                  TRUE:      Force remaining equality slacks out of the
                             basis */
  if (dualphase1 || inP1extra || ((lp->fixedvars > 0) && is_anti_degen(lp, ANTIDEGEN_FIXEDVARS)))
  {
  ...
  RetryRow:
    ...
    if (rownr > 0)
    {
      colnr = coldual(lp,
                      ...
                      (MYBOOL)(minit == ITERATE_MINORRETRY),
                      &candidatecount,
                      &cviolated);
      if (colnr < 0)
      {
    ...
    if (rownr > 0)
    {
      ...
      if (colnr > 0)
      {
        ...
        if ((rownr > lp->rows) || (colnr > lp->sum))
        {
          report(lp,
                 SEVERE,
                 "dualloop: Invalid row %d(%d) and column %d(%d) pair selected at iteration %.0f\n",
                 rownr,
                 lp->rows,
                 colnr - lp->columns,
                 lp->columns,
        ...
        else
        {
          ...
          if ((lp->improve & IMPROVE_THETAGAP) && !refactRecent(lp) &&
              (my_reldiff(fabs(theta), fabs(prow[colnr])) > lp->epspivot * 10.0 * log(2.0 + 50.0 * lp->rows)))
          { /* This is my kludge - KE */
            ...
            report(lp,
                   DETAILED,
                   "dualloop: Refactorizing at iter %.0f due to loss of accuracy.\n",
                   (double)get_total_iter(lp));
          }
          theta = prow[colnr];
    ...
    else
    {
      ...
      if (inP1extra && (colnr == 0) && (lp->fixedvars > 0) && is_anti_degen(lp, ANTIDEGEN_FIXEDVARS))
      {
        report(lp,
               DETAILED,
               "dualloop: Trying to pivot out %d fixed basic variables at iter %.0f\n",
               lp->fixedvars,
               (double)get_total_iter(lp));
        rownr = 0;
        while (lp->fixedvars > 0)
        {
          ...
          {
            theta = 0;
            performiteration(lp, rownr, colnr, theta, TRUE, FALSE, prow, NULL, NULL, NULL, NULL);
            lp->fixedvars--;
          }
        }
      }
...
STATIC int
spx_run(lprec * lp, MYBOOL validInvB)
{
  (void)validInvB;
  ...
  while (lp->spx_status == RUNNING)
  {
    ...
    dualfeasible = isbb || isDualFeasible(lp, lp->epsprimal, boundflip_count, infeasibles, &dualoffset);
    ...
    if (((lp->simplex_strategy & SIMPLEX_Phase1_DUAL) == 0) ||
        ((MIP_count(lp) > 0) && (lp->total_iter == 0) && is_presolve(lp, PRESOLVE_REDUCEMIP)))
    {
    ...
    else
    {
      ...
      if (lost_feas_state)
      {
        ...
        else
        {
          report(lp,
                 IMPORTANT,
                 "spx_run: Lost feasibility %d times - iter %10.0f and %9.0f nodes.\n",
                 lost_feas_count,
                 (double)get_total_iter(lp),
...
STATIC int
lag_solve(lprec * lp, REAL start_bound, int num_iter)
{
  ...
  if (!allocREAL(lp, &OrigObj, lp->columns + 1, FALSE) || !allocREAL(lp, &ModObj, lp->columns + 1, TRUE) ||
      !allocREAL(lp, &SubGrad, get_Lrows(lp) + 1, TRUE) || !allocREAL(lp, &BestFeasSol, lp->sum + 1, TRUE))
  {
...
#define LagBasisContract
    ...
    if (lp->lag_trace)
    {
      report(lp,
             IMPORTANT,
             "Zub: %10g Zlb: %10g Stepsize: %10g Phi: %10g Feas %d\n",
             (double)Zub,
             (double)Zlb,
             (double)StepSize,
             (double)Phi,
    ...
    else if ((lp->spx_status == NUMFAILURE) || (lp->spx_status == PROCFAIL) || (lp->spx_status == USERABORT) ||
             (lp->spx_status == TIMEOUT) || (lp->spx_status == INFEASIBLE))
    {
    ...
    {
      report(
        lp, DETAILED, "lag_solve: Simplex status code %d, same basis %s\n", lp->spx_status, my_boolstr(same_basis));
      print_solution(lp, 1);
    }
#endif
  }
...
Leave:
  ...
  if (lp->lag_status == OPTIMAL)
  {
    ...
    i = check_solution(lp, lp->columns, lp->best_solution, lp->orig_upbo, lp->orig_lowbo, lp->epssolution);
  }
  else
  {
...
Leave:
  ...
  if ((lp->lag_status != RUNNING) && (lp->invB != NULL))
  {
    ...
    report(lp,
           NORMAL,
           "MEMO: lp_solve version %d.%d.%d.%d for %d bit OS, with %d bit REAL variables.\n",
           MAJORVERSION,
           MINORVERSION,
           RELEASE,
           BUILD,
    ...
    report(lp,
           NORMAL,
           "      In the total iteration count %.0f, %.0f (%.1f%%) were bound flips.\n",
           (double)lp->total_iter,
           (double)lp->total_bswap,
           test);
    report(lp,
           NORMAL,
           "      There were %d refactorizations, %d triggered by time and %d by density.\n",
    ...
    report(lp,
           NORMAL,
           "      The largest [%s] fact(B) had %d NZ entries, %.1fx largest basis.\n",
           lp->bfp_name(),
           itemp,
           lp->bfp_efficiency(lp));
    ...
    if (MIP_count(lp) > 0)
    {
      if (lp->bb_solutionlevel > 0)
        report(lp,
               NORMAL,
               "      The maximum B&B level was %d, %.1fx MIP order, %d at the optimal solution.\n",
               lp->bb_maxlevel,
               (double)lp->bb_maxlevel / (MIP_count(lp) + lp->int_vars),
               lp->bb_solutionlevel);
      else
        report(lp,
               NORMAL,
               "      The maximum B&B level was %d, %.1fx MIP order, with %.0f nodes explored.\n",
               lp->bb_maxlevel,
               (double)lp->bb_maxlevel / (MIP_count(lp) + lp->int_vars),
               (double)get_total_nodes(lp));
      if (GUB_count(lp) > 0)
        report(
          lp, NORMAL, "      %d general upper-bounded (GUB) structures were employed during B&B.\n", GUB_count(lp));
    }
    report(lp,
           NORMAL,
           "      The constraint matrix inf-norm is %g, with a dynamic range of %g.\n",
           lp->matA->infnorm,
           lp->matA->dynrange);
    report(lp,
           NORMAL,
           "      Time to load data was %.3f seconds, presolve used %.3f seconds,\n",
           lp->timestart - lp->timecreate,
           lp->timepresolved - lp->timestart);
    report(lp,
           NORMAL,
           "       ... %.3f seconds in simplex solver, in total %.3f seconds.\n",
           lp->timeend - lp->timepresolved,
           lp->timeend - lp->timecreate);
  }
  return (lp->spx_status);
