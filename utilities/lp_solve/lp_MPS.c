...
/* copy a MPS name, only trailing spaces are removed. In MPS, names can have
   embedded spaces! */
STATIC void
namecpy(char * into, const char * from)
{
  int i;
...
STATIC int
lenfield(char * line, int line_len)
{
  int    l;
  char * line1 = line;
              ...
              double * field6)
{
  ...
  if (*field3)
  {
    ...
    else if ((section == MPSBOUNDS) && ((strcmp(field1, "FR") == 0) || (strcmp(field1, "MI") == 0) ||
                                        (strcmp(field1, "PL") == 0) || (strcmp(field1, "BV") == 0)))
  ...
  {
    ptr1 = NULL;
    if ((section == MPSBOUNDS) && ((strcmp(field1, "FR") == 0) || (strcmp(field1, "MI") == 0) ||
                                   (strcmp(field1, "PL") == 0) || (strcmp(field1, "BV") == 0)))
    {
      strcpy(field3, field2);
      *field2 = 0;
...
STATIC MYBOOL
appendmpsitem(int * count, int rowIndex[], REAL rowValue[])
{
  int i = *count;
...
static int __WINAPI
MPS_input(void * fpin, char * buf, int max_size)
{
...
MYBOOL __WINAPI
MPS_readex(lprec ** newlp, void * userhandle, read_modeldata_func read_modeldata, int typeMPS, int verbose)
{
  char field1[BUFSIZ], field2[BUFSIZ], field3[BUFSIZ], field5[BUFSIZ], line[BUFSIZ], tmp[BUFSIZ], Last_col_name[BUFSIZ],
    probname[BUFSIZ], OBJNAME[BUFSIZ], *ptr;
  int    items, row, Lineno, var, section = MPSUNDEF, variant = 0, NZ = 0, SOS = 0;
  MYBOOL Int_section, Column_ready, Column_ready1, Unconstrained_rows_found = FALSE, OF_found = FALSE,
  ...
  if (lp != NULL)
  {
      ...
      { /* normal line, process */
        ...
        switch (section)
        {
              ...
              else if (*field2)
              {
                ...
                if (Column_ready1)
                {
                  ...
                  { /* Added ability to handle non-standard "same as above" column name */
                    if (addmpscolumn(
                          lp, Int_section, typeMPS, &Column_ready, &count, Last_column, Last_columnno, Last_col_name))
                    {
                      strcpy(Last_col_name, field2);
                      NZ = 0;
                    }
          ...
          case MPSBOUNDS:
            ...
            {
              report(lp, CRITICAL, "BOUND type %s on line %d is not supported", field1, Lineno);
              break;
            }
          ...
          case MPSRANGES:
            ...
            report(lp, FULL, "RANGES line: %s %s %g %s %g", field2, field3, field4, field5, field6);

            if ((items != 4) && (items != 6))
            {
              report(lp, CRITICAL, "Wrong number of items (%d) in RANGES section line %d", items, Lineno);
              break;
            }
          ...
          case MPSSOS:
            report(lp, FULL, "SOS line: %s %s %g %s %g", field2, field3, field4, field5, field6);

            if ((items == 0) || (items > 4))
            {
              report(lp, IMPORTANT, "Invalid number of items (%d) in SOS section line %d\n", items, Lineno);
              break;
            }
            ...
            if (items == 1 || items == 4)
            {
              ...
              if ((row <= 0) || (row > 9))
              {
                report(lp, IMPORTANT, "Error: Invalid SOS type %s line %d\n", field1, Lineno);
                break;
              }
            ...
            else
            {
              ...
              if ((var < 0) || (SOS < 1)) /* undefined var and could add ... */
                ...
                append_SOSrec(lp->SOS->sos_list[SOS - 1], 1, &var, &field4);
            }
        ...
        report(lp, IMPORTANT, "Error: Cannot handle line %d\n", Lineno);
        break;
      }
    }
    ...
    if ((*OBJNAME) && (!OF_found))
    {
      report(lp, IMPORTANT, "Error: Objective function specified by OBJNAME card not found\n");
      CompleteStatus = FALSE;
    }
...
number(char * str, REAL value)
{
  ...
  else if ((value < -0.99999999e11) || (value > -0.0001))
  {
    int n = 15;
  ...
  else if (((i = sprintf(_str, "%12.9f", (double)value)) > 12) && (_str[12] >= '5'))
  {
    for (i = 11; i >= 1; i--)
      if (_str[i] != '.')
      {
...
static char *
formatnumber12(char * numberbuffer, double a)
{
#if 0
...
STATIC const char *
MPSnameFIXED(char * name0, const char * name)
{
...
STATIC const char *
MPSnameFREE(char * name0, const char * name)
{
  if (strlen(name) < 8)
...
static void
write_data(void * userhandle, write_modeldata_func write_modeldata, const char * format, ...)
{
...
MYBOOL __WINAPI
MPS_writefileex(lprec * lp, int typeMPS, void * userhandle, write_modeldata_func write_modeldata)
{
  ...
  if ((typeMPS & MPSFIXED) == MPSFIXED)
  {
    /* Check if there is no variable name where the first 8 charachters are equal to the first 8 characters of anothe
     * variable */
    if (names_used)
      for (i = 1; (i <= lp->columns) && (ok); i++)
        if ((lp->col_name[i] != NULL) && (lp->col_name[i]->name != NULL) && (!is_splitvar(lp, i)) &&
            (strlen(lp->col_name[i]->name) > 8))
          for (j = 1; (j < i) && (ok); j++)
  ...
  for (i = 1; i <= lp->columns; i++)
  {
    if (!is_splitvar(lp, i))
    {
      ...
      for (k = 1, val1 = val, idx1 = idx, jj = 0; jj < je; jj++)
      {
        ...
        if (k == 0)
        {
          write_data(userhandle, write_modeldata, "    %s", MPSname(name0, get_col_name(lp, i)));
          write_data(userhandle,
                     write_modeldata,
                     "  %s  %s",
  ...
  for (i = lp->rows + 1; i <= lp->sum; i++)
    if (!is_splitvar(lp, i - lp->rows))
    {
      ...
      else
      {
        ...
        { /* Some solvers like CPLEX need to have a bound on a variable if it is integer, but not binary else it is
             interpreted as binary which is not ment */
          a = lp->orig_lowbo[i];
          a = unscaled_value(lp, a, i);
          if (putheader)
          {
...
static int __WINAPI
write_lpdata(void * userhandle, char * buf)
{
...
#ifdef OldNameMatch
static int
MPS_getnameidx(lprec * lp, const char * varname, MYBOOL isrow)
{
  int in = -1;
...
#else
static int
MPS_getnameidx(lprec * lp, const char * varname, MYBOOL tryrowfirst)
{
  int in = -1;
  ...
  if (in == -1)
  {
    if (strncmp(varname, (tryrowfirst ? ROWNAMEMASK : COLNAMEMASK), 1) == 0)
    {
      ...
      if ((sscanf(varname + 1, "%d", &in) != 1) || (in < (tryrowfirst ? 0 : 1)) ||
          (in > (tryrowfirst ? lp->rows : lp->columns)))
        in = -1;
    }
...
{
  char   field1[BUFSIZ], field2[BUFSIZ], field3[BUFSIZ], field5[BUFSIZ], line[BUFSIZ], tmp[BUFSIZ], *ptr;
  double field4, field6;
...
MPS_writeBAS(lprec * lp, int typeMPS, const char * filename)
{
  ...
  while ((ib < lp->sum) || (in < lp->sum))
  {
    ...
    in++;
    while ((in <= lp->sum) && (lp->is_basic[in] || ((in > lp->rows) && lp->is_lower[in])))
      in++;

    /* Check if we have a basic/non-basic variable pair */
    if ((ib <= lp->sum) && (in <= lp->sum))
    {
      strcpy(name1, MPSname(name0, (ib <= lp->rows ? get_row_name(lp, ib) : get_col_name(lp, ib - lp->rows))));
      strcpy(name2, MPSname(name0, (in <= lp->rows ? get_row_name(lp, in) : get_col_name(lp, in - lp->rows))));
    ...
    {
      strcpy(name1, MPSname(name0, (in <= lp->rows ? get_row_name(lp, in) : get_col_name(lp, in - lp->rows))));
