...
bfp_resize(lprec * lp, int newsize)
{
  INVrec * lu;
...
/* MUST MODIFY */
int BFP_CALLMODEL
bfp_nonzeros(lprec * lp, MYBOOL maximum)
{
  INVrec * lu;
...
/* MUST MODIFY */
int BFP_CALLMODEL
bfp_preparefactorization(lprec * lp)
{
  INVrec * lu = lp->invB;
...
int
bfp_LUSOLsetcolumn(lprec * lp, int posnr, int colnr)
{
  int nz, inform;
...
int
bfp_LUSOLidentity(lprec * lp, int * rownum)
{
  int      i, nz;
  INVrec * invB = lp->invB;
...
int
bfp_LUSOLfactorize(lprec * lp, MYBOOL * usedpos, int * rownum, int * singular)
{
  (void)usedpos;
...
bfp_LUSOLtighten(lprec * lp)
{
  ...
  {
    case FALSE:
      lp->report(lp, infolevel, "bfp_factorize: Very hard numerics, but cannot tighten LUSOL thresholds further.\n");
      break;
    case TRUE:
      lp->report(lp,
                 infolevel,
                 "bfp_factorize: Frequent refact pivot count %d at iter %.0f; tightened thresholds.\n",
                 lp->invB->num_pivots,
                 (REAL)lp->get_total_iter(lp));
      break;
    default:
      lp->report(lp,
                 infolevel,
                 "bfp_factorize: LUSOL switched to %s pivoting model to enhance stability.\n",
...
/* MUST MODIFY */
int BFP_CALLMODEL
bfp_factorize(lprec * lp, int uservars, int Bsize, MYBOOL * usedpos, MYBOOL final)
{
  int        kcol, inform, *rownum = NULL, singularities = 0, dimsize = lp->invB->dimcount;
  LUSOLrec * LUSOL = lp->invB->LUSOL;
  ...
  if (!final &&                                               /* No solution update-based refactorization */
      ...
      !lp->is_action(lp->spx_action, ACTION_TIMEDREINVERT) && /* No optimal time-based refactorization */
  ...
  {
    int  singularcols, replacedcols = 0;
    REAL hold;
    ...
    while ((inform == LUSOL_INFORM_LUSINGULAR) && (replacedcols < dimsize))
    {
      ...
      lp->report(lp,
                 NORMAL,
                 "bfp_factorize: Resolving %d singularit%s at refact %d, iter %.0f\n",
                 singularcols,
                 my_plural_y(singularcols),
                 lp->invB->num_refact,
                 hold);
...
/* MUST MODIFY */
MYBOOL BFP_CALLMODEL
bfp_finishupdate(lprec * lp, MYBOOL changesign)
/* Was addetacol() in versions of lp_solve before 4.0.1.8 - KE */
{
  ...
  if (TRUE || !changesign)
  {
    ...
    LU8RPC(LUSOL, LUSOL_UPDATE_OLDNONEMPTY, LUSOL_UPDATE_USEPREPARED, k, NULL, NULL, &i, &DIAG, &VNORM);
  }
  else
#endif
  {
    ...
    lu->value[0] = 0;
    /* Execute the update */
    LU8RPC(LUSOL, LUSOL_UPDATE_OLDNONEMPTY, LUSOL_UPDATE_NEWNONEMPTY, k, lu->value, NULL, &i, &DIAG, &VNORM);
  }
  ...
  else
  {
    ...
    lp->report(lp,
               infolevel,
               "bfp_finishupdate: Failed at iter %.0f, pivot %d;\n%s\n",
               (REAL)(lp->total_iter + lp->current_iter),
               lu->num_pivots,
               LUSOL_informstr(LUSOL, i));
    if (i == LUSOL_INFORM_ANEEDMEM)
    { /* To compress used memory and realloc, if necessary */
      lp->invert(lp, INITSOL_USEZERO, FALSE);
      if (i != LUSOL_INFORM_LUSUCCESS)
        lp->report(lp,
                   NORMAL,
                   "bfp_finishupdate: Insufficient memory at iter %.0f;\n%s\n",
                   (REAL)(lp->total_iter + lp->current_iter),
                   LUSOL_informstr(LUSOL, i));
    }
    else if (i == LUSOL_INFORM_RANKLOSS)
    { /* To fix rank loss and clear cumulative errors */
#if 0
...
#  endif
      ...
      if (i != LUSOL_INFORM_LUSUCCESS)
        lp->report(lp,
                   NORMAL,
                   "bfp_finishupdate: Recovery attempt unsuccessful at iter %.0f;\n%s\n",
                   (REAL)(lp->total_iter + lp->current_iter),
                   LUSOL_informstr(LUSOL, i));
      else
...
/* MUST MODIFY */
void BFP_CALLMODEL
bfp_ftran_normal(lprec * lp, REAL * pcol, int * nzidx)
{
  int      i;
  INVrec * lu;
  ...
  if (i != LUSOL_INFORM_LUSUCCESS)
  {
    ...
    lp->report(lp,
               NORMAL,
               "bfp_ftran_normal: Failed at iter %.0f, pivot %d;\n%s\n",
               (REAL)(lp->total_iter + lp->current_iter),
               lu->num_pivots,
               LUSOL_informstr(lu->LUSOL, i));
  }
}
...
/* MAY MODIFY */
void BFP_CALLMODEL
bfp_ftran_prepare(lprec * lp, REAL * pcol, int * nzidx)
{
  int      i;
  INVrec * lu;
  ...
  if (i != LUSOL_INFORM_LUSUCCESS)
  {
    ...
    lp->report(lp,
               NORMAL,
               "bfp_ftran_prepare: Failed at iter %.0f, pivot %d;\n%s\n",
               (REAL)(lp->total_iter + lp->current_iter),
               lu->num_pivots,
               LUSOL_informstr(lu->LUSOL, i));
  }
...
/* MUST MODIFY */
void BFP_CALLMODEL
bfp_btran_normal(lprec * lp, REAL * prow, int * nzidx)
{
  int      i;
  INVrec * lu;
  ...
  if (i != LUSOL_INFORM_LUSUCCESS)
  {
    ...
    lp->report(lp,
               NORMAL,
               "bfp_btran_normal: Failed at iter %.0f, pivot %d;\n%s\n",
               (REAL)(lp->total_iter + lp->current_iter),
               lu->num_pivots,
               LUSOL_informstr(lu->LUSOL, i));
  }
...
int BFP_CALLMODEL
bfp_findredundant(lprec * lp, int items, getcolumnex_func cb, int * maprow, int * mapcol)
{
  ...
  if (!allocINT(lp, &nzrows, items, FALSE) || !allocREAL(lp, &nzvalues, items, FALSE))
    return (n);
