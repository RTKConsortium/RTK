...
void *
clean_realloc(void * oldptr, int width, int newsize, int oldsize)
{
  newsize *= width;
  oldsize *= width;
...
MYBOOL
LUSOL_realloc_a(LUSOLrec * LUSOL, int newsize)
{
  int oldsize;
...
MYBOOL
LUSOL_expand_a(LUSOLrec * LUSOL, int * delta_lena, int * right_shift)
{
...
MYBOOL
LUSOL_realloc_r(LUSOLrec * LUSOL, int newsize)
{
  int oldsize;
  ...
  LUSOL->lenr = (int *)clean_realloc(LUSOL->lenr, sizeof(*(LUSOL->lenr)), newsize, oldsize);
  LUSOL->ip = (int *)clean_realloc(LUSOL->ip, sizeof(*(LUSOL->ip)), newsize, oldsize);
  LUSOL->iqloc = (int *)clean_realloc(LUSOL->iqloc, sizeof(*(LUSOL->iqloc)), newsize, oldsize);
  LUSOL->ipinv = (int *)clean_realloc(LUSOL->ipinv, sizeof(*(LUSOL->ipinv)), newsize, oldsize);
  LUSOL->locr = (int *)clean_realloc(LUSOL->locr, sizeof(*(LUSOL->locr)), newsize, oldsize);

  if ((newsize == 0) || ((LUSOL->lenr != NULL) && (LUSOL->ip != NULL) && (LUSOL->iqloc != NULL) &&
...
#  endif
    {
      LUSOL->amaxr = (REAL *)clean_realloc(LUSOL->amaxr, sizeof(*(LUSOL->amaxr)), newsize, oldsize);
...
MYBOOL
LUSOL_realloc_c(LUSOLrec * LUSOL, int newsize)
{
  int oldsize;
  ...
  LUSOL->lenc = (int *)clean_realloc(LUSOL->lenc, sizeof(*(LUSOL->lenc)), newsize, oldsize);
  LUSOL->iq = (int *)clean_realloc(LUSOL->iq, sizeof(*(LUSOL->iq)), newsize, oldsize);
  LUSOL->iploc = (int *)clean_realloc(LUSOL->iploc, sizeof(*(LUSOL->iploc)), newsize, oldsize);
  LUSOL->iqinv = (int *)clean_realloc(LUSOL->iqinv, sizeof(*(LUSOL->iqinv)), newsize, oldsize);
  LUSOL->locc = (int *)clean_realloc(LUSOL->locc, sizeof(*(LUSOL->locc)), newsize, oldsize);
  LUSOL->w = (REAL *)clean_realloc(LUSOL->w, sizeof(*(LUSOL->w)), newsize, oldsize);
#ifdef LUSOLSafeFastUpdate
  LUSOL->vLU6L = (REAL *)clean_realloc(LUSOL->vLU6L, sizeof(*(LUSOL->vLU6L)), newsize, oldsize);
#else
  LUSOL->vLU6L = LUSOL->w;
#endif

  if ((newsize == 0) || ((LUSOL->w != NULL) && (LUSOL->lenc != NULL) && (LUSOL->iq != NULL) && (LUSOL->iploc != NULL) &&
                         (LUSOL->iqinv != NULL) && (LUSOL->locc != NULL)))
  {
    ...
    {
      LUSOL->Ha = (REAL *)clean_realloc(LUSOL->Ha, sizeof(*(LUSOL->Ha)), newsize, oldsize);
      LUSOL->Hj = (int *)clean_realloc(LUSOL->Hj, sizeof(*(LUSOL->Hj)), newsize, oldsize);
      LUSOL->Hk = (int *)clean_realloc(LUSOL->Hk, sizeof(*(LUSOL->Hk)), newsize, oldsize);
      if ((newsize > 0) && ((LUSOL->Ha == NULL) || (LUSOL->Hj == NULL) || (LUSOL->Hk == NULL)))
        return (FALSE);
    }
#endif
#ifndef ClassicdiagU
    ...
    {
      LUSOL->diagU = (REAL *)clean_realloc(LUSOL->diagU, sizeof(*(LUSOL->diagU)), newsize, oldsize);
...
LUSOLrec *
LUSOL_create(FILE * outstream, int msgfil, int pivotmodel, int updatelimit)
{
  LUSOLrec * newLU;
...
MYBOOL
LUSOL_sizeto(LUSOLrec * LUSOL, int init_r, int init_c, int init_a)
{
  if (init_c == 0)
    LUSOL_FREE(LUSOL->isingular);
  if (LUSOL_realloc_a(LUSOL, init_a) && LUSOL_realloc_r(LUSOL, init_r) && LUSOL_realloc_c(LUSOL, init_c))
    return (TRUE);
  else
    return (FALSE);
}
...
const char *
LUSOL_pivotLabel(LUSOLrec * LUSOL)
{
  static const char * pivotText[LUSOL_PIVMOD_MAX + 1] = { "TPP", "TRP", "TCP", "TSP" };
  return (pivotText[LUSOL->luparm[LUSOL_IP_PIVOTTYPE]]);
}

void
LUSOL_setpivotmodel(LUSOLrec * LUSOL, int pivotmodel, int initlevel)
{
  REAL newFM, newUM;
...
MYBOOL
LUSOL_tightenpivot(LUSOLrec * LUSOL)
{
#if 0
  REAL newvalue;
#endif
  ...
  if (MIN(LUSOL->parmlu[LUSOL_RP_FACTORMAX_Lij], LUSOL->parmlu[LUSOL_RP_UPDATEMAX_Lij]) < 1.1)
  {
    if (LUSOL->luparm[LUSOL_IP_PIVOTTYPE] >= LUSOL_PIVMOD_TRP)
      return (FALSE);
...
MYBOOL
LUSOL_addSingularity(LUSOLrec * LUSOL, int singcol, int * inform)
{
  int NSING = LUSOL->luparm[LUSOL_IP_SINGULARITIES], ASING = LUSOL->luparm[LUSOL_IP_SINGULARLISTSIZE];
...
int
LUSOL_getSingularity(LUSOLrec * LUSOL, int singitem)
{
...
int
LUSOL_findSingularityPosition(LUSOLrec * LUSOL, int singcol)
...
const char *
LUSOL_informstr(LUSOLrec * LUSOL, int inform)
{
...
void
LUSOL_clear(LUSOLrec * LUSOL, MYBOOL nzonly)
{
  int len;
...
MYBOOL
LUSOL_assign(LUSOLrec * LUSOL, int iA[], int jA[], REAL Aij[], int nzcount, MYBOOL istriplet)
{
  int k, m, n, ij, kol;
  ...
  for (k = 1; k <= nzcount; k++)
  {
    ...
    {
      m = ij;
      if (m > LUSOL->maxm && !LUSOL_realloc_r(LUSOL, -(m / LUSOL_MINDELTA_FACTOR + 1)))
        return (FALSE);
    }
    LUSOL->indc[k] = ij;
    ...
    {
      n = ij;
      if (n > LUSOL->maxn && !LUSOL_realloc_c(LUSOL, -(n / LUSOL_MINDELTA_FACTOR + 1)))
        return (FALSE);
    }
    LUSOL->indr[k] = ij;
...
int
LUSOL_loadColumn(LUSOLrec * LUSOL, int iA[], int jA, REAL Aij[], int nzcount, int offset1)
{
  int i, ii, nz, k;
  ...
  for (ii = 1; ii <= nzcount; ii++)
  {
    ...
    if (Aij[i] == 0)
      continue;
    if (iA[i] <= 0 || iA[i] > LUSOL->m || jA <= 0 || jA > LUSOL->n)
    {
      LUSOL_report(
        LUSOL, 0, "Variable index outside of set bounds (r:%d/%d, c:%d/%d)\n", iA[i], LUSOL->m, jA, LUSOL->n);
      continue;
    }
    k++;
    nz++;
...
void
LUSOL_report(LUSOLrec * LUSOL, int msglevel, const char * format, ...)
{
  va_list ap;
...
void
LUSOL_timer(LUSOLrec * LUSOL, int timerid, const char * text)
{
  LUSOL_report(LUSOL, -1, "TimerID %d at %s - %s\n", timerid, "", text);
}

int
LUSOL_factorize(LUSOLrec * LUSOL)
{
  int inform;
...
int
LUSOL_ftran(LUSOLrec * LUSOL, REAL b[], int NZidx[], MYBOOL prepareupdate)
{
  int    inform;
  REAL * vector;
...
int
LUSOL_btran(LUSOLrec * LUSOL, REAL b[], int NZidx[])
{
  int inform;
...
int
LUSOL_replaceColumn(LUSOLrec * LUSOL, int jcol, REAL v[])
{
  int  inform;
  REAL DIAG, VNORM;

  LU8RPC(LUSOL, LUSOL_UPDATE_OLDNONEMPTY, LUSOL_UPDATE_NEWNONEMPTY, jcol, v, NULL, &inform, &DIAG, &VNORM);
...
REAL
LUSOL_vecdensity(LUSOLrec * LUSOL, REAL V[])
{
  int I, N = 0;
...
char
relationChar(REAL left, REAL right)
{
  if (left > right)
    return ('>');
  else if (left == right)
...
void
LUSOL_dump(FILE * output, LUSOLrec * LUSOL)
{
...
LUSOLmat *
LUSOL_matcreate(int dim, int nz)
{
  LUSOLmat * newm;
  ...
  if (newm != NULL)
  {
    ...
    if ((newm->a == NULL) || (newm->lenx == NULL) || (newm->indx == NULL) || (newm->indr == NULL) ||
        (newm->indc == NULL))
      LUSOL_matfree(&newm);
  }
  return (newm);
