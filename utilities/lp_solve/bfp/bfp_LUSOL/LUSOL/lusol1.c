...
/* ==================================================================
   ...
   ================================================================== */
void
LU1DCP(LUSOLrec * LUSOL, REAL DA[], int LDA, int M, int N, REAL SMALL, int * NSING, int IPVT[], int IX[])
{
  ...
  for (K = 1; K <= N; K++)
  {
    ...
    if (M > K)
    {
      ...
      for (J = KP1; J <= LAST; J++)
      {
        ...
        daxpy(M - K, T, DA + DAPOS(KP1, K) - LUSOL_ARRAYOFFSET, 1, DA + DAPOS(KP1, J) - LUSOL_ARRAYOFFSET, 1);
      }
    }
    else
      break;
...
/* ==================================================================
   ...
   ================================================================== */
void
LU1DPP(LUSOLrec * LUSOL, REAL DA[], int LDA, int M, int N, REAL SMALL, int * NSING, int IPVT[], int IX[])
{
...
/* ==================================================================
   ...
   ================================================================== */
void
LU1PQ1(LUSOLrec * LUSOL, int M, int N, int LEN[], int IPERM[], int LOC[], int INV[], int NUM[])
{
  int NZEROS, NZ, I, L;
  (void)LUSOL;
...
/* ==================================================================
   ...
   ================================================================== */
void
LU1PQ3(LUSOLrec * LUSOL, int MN, int LEN[], int IPERM[], int IW[], int * NRANK)
{
  int NZEROS, K, I;
  (void)LUSOL;
...
/* ==================================================================
   ...
   ================================================================== */
void
LU1REC(LUSOLrec * LUSOL, int N, MYBOOL REALS, int * LTOP, int IND[], int LEN[], int LOC[])
{
...
{
  MYBOOL ATEND;
  int    LR, J, LENJ, NFREE, LC1, LC2, NDONE, NDROP, L, I, LL, K, LR1, LAST, LREP, L1, L2, LC, LENI;
  REAL   UJ;
  REAL   AIJ;
...
LU1MAR(LUSOLrec * LUSOL,
       ...
       int *      MBEST)
{
  int  KBEST, NCOL, NROW, NZ1, NZ, LQ1, LQ2, LQ, J, LC1, LC2, LC, I, LEN1, MERIT, LP1, LP2, LP, LR1, LR2, LR;
    ...
    for (LQ = LQ1; LQ <= LQ2; LQ++)
    {
      ...
      for (LC = LC1; LC <= LC2; LC++)
      {
        ...
        if (MERIT == *MBEST)
        {
          ...
          if (LBEST <= LUSOL->parmlu[LUSOL_RP_GAMMA] && CMAX <= LUSOL->parmlu[LUSOL_RP_GAMMA])
          {
            if (ABEST >= AIJ)
              continue;
          }
...
/* ==================================================================
   ...
   ================================================================== */
void
LU1MCP(LUSOLrec * LUSOL, REAL AIJTOL, int * IBEST, int * JBEST, int * MBEST, int HLEN, REAL HA[], int HJ[])
{
...
LU1MRP(LUSOLrec * LUSOL,
       ...
       REAL       AMAXR[])
{
  int  I, J, KBEST, LC, LC1, LC2, LEN1, LP, LP1, LP2, LQ, LQ1, LQ2, LR, LR1, LR2, MERIT, NCOL, NROW, NZ, NZ1;
...
/* ==================================================================
   ...
   ================================================================== */
void
LU1MSP(LUSOLrec * LUSOL, int MAXMN, REAL LTOL, int MAXCOL, int * IBEST, int * JBEST, int * MBEST)
{
...
/* ==================================================================
   ...
   ================================================================== */
void
LU1MXC(LUSOLrec * LUSOL, int K1, int K2, int IX[])
{
  int  I, J, K, L, LC, LENJ;
  REAL AMAX;
  ...
  for (K = K1; K <= K2; K++)
  {
    ...
    LENJ = LUSOL->lenc[J];
    if (LENJ == 0)
      /*      LUSOL->a[LC] = ZERO;  Removal suggested by Yin Zhang to avoid overwriting next column when current is
       * empty */
      ;
    else
    {
...
/* ==================================================================
   ...
   ================================================================== */
void
LU1MXR(LUSOLrec * LUSOL, int K1, int K2, int IX[], REAL AMAXR[])
{
#define FastMXR
#ifdef FastMXR
...
#ifdef FastMXR
    for (LR = LR1, J = LUSOL->indr + LR1; LR <= LR2; LR++, J++)
    {
      /*      Find where  aij  is in column  j. */
      LC1 = LUSOL->locc[*J];
      LC2 = LC1 + LUSOL->lenc[*J];
      for (LC = LC1, IC = LUSOL->indc + LC1; LC < LC2; LC++, IC++)
      {
        if (*IC == I)
          break;
      }
...
LU1FUL(LUSOLrec * LUSOL,
       ...
       int        IPVT[])
{
  int  L, I, J, IPBASE, LDBASE, LQ, LC1, LC2, LC, LD, LKK, LKN, LU, K, L1, L2, IBEST, JBEST, LA, LL, NROWD, NCOLD;
  REAL AI, AJ;
...
/* ==================================================================
   ...
   ================================================================== */
void
LU1OR1(LUSOLrec * LUSOL, REAL SMALL, REAL * AMAX, int * NUMNZ, int * LERR, int * INFORM)
{
  int I, J, L, LDUMMY;
...
/* ==================================================================
   ...
   ================================================================== */
void
LU1OR3(LUSOLrec * LUSOL, int * LERR, int * INFORM)
{
  int I, J, L1, L2, L;
       ...
       REAL * AKMAX)
{
  ...
  int HLEN, HOPS, H, LPIV, LPRINT, MAXCOL, MAXROW, ILAST, JLAST, LFILE, LROW, LCOL, MINMN, MAXMN, NZLEFT, NSPARE, LU1,
    KK, J, LC, MLEFT, NLEFT, NROWU, LQ1, LQ2, JBEST, LQ, I, IBEST, MBEST, LEND, NFREE, LD, NCOLD, NROWD, MELIM, NELIM,
    JMAX, IMAX, LL1, LSAVE, LFREE, LIMIT, MINFRE, LPIVR, LPIVR1, LPIVR2, L, LPIVC, LPIVC1, LPIVC2, KBEST, LU, LR, LENJ,
    LC1, LAST, LL, LS, LENI, LR1, LFIRST, NFILL, NZCHNG, K, MRANK, NSING;
...
/*            ------------------------------------------------------------
    ...
    if (DENSE)
    {
      ...
      if (NFREE >= 2 * LEND)
      {
        ...
        *NDENS2 = NLEFT;
        ...
        if (LCOL >= LD)
        {
          LU1REC(LUSOL, LUSOL->n, TRUE, &LCOL, LUSOL->indc, LUSOL->lenc, LUSOL->locc);
          LFILE = LCOL;
  ...
  x300:
    ...
    if (LPRINT >= LUSOL_MSG_PIVOT)
    {
      ...
      {
        LUSOL_report(
          LUSOL, 0, "nrowu:%7d   i,jbest:%7d,%7d   nrowd,ncold:%6d,%6d\n", NROWU, IBEST, JBEST, NROWD, NCOLD);
        /*      TCP */
      }
      else
      {
        ...
        LUSOL_report(LUSOL,
                     0,
                     "nrowu:%7d   i,jbest:%7d,%7d   nrowd,ncold:%6d,%6d   i,jmax:%7d,%7d   aijmax:%g\n",
                     NROWU,
                     IBEST,
                     JBEST,
                     NROWD,
...
/*         Make sure the row file has room. */
    ...
    {
      LU1REC(LUSOL, LUSOL->m, FALSE, &LROW, LUSOL->indr, LUSOL->lenr, LUSOL->locr);
      LFILE = LROW;
  ...
  x400:
    ...
    LU1GAU(LUSOL,
           ...
           &NFILL,
           LUSOL->iqloc,
           LUSOL->a + LL1 - LUSOL_ARRAYOFFSET,
           LUSOL->indc + LL1 - LUSOL_ARRAYOFFSET,
           LUSOL->a + LU1 - LUSOL_ARRAYOFFSET,
           LUSOL->indr + LL1 - LUSOL_ARRAYOFFSET,
           LUSOL->indr + LU1 - LUSOL_ARRAYOFFSET);
#ifdef UseTimer
    timer("finish", eltime);
#endif
    if (LFIRST > 0)
  ...
  x700:
    ...
    LU1PQ2(LUSOL,
           ...
           LUSOL->indc + LU1 - LUSOL_ARRAYOFFSET,
           LUSOL->lenc,
           LUSOL->iqloc,
           LUSOL->iq,
           LUSOL->iqinv);
    LU1PQ2(LUSOL,
           ...
           LUSOL->indc + LSAVE - LUSOL_ARRAYOFFSET,
           LUSOL->lenr,
           LUSOL->iploc,
           LUSOL->ip,
           LUSOL->ipinv);
...
/* ==================================================================
   ...
   ================================================================== */
void
LU1FAC(LUSOLrec * LUSOL, int * INFORM)
{
  MYBOOL KEEPLU, TCP, TPP, TRP, TSP;
  int LPIV, NELEM0, LPRINT, MINLEN, NUML0, LENL, LENU, LROW, MERSUM, NUTRI, NLTRI, NDENS1, NDENS2, NRANK, NSING, JSING,
    JUMIN, NUMNZ, LERR, LU, LL, LM, LTOPL, K, I, LENUK, J, LENLK, IDUMMY, LLSAVE, NMOVE, L2, L, NCP, NBUMP;
#ifdef ClassicHamaxR
  int LENH, LENA2, LOCH, LMAXR;
#endif

  REAL LMAX, LTOL, SMALL, AMAX, UMAX, DUMAX, DUMIN, AKMAX, DM, DN, DELEM, DENSTY, AGRWTH, UGRWTH, GROWTH, CONDU, DINCR,
    AVGMER;
...
x990:
  ...
  if (LPRINT >= LUSOL_MSG_STATISTICS)
  {
    ...
    else
    {
      LUSOL_report(LUSOL,
                   0,
                   "Merit %s %g %d %d %d %g %d %d %g %g %d %d %d %g %g\n",
