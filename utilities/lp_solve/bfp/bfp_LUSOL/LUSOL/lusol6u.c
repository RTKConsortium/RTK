...
MYBOOL
LU1U0(LUSOLrec * LUSOL, LUSOLmat ** mat, int * inform)
{
  MYBOOL status = FALSE;
  ...
  *inform = LUSOL_INFORM_LUSUCCESS;
  ...
  if ((NUMU == 0) || (LENU == 0) || (LUSOL->luparm[LUSOL_IP_ACCELERATION] == LUSOL_BASEORDER) ||
...
void
LU6U0_v(LUSOLrec * LUSOL, LUSOLmat * mat, REAL V[], REAL W[], int NZidx[], int * INFORM)
{
  (void)NZidx;
#ifdef DoTraceU0
  REAL TEMP;
...
#ifdef xxLUSOLFastSolve
    L--;
    for (aptr = mat->a + L, jptr = mat->indc + L; LEN > 0; LEN--, aptr--, jptr--)
      V[*jptr] -= T * (*aptr);
#else
    for (; LEN > 0; LEN--)
    {
