...
#  define SET_MAT_ijA(item, i, j, A) \
    mat->col_mat[item].rownr = i;    \
...
#  define COL_MAT_COPY(left, right)             \
    COL_MAT_COLNR(left) = COL_MAT_COLNR(right); \
    COL_MAT_ROWNR(left) = COL_MAT_ROWNR(right); \
    COL_MAT_VALUE(left) = COL_MAT_VALUE(right)
#  define COL_MAT_MOVE(to, from, rec)                       \
...
__EXTERN_C
{
  ...
  STATIC MYBOOL   mat_setrow(
      MATrec * mat, int rowno, int count, REAL * row, int * colno, MYBOOL doscale, MYBOOL checkrowmode);
  STATIC MYBOOL mat_setcol(
    MATrec * mat, int colno, int count, REAL * column, int * rowno, MYBOOL doscale, MYBOOL checkrowmode);
  ...
  STATIC MYBOOL vec_compress(
    REAL * densevector, int startpos, int endpos, REAL epsilon, REAL * nzvector, int * nzindex);
  ...
  STATIC int    prod_xA(lprec * lp,
                        ...
                        int *  nzoutput,
                        int    roundmode);
  STATIC MYBOOL prod_xA2(lprec * lp,
                         int *  coltarget,
                         REAL * prow,
                         REAL   proundzero,
                         int *  pnzprow,
  ...
  STATIC MYBOOL fsolve(
    lprec * lp, int varin, REAL * pcol, int * nzidx, REAL roundzero, REAL ofscalar, MYBOOL prepareupdate);
  STATIC MYBOOL bsolve(lprec * lp, int row_nr, REAL * rhsvector, int * nzidx, REAL roundzero, REAL ofscalar);
  STATIC void   bsolve_xA2(lprec * lp,
                           int *  coltarget,
                           int    row_nr1,
                           REAL * vector1,
                           REAL   roundzero1,
