...
STATIC void
resize_SOSgroup(SOSgroup * group)
{
  if (group->sos_count == group->sos_alloc)
  {
    group->sos_alloc = (int)((double)group->sos_alloc * RESIZEFACTOR);
    group->sos_list = (SOSrec **)realloc(group->sos_list, (group->sos_alloc) * sizeof(*group->sos_list));
  }
}

STATIC int
append_SOSgroup(SOSgroup * group, SOSrec * SOS)
{
  int      i, k;
  SOSrec * SOSHold;
...
STATIC int
clean_SOSgroup(SOSgroup * group, MYBOOL forceupdatemap)
{
  int      i, n, k;
  SOSrec * SOS;
...
STATIC SOSrec *
create_SOSrec(SOSgroup * group, const char * name, int type, int priority, int size, int * variables, REAL * weights)
{
  SOSrec * SOS;
...
STATIC int
append_SOSrec(SOSrec * SOS, int size, int * variables, REAL * weights)
{
...
STATIC int
make_SOSchain(lprec * lp, MYBOOL forceresort)
{
  int        i, j, k, n;
  MYBOOL *   hold = NULL;
...
STATIC MYBOOL
delete_SOSrec(SOSgroup * group, int sosindex)
{
#ifdef Paranoia
...
STATIC MYBOOL
SOS_member_sortlist(SOSgroup * group, int sosindex)
...
STATIC int
SOS_member_updatemap(SOSgroup * group)
{
  int      i, j, k, n, nvars = 0, *list, *tally = NULL;
  SOSrec * rec;
  lprec *  lp = group->lp;
  ...
  for (i = 0; i < group->sos_count; i++)
  {
    ...
    for (j = 1; j <= n; j++)
    {
      ...
      if ((k < 1) || (k > lp->columns))
        report(
          lp, SEVERE, "SOS_member_updatemap: Member %j of SOS number %d is out of column range (%d)\n", j, i + 1, k);
#endif
      tally[k]++;
    }
  }
  ...
  for (i = 0; i < group->sos_count; i++)
  {
    ...
    for (j = 1; j <= n; j++)
    {
      ...
      if (k > group->memberpos[lp->columns])
        report(lp,
               SEVERE,
               "SOS_member_updatemap: Member mapping for variable %j of SOS number %d is invalid\n",
               list[j],
               i + 1);
#endif
...
STATIC MYBOOL
SOS_shift_col(SOSgroup * group, int sosindex, int column, int delta, LLrec * usedmap, MYBOOL forceresort)
    ...
    else
    {
      ...
      if (usedmap != NULL)
      {
        ...
        if (newidx == NULL)
        {
          ...
          for (i = firstActiveLink(usedmap), ii = 1; i != 0; i = nextActiveLink(usedmap, i), ii++)
            newidx[i] = ii;
        }
        for (i = 1, ii = 0; i <= n; i++)
        {
...
int
SOS_member_count(SOSgroup * group, int sosindex)
{
  SOSrec * SOS;
...
int
SOS_member_delete(SOSgroup * group, int sosindex, int member)
{
...
int
SOS_get_type(SOSgroup * group, int sosindex)
{
#ifdef Paranoia
...
int
SOS_infeasible(SOSgroup * group, int sosindex)
{
  ...
  else
  {
    ...
    {
      varnr = abs(list[i]);
      if ((lp->orig_lowbo[lp->rows + varnr] > 0) && !((lp->sc_vars > 0) && is_semicont(lp, varnr)))
        break;
    }
...
int
SOS_member_index(SOSgroup * group, int sosindex, int member)
{
  int      n;
  SOSrec * SOS;
...
int
SOS_memberships(SOSgroup * group, int varnr)
{
  int     i, n = 0;
  lprec * lp;
...
int
SOS_is_member(SOSgroup * group, int sosindex, int column)
{
  int     i, n = FALSE, *list;
  lprec * lp;
...
MYBOOL
SOS_is_member_of_type(SOSgroup * group, int column, int sostype)
{
  int i, k, n;
  ...
  if (group != NULL)
    for (i = group->memberpos[column - 1]; i < group->memberpos[column]; i++)
    {
      ...
      if (((n == sostype) || ((sostype == SOSn) && (n > 2))) && SOS_is_member(group, k, column))
        return (TRUE);
    }
  return (FALSE);
}
...
MYBOOL
SOS_set_GUB(SOSgroup * group, int sosindex, MYBOOL state)
{
  int i;
...
MYBOOL
SOS_is_GUB(SOSgroup * group, int sosindex)
{
  int i;
...
MYBOOL
SOS_is_marked(SOSgroup * group, int sosindex, int column)
{
  int     i, k, n, *list;
  lprec * lp;
...
MYBOOL
SOS_is_active(SOSgroup * group, int sosindex, int column)
{
  int     i, n, nn, *list;
  lprec * lp = group->lp;
...
MYBOOL
SOS_is_full(SOSgroup * group, int sosindex, int column, MYBOOL activeonly)
{
  int     i, nn, n, *list;
  lprec * lp = group->lp;
...
MYBOOL
SOS_can_activate(SOSgroup * group, int sosindex, int column)
{
  int     i, n, nn, nz, *list;
  lprec * lp;
...
MYBOOL
SOS_set_marked(SOSgroup * group, int sosindex, int column, MYBOOL asactive)
{
  int     i, n, nn, *list;
  lprec * lp = group->lp;
...
MYBOOL
SOS_unmark(SOSgroup * group, int sosindex, int column)
{
  int     i, n, nn, *list;
  MYBOOL  isactive;
  lprec * lp = group->lp;
...
int
SOS_fix_unmarked(SOSgroup *  group,
                 int         sosindex,
                 int         variable,
                 REAL *      bound,
                 REAL        value,
...
int *
SOS_get_candidates(SOSgroup * group, int sosindex, int column, MYBOOL excludetarget, REAL * upbound, REAL * lobound)
{
...
int
SOS_is_satisfied(SOSgroup * group, int sosindex, REAL * solution)
...
MYBOOL
SOS_is_feasible(SOSgroup * group, int sosindex, REAL * solution)
