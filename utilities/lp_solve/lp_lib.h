...
#define PRICE_STRATEGYMASK                                                                                   \
  (PRICE_PRIMALFALLBACK + PRICE_MULTIPLE + PRICE_PARTIAL + PRICE_ADAPTIVE + PRICE_HYBRID + PRICE_RANDOMIZE + \
   PRICE_AUTOPARTIAL + PRICE_AUTOMULTIPLE + PRICE_LOOPLEFT + PRICE_LOOPALTERNATE + PRICE_HARRISTWOPASS +     \
...
#define MAX_MINITUPDATES                                       \
  60 /* Maximum number of bound swaps between refactorizations \
        without recomputing the whole vector - contain errors */
#define MIN_REFACTFREQUENCY                             \
  5 /* Refactorization frequency indicating an inherent \
       numerical instability of the basis */
#define LAG_SINGULARLIMIT                                     \
  5 /* Number of times the objective does not change          \
...
#define MIN_TIMEPIVOT                                            \
  5.0e-02 /* Minimum time per pivot for reinversion optimization \
...
#define MAX_STALLCOUNT                                        \
  12 /* The absolute upper limit to the number of stalling or \
        cycling iterations before switching rule */
#define MAX_RULESWITCH                                              \
  5 /* The maximum number of times to try an alternate pricing rule \
       to recover from stalling; set negative for no limit. */
#define DEF_TIMEDREFACT                                   \
  AUTOMATIC /* Default for timed refactorization in BFPs; \
               can be FALSE, TRUE or AUTOMATIC (dynamic) */

#define DEF_SCALINGLIMIT 5 /* The default maximum number of scaling iterations */

#define DEF_NEGRANGE                                                \
  -1.0e+06 /* Downward limit for expanded variable range before the \
              variable is split into positive and negative components */
#define DEF_BB_LIMITLEVEL                                 \
  -50 /* Relative B&B limit to protect against very deep, \
...
#define DOUBLEROUND                                                                            \
  0.0e-02                       /* Extra rounding scalar used in btran/ftran calculations; the \
...
#  define DEF_PERTURB                                                             \
    1.0e-05                       /* Perturbation scalar for degenerate problems; \
...
#  define DEF_PERTURB                                                             \
    10e-03                        /* Perturbation scalar for degenerate problems; \
                                     must at least be RANDSCALE greater than EPSPRIMAL */
#  define DEF_EPSSOLUTION 1.0e-05 /* Margin of error for solution bounds */
...
#define DEF_PSEUDOCOSTUPDATES                                     \
  7 /* The default number of times pseudo-costs are recalculated; \
       experiments indicate that costs tend to stabilize */
#define DEF_PSEUDOCOSTRESTART                                    \
  0.15 /* The fraction of price updates required for B&B restart \
          when the mode is NODE_RESTARTMODE */
#define DEF_MAXPRESOLVELOOPS                               \
  0 /* Upper limit to the number of loops during presolve, \
       <= 0 for no limit. */
...
typedef MYBOOL(
  __WINAPI add_constraintex_func)(lprec * lp, int count, REAL * row, int * colno, int constr_type, REAL rh);
typedef MYBOOL(__WINAPI add_lag_con_func)(lprec * lp, REAL * row, int con_type, REAL rhs);
typedef int(__WINAPI add_SOS_func)(lprec *      lp,
                                   const char * name,
                                   int          sostype,
                                   int          priority,
                                   int          count,
...
typedef MYBOOL(__WINAPI get_ptr_sensitivity_objex_func)(lprec * lp,
                                                        REAL ** objfrom,
                                                        REAL ** objtill,
                                                        REAL ** objfromvalue,
                                                        REAL ** objtillvalue);
...
typedef MYBOOL(__WINAPI get_sensitivity_objex_func)(lprec * lp,
                                                    REAL *  objfrom,
                                                    REAL *  objtill,
                                                    REAL *  objfromvalue,
                                                    REAL *  objtillvalue);
...
typedef lprec *(__WINAPI read_XLI_func)(char *       xliname,
...
typedef int(
  BFP_CALLMODEL BFPint_lpintrealcbintint)(lprec * lp, int items, getcolumnex_func cb, int * maprow, int * mapcol);
...
typedef MYBOOL(XLI_CALLMODEL XLIbool_lpcharcharcharint)(lprec *      lp,
                                                        const char * modelname,
                                                        const char * dataname,
                                                        const char * options,
                                                        int          verbose);
typedef MYBOOL(XLI_CALLMODEL XLIbool_lpcharcharbool)(lprec *      lp,
                                                     const char * filename,
                                                     const char * options,
                                                     MYBOOL       results);
...
__EXTERN_C
{
  ...
  lprec __EXPORT_TYPE * __WINAPI read_XLI(
    char * xliname, const char * modelname, const char * dataname, const char * options, int verbose);
  ...
  MYBOOL __EXPORT_TYPE __WINAPI add_constraintex(
    lprec * lp, int count, REAL * row, int * colno, int constr_type, REAL rh);
  ...
  int __EXPORT_TYPE __WINAPI add_SOS(
    lprec * lp, const char * name, int sostype, int priority, int count, int * sosvars, REAL * weights);
  ...
  MYBOOL __EXPORT_TYPE __WINAPI get_ptr_sensitivity_rhs(
  ...
  MYBOOL __EXPORT_TYPE __WINAPI get_sensitivity_objex(
    lprec * lp, REAL * objfrom, REAL * objtill, REAL * objfromvalue, REAL * objtillvalue);
  MYBOOL __EXPORT_TYPE __WINAPI get_ptr_sensitivity_obj(lprec * lp, REAL * *objfrom, REAL * *objtill);
  MYBOOL __EXPORT_TYPE __WINAPI get_ptr_sensitivity_objex(
    lprec * lp, REAL * *objfrom, REAL * *objtill, REAL * *objfromvalue, REAL * *objtillvalue);
  ...
  MYBOOL __EXPORT_TYPE __WINAPI MPS_writefileex(
    lprec * lp, int typeMPS, void * userhandle, write_modeldata_func write_modeldata);
  /* #endif */
...
/* INLINE */ MYBOOL
is_fixedvar(lprec * lp, int variable);
/* INLINE */ MYBOOL
...
/* INLINE */ MYBOOL
is_bb_rule(lprec * lp, int bb_rule);
/* INLINE */ MYBOOL
is_bb_mode(lprec * lp, int bb_mask);
/* INLINE */ int
get_piv_rule(lprec * lp);
STATIC const char *
get_str_piv_rule(int rule);
STATIC MYBOOL __WINAPI
...
STATIC int
add_GUB(lprec * lp, const char * name, int priority, int count, int * sosvars);
STATIC basisrec *
...
STATIC MYBOOL
isDualFeasible(lprec * lp, REAL tol, int * boundflips, int infeasibles[], REAL * feasibilitygap);
...
STATIC void
varmap_addconstraint(lprec * lp);
STATIC void
varmap_addcolumn(lprec * lp);
STATIC void
