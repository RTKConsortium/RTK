...
const char * __VACALL
explain(lprec * lp, const char * format, ...)
{
...
void __VACALL
report(lprec * lp, int level, const char * format, ...)
{
  char    buff[DEF_STRBUFSIZE + 1];
  va_list ap;
...
STATIC void
debug_print(lprec * lp, const char * format, ...)
{
  va_list ap;
...
debug_print_solution(lprec * lp)
{
  ...
  if (lp->bb_trace)
    ...
    {
      print_indent(lp);
      report(lp, NEUTRAL, "%s " RESULTVALUEMASK "\n", get_col_name(lp, i - lp->rows), (double)lp->solution[i]);
    }
} /* debug_print_solution */

STATIC void
debug_print_bounds(lprec * lp, REAL * upbo, REAL * lowbo)
{
  int i;
  ...
  if (lp->bb_trace)
    for (i = lp->rows + 1; i <= lp->sum; i++)
    {
      ...
      {
        print_indent(lp);
        report(lp, NEUTRAL, "%s = " RESULTVALUEMASK "\n", get_col_name(lp, i - lp->rows), (double)lowbo[i]);
      }
      else
      {
        if (lowbo[i] != 0)
        ...
        {
          print_indent(lp);
          report(lp, NEUTRAL, "%s < " RESULTVALUEMASK "\n", get_col_name(lp, i - lp->rows), (double)upbo[i]);
        }
      }
    }
} /* debug_print_bounds */
...
void
blockWriteLREAL(FILE * output, const char * label, LREAL * vector, int first, int last)
{
  int i, k = 0;
...
void
blockWriteAMAT(FILE * output, const char * label, lprec * lp, int first, int last)
{
  int      i, j, k = 0;
  int      nzb, nze, jb;
  double   hold;
...
void
blockWriteBMAT(FILE * output, const char * label, lprec * lp, int first, int last)
{
  int    i, j, jb, k = 0;
  double hold;
...
MYBOOL
REPORT_debugdump(lprec * lp, const char * filename, MYBOOL livedata)
{
  FILE * output = stdout;
  MYBOOL ok;
  ...
  fprintf(output, "\nGENERAL INFORMATION\n-------------------\n\n");
  fprintf(output,
          "Model size:     %d rows (%d equalities, %d Lagrangean), %d columns (%d integers, %d SC, %d SOS, %d GUB)\n",
          lp->rows,
          lp->equalities,
          get_Lrows(lp),
          lp->columns,
          ...
          GUB_count(lp));
  fprintf(output,
          "Data size:      %d model non-zeros, %d invB non-zeros (engine is %s)\n",
          get_nonzeros(lp),
          my_if(lp->invB == NULL, 0, lp->bfp_nonzeros(lp, FALSE)),
          lp->bfp_name());
  fprintf(output,
          "Internal sizes: %d rows allocated, %d columns allocated, %d columns used, %d eta length\n",
          lp->rows_alloc,
          lp->columns_alloc,
          lp->columns,
          my_if(lp->invB == NULL, 0, lp->bfp_colcount(lp)));
  fprintf(output,
          "Memory use:     %d sparse matrix, %d eta\n",
          lp->matA->mat_alloc,
          my_if(lp->invB == NULL, 0, lp->bfp_memallocated(lp)));
  fprintf(output,
          "Parameters:     Maximize=%d, Names used=%d, Scalingmode=%d, Presolve=%d, SimplexPivot=%d\n",
          is_maxim(lp),
          lp->names_used,
          lp->scalemode,
          lp->do_presolve,
          lp->piv_strategy);
  fprintf(output,
          "Precision:      EpsValue=%g, EpsPrimal=%g, EpsDual=%g, EpsPivot=%g, EpsPerturb=%g\n",
          lp->epsvalue,
          lp->epsprimal,
          lp->epsdual,
          lp->epspivot,
          lp->epsperturb);
  fprintf(output,
          "Stability:      AntiDegen=%d, Improvement=%d, Split variables at=%g\n",
          lp->improve,
          lp->anti_degen,
          lp->negrange);
  fprintf(output,
          "B&B settings:   BB pivot rule=%d, BB branching=%s, BB strategy=%d, Integer precision=%g, MIP gaps=%g,%g\n",
          lp->bb_rule,
          my_boolstr(lp->bb_varbranch),
          lp->bb_floorfirst,
          lp->epsint,
          lp->mip_absgap,
          lp->mip_relgap);
...
void
REPORT_solution(lprec * lp, int columns)
{
  int               i, j, n;
  REAL              value;
...
void
REPORT_constraints(lprec * lp, int columns)
{
  int    i, n;
  REAL   value;
  MYBOOL NZonly = (MYBOOL)((lp->print_sol & AUTOMATIC) > 0);
...
REPORT_duals(lprec * lp)
{
  ...
  if (ret)
  {
    ...
    for (i = 1; i <= lp->columns; i++)
      if (!is_splitvar(lp, i))
        fprintf(lp->outstream,
                "%-20s  %15.7g %15.7g %15.7g\n",
                get_col_name(lp, i),
                (double)objfrom[i - 1],
                (double)objtill[i - 1],
                (double)objfromvalue[i - 1]);
  }
  ...
  if (ret)
  {
    ...
    for (i = 1; i <= lp->sum; i++)
      fprintf(lp->outstream,
              ...
              (double)duals[i - 1],
              (double)dualsfrom[i - 1],
              (double)dualstill[i - 1]);
    fflush(lp->outstream);
  }
}
...
REPORT_extended(lprec * lp)
{
  ...
  for (j = 1; j <= lp->columns; j++)
  {
    ...
    report(lp,
           NORMAL,
           "  %-25s " MPSVALUEMASK MPSVALUEMASK MPSVALUEMASK MPSVALUEMASK "\n",
           get_col_name(lp, j),
...
REPORT_tableau(lprec * lp)
{
  ...
  if (!lp->model_is_valid || !has_BFP(lp) || (get_total_iter(lp) == 0) || (lp->spx_status == NOTRUN))
  {
  ...
  for (j = 1; j <= lp->sum; j++)
    if (!lp->is_basic[j])
      fprintf(
        stream,
        "%15d",
        (j <= lp->rows ? (j + lp->columns) * ((lp->orig_upbo[j] == 0) || (is_chsign(lp, j)) ? 1 : -1) : j - lp->rows) *
          (lp->is_lower[j] ? 1 : -1));
  fprintf(stream, "\n");
...
#  pragma clang diagnostic pop
  ...
  for (row_nr = 1; (row_nr <= lp->rows + 1); row_nr++)
  {
    if (row_nr <= lp->rows)
      fprintf(stream,
              "%3d",
              (lp->var_basic[row_nr] <= lp->rows
                 ? (lp->var_basic[row_nr] + lp->columns) *
                     ((lp->orig_upbo[lp->var_basic[row_nr]] == 0) || (is_chsign(lp, lp->var_basic[row_nr])) ? 1 : -1)
                 : lp->var_basic[row_nr] - lp->rows) *
                (lp->is_lower[lp->var_basic[row_nr]] ? 1 : -1));
    else
      fprintf(stream, "   ");
    bsolve(lp, row_nr <= lp->rows ? row_nr : 0, prow, NULL, lp->epsmachine * DOUBLEROUND, 1.0);
    prod_xA(lp, coltarget, prow, NULL, lp->epsmachine, 1.0, prow, NULL, MAT_ROUNDDEFAULT);

    for (j = 1; j <= lp->rows + lp->columns; j++)
      if (!lp->is_basic[j])
        fprintf(stream, "%15.7f", prow[j] * (lp->is_lower[j] ? 1 : -1) * (row_nr <= lp->rows ? 1 : -1));
    fprintf(stream,
            "%15.7f",
            lp->rhs[row_nr <= lp->rows ? row_nr : 0] * (double)((row_nr <= lp->rows) || (is_maxim(lp)) ? 1 : -1));
    fprintf(stream, "\n");
  }
  fflush(stream);
...
void
REPORT_constraintinfo(lprec * lp, const char * datainfo)
{
...
void
REPORT_modelinfo(lprec * lp, MYBOOL doName, const char * datainfo)
{
  if (doName)
  {
  ...
  if (GUB_count(lp) + SOS_count(lp) > 0)
    report(lp,
           NORMAL,
           "Var-types:   %7d integer,     %7d semi-cont.,     %7d SOS.\n",
           lp->int_vars,
           lp->sc_vars,
           lp->sos_vars);
...
MYBOOL
REPORT_mat_mmsave(lprec * lp, const char * filename, int * colndx, MYBOOL includeOF, const char * infotext)
{
  int         n, m, nz, i, j, k, kk;
  MATrec *    mat = lp->matA;
  MM_typecode matcode;
