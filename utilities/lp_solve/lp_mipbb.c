...
STATIC BBrec *
create_BB(lprec * lp, BBrec * parentBB, MYBOOL dofullcopy)
{
  BBrec * newBB;
...
STATIC BBrec *
push_BB(lprec * lp, BBrec * parentBB, int varno, int vartype, int varcus)
/* Push ingoing bounds and B&B data onto the stack */
{
  ...
  if (newBB != NULL)
  {
    ...
    if ((parentBB != NULL) && (parentBB->lastrcf > 0))
    {
      ...
      {
        report(
          lp, DETAILED, "push_BB: Used reduced cost to fix %d variables and tighten %d bounds\n", nfixed, ntighten);
      }
    }
    ...
    else if (MIP_count(lp) > 0)
    {
      if ((lp->bb_level <= 1) && (lp->bb_varactive == NULL) &&
          (!allocINT(lp, &lp->bb_varactive, lp->columns + 1, TRUE) || !initcuts_BB(lp)))
        newBB = pop_BB(newBB);
      if (varno > 0)
      {
...
initbranches_BB(BBrec * BB)
{
  ...
  else
  {
    ...
    if ((BB->vartype == BB_SOS) && !SOS_is_member(lp->SOS, 0, k))
      report(
        lp, SEVERE, "initbranches_BB: Inconsistent identification of SOS variable %s (%d)\n", get_col_name(lp, k), k);
#endif
    ...
    if (is_int(lp, k) && ((new_bound > 0) && (BB->lastsolution > floor(new_bound))))
    {
      if (BB->lastsolution < ceil(new_bound))
        BB->lastsolution += 1;
      modifyUndoLadder(lp->bb_lowerchange, BB->varno, BB->lowbo, scaled_floor(lp, BB->varno, BB->lastsolution, 1));
    }
  }
...
fillbranches_BB(BBrec * BB)
{
  ...
  if (K > 0)
  {
    ...
    {
#if 1
      if (((ult_lowbo >= 0) && ((floor(BB->lastsolution) < /* Skip cases where the lower bound becomes violated */
...
#ifdef Paranoia
      debug_print(
        lp, "fillbranches_BB: New upper bound value %g conflicts with old lower bound %g\n", new_bound, BB->lowbo[K]);
#endif
      BB->nodesleft--;
      goto SetLB;
    }
  ...
  SetLB:
    ...
    else if (BB->vartype == BB_INT)
    {
      ...
      { /* Don't branch 0 since this is handled in SOS logic */
        BB->nodesleft--;
        goto Finish;
      }
    ...
    { /* Handle all SOS variants */
      ...
      else
      {
        ...
        if ((lp->SOS->maxorder > 2) && (BB->lastsolution == 0) && SOS_is_member_of_type(lp->SOS, k, SOSn))
        {
          BB->isSOS = AUTOMATIC;
        }
      }
...
#ifdef Paranoia
      debug_print(
        lp, "fillbranches_BB: New lower bound value %g conflicts with old upper bound %g\n", new_bound, BB->upbo[K]);
#endif
      BB->nodesleft--;
      goto Finish;
    }
    ...
    if (BB->nodesleft > 0)
    {
      ...
      if ((BB->vartype != BB_SOS) && (fabs(BB->LObound - BB->UPbound) < intmargin))
      {
        ...
        else
        {
          BB->isfloor = TRUE;
          report(
            BB->lp, IMPORTANT, "fillbranches_BB: Inconsistent equal-valued bounds for %s\n", get_col_name(BB->lp, k));
        }
      }
      if ((BB->nodesleft == 1) &&
          ((BB->isfloor && (BB->UPbound >= lp->infinite)) || (!BB->isfloor && (BB->LObound <= -lp->infinite))))
        BB->isfloor = !BB->isfloor;
  ...
  if (BB->nodesleft > 0)
  {
    ...
    if (BB->isSOS && (BB->vartype != BB_INT))
    {
      ...
      else
      {
        ...
        if (SOS_fix_unmarked(lp->SOS, 0, k, BB->upbo, 0, TRUE, NULL, lp->bb_upperchange) < 0)
          return (OKstatus);
      }
    }
    ...
    else if (BB->isGUB)
    {
      ...
      if (BB->isfloor)
      {
        if (SOS_fix_list(lp->GUB, 0, k, BB->upbo, BB->varmanaged, (MYBOOL)(BB->nodesleft > 0), lp->bb_upperchange) < 0)
          return (OKstatus);
      }
      ...
      {
        if (SOS_fix_unmarked(lp->GUB, 0, k, BB->upbo, 0, TRUE, NULL, lp->bb_upperchange) < 0)
          return (OKstatus);
      }
    }
...
solve_LP(lprec * lp, BBrec * BB)
{
  ...
  if (lp->bb_trace && !validate_bounds(lp, upbo, lowbo))
    report(lp,
           SEVERE,
           "solve_LP: Inconsistent bounds at iter %.0f, B&B node level %d.\n",
  ...
  while (status == RUNNING)
  {
              ...
              ((status == DEGENERATE) && is_anti_degen(lp, ANTIDEGEN_STALLING))))
    {
      ...
      { /* but not iterating infeasibility */
  ...
  if (status != OPTIMAL)
  {
    ...
    if ((status == USERABORT) || (status == TIMEOUT))
    {
      ...
      if ((lp->solutioncount == 0) &&
          /*
             30/01/08 <peno> added MIP_count test because in following situation thing were wrong:
              ...
              - When the timeout occurs, the simplex algorithm is in phase 2 and has a feasible (but non-integer)
             solution, but not optimal yet. If above situation occurs then a (sub-optimal) solution was returned while
             no integer solution isn't found yet at this time
          */
          (MIP_count(lp) == 0) && ((lp->simplex_mode & (SIMPLEX_Phase2_PRIMAL | SIMPLEX_Phase2_DUAL)) > 0))
      {
        lp->solutioncount++;
      ...
      report(lp,
             NORMAL,
             "The model %s\n",
             (status == UNBOUNDED) ? "is UNBOUNDED" : ((status == INFEASIBLE) ? "is INFEASIBLE" : "FAILED"));
    else
    {
#ifdef Paranoia
...
STATIC int
rcfbound_BB(BBrec * BB, int varno, MYBOOL isINT, REAL * newbound, MYBOOL * isfeasible)
{
  int     i = FR;
  lprec * lp = BB->lp;
  ...
  if (rangeLU > lp->epsprimal)
  {
    ...
    if (deltaRC <= 0)
      report(lp,
             SEVERE,
             "rcfbound_BB: A negative bound fixing level was encountered after node %.0f\n",
...
STATIC MYBOOL
findnode_BB(BBrec * BB, int * varno, int * vartype, int * varcus)
{
  ...
  *varcus = 0;
  ...
  if ((lp->bb_limitlevel != 1) && (MIP_count(lp) > 0))
  {
    ...
    if ((lp->bb_limitlevel > 0) && (lp->bb_level > lp->bb_limitlevel + countsossc))
      return (FALSE);
    else if ((lp->bb_limitlevel < 0) && (lp->bb_level > 2 * (lp->int_vars + countsossc) * abs(lp->bb_limitlevel)))
    {
      if (lp->bb_limitlevel == DEF_BB_LIMITLEVEL)
        report(lp,
               IMPORTANT,
               "findnode_BB: Default B&B limit reached at %d; optionally change strategy or limit.\n\n",
               lp->bb_level);
      return (FALSE);
    }
    ...
    if (((lp->solutioncount == 0) && !bb_better(lp, OF_HEURISTIC, OF_TEST_BE)) ||
        ((lp->solutioncount > 0) && (!bb_better(lp, OF_INCUMBENT | OF_DELTA, OF_TEST_BE | OF_TEST_RELGAP) ||
    ...
    k = *varno - lp->rows;
    if ((*varno > 0) && (lp->bb_limitlevel != 0) &&
        (lp->bb_varactive[k] >= abs(lp->bb_limitlevel) /* abs(DEF_BB_LIMITLEVEL) */))
    {
      /* if(!is_action(lp->nomessage, NOMSG_BBLIMIT)) {*/
      /*
              report(lp, IMPORTANT, "findnode_BB: Reached B&B depth limit %d for variable %d; will not dive
         further.\n\n", lp->bb_varactive[k], k);
      */
    ...
    if (*varno == 0)
    {
      ...
      is_better =
        (MYBOOL)(lp->solutioncount == 0) ||
        (MYBOOL)((lp->bb_deltaOF > 0) && (my_chsign(is_maxim(lp), lp->solution[0] - lp->best_solution[0]) < 0));
      ...
      else if (is_better)
      {
        ...
        {
          lp->bb_varactive[0]++;
          if ((lp->bb_varactive[0] == 1) && is_bb_mode(lp, NODE_DEPTHFIRSTMODE) && is_bb_mode(lp, NODE_DYNAMICMODE))
            lp->bb_rule &= !NODE_DEPTHFIRSTMODE;
        }

        if (lp->bb_trace || ((lp->verbose >= NORMAL) && (lp->print_sol == FALSE) && (lp->lag_status != RUNNING)))
        {
          report(lp,
                 IMPORTANT,
        ...
        if (lp->bb_breakfirst || (!is_infinite(lp, lp->bb_breakOF) && bb_better(lp, OF_USERBREAK, OF_TEST_BE)))
          lp->bb_break = TRUE;
      }
    }
  }
  ...
  if (is_better || is_equal)
  {
#ifdef ParanoiaMIP
    if ((lp->bb_level > 0) &&
        (check_solution(lp, lp->columns, lp->solution, lp->orig_upbo, lp->orig_lowbo, lp->epssolution) != OPTIMAL))
    {
      lp->solutioncount = 0;
      lp->spx_status = NUMFAILURE;
    ...
    if ((MIP_count(lp) > 0) && (lp->bb_totalnodes > 0))
    {
      if ((!construct_duals(lp)) ||
          (is_presolve(lp, PRESOLVE_SENSDUALS) && (!construct_sensitivity_duals(lp) || !construct_sensitivity_obj(lp))))
      {
      }
    }
  ...
  if (MIP_count(lp) > 0)
  {
    ...
    else if (lp->bb_level > 0)
    {
      ...
      if (lp->spx_trace)
        report(lp,
               DETAILED,
               "B&B level %5d OPT %16s value " RESULTVALUEMASK "\n",
               lp->bb_level,
               (*varno) ? "   " : "INT",
               lp->solution[0]);
    }
...
STATIC MYBOOL
strongbranch_BB(lprec * lp, BBrec * BB, int varno, int vartype, int varcus)
{
  ...
  do
  {
    ...
    if (solve_BB(strongBB) == OPTIMAL)
    {
      ...
      update_pseudocost(lp->bb_PseudoCost, varno, strongBB->vartype, strongBB->isfloor, lp->solution[strongBB->varno]);
    }
