...
#    endif
    ...
    if ((file != NULL) && (file != stderr) && (file != stdout))
    {
      ...
      time(&t);
      fprintf(file, "Generated on: %s%s\n", ctime(&t), (file == stdout) || (file == stderr) ? "\r" : "");
    }
  }
  if (file != NULL)
  {
...
#      if defined LINE_BY_LINE
      if (MessageBox((HWND)NULL, (LPCSTR)String, (LPCSTR) "Fortify", (UINT)MB_OKCANCEL /* |MB_ICONINFORMATION */) ==
          IDCANCEL)
#        if 0
#          if defined WIN32
                         /* TerminateProcess(GetCurrentProcess(),65535); */
      ...
      if ((file == stdout) || (file == stderr))
      {
        ...
        do
        {
          for (i = 0; (ptr[i]) && (ptr[i] != '\r') && (ptr[i] != '\n'); i++)
            ;
          Nchars += fprintf(file, "%-*.*s%s", i, i, ptr, (ptr[i]) ? "\r\n" : "");
          ptr += i;
          if (ptr[0] == '\r')
            ptr++;
          if (ptr[0] == '\n')
...
#      ifdef FORTIFY_WAIT_FOR_KEY
    ...
    if (wait_on_key < 0)
    {
      ...
      if (((ptr = getenv("FORTIFY_WAIT_FOR_KEY")) != NULL) && (tolower(*ptr) == 'n'))
        wait_on_key = 0;
      else
        wait_on_key = 1;
    }
...
#      if !defined LINE_BY_LINE
  if ((stdOutput) && (gfile != NULL) && (Nchars))
  {
    ...
    if (ptr != NULL)
    {
      ...
      if (MessageBox((HWND)NULL, (LPCSTR)ptr, (LPCSTR) "Fortify", (UINT)MB_OKCANCEL /* |MB_ICONINFORMATION */) ==
          IDCANCEL)
#        if 0
#          if defined WIN32
                         /* TerminateProcess(GetCurrentProcess(),65535); */
...
#    ifdef WARN_ON_SIZE_T_OVERFLOW
  {
    size_t private_size = sizeof(struct Header) + FORTIFY_BEFORE_SIZE + size + FORTIFY_AFTER_SIZE;
...
#    endif
  ...
  ptr = (unsigned char *)malloc(sizeof(struct Header) + FORTIFY_BEFORE_SIZE + size + FORTIFY_AFTER_SIZE);
  if (!ptr)
  {
...
#    endif
   ...
   */
  SetFortification(ptr + sizeof(struct Header), FORTIFY_BEFORE_VALUE, FORTIFY_BEFORE_SIZE);
  SetFortification(ptr + sizeof(struct Header) + FORTIFY_BEFORE_SIZE + size, FORTIFY_AFTER_VALUE, FORTIFY_AFTER_SIZE);

#    ifdef FILL_ON_MALLOC
  /*
   * Fill the actual user memory
   */
  SetFortification(ptr + sizeof(struct Header) + FORTIFY_BEFORE_SIZE, FILL_ON_MALLOC_VALUE, size);
#    endif
...
#    ifdef FILL_ON_FREE
   ...
   */
  SetFortification(ptr, FILL_ON_FREE_VALUE, sizeof(struct Header) + FORTIFY_BEFORE_SIZE + h->Size + FORTIFY_AFTER_SIZE);
#    endif
...
CheckBlock(struct Header * h, char * file, unsigned long line)
{
  ...
  if (!CheckFortification(ptr + sizeof(struct Header), FORTIFY_BEFORE_VALUE, FORTIFY_BEFORE_SIZE))
  {
    ...
    OutputFortification(ptr + sizeof(struct Header), FORTIFY_BEFORE_VALUE, FORTIFY_BEFORE_SIZE);
    OutputLastVerifiedPoint();
    result = 0;
  }

  if (!CheckFortification(
        ptr + sizeof(struct Header) + FORTIFY_BEFORE_SIZE + h->Size, FORTIFY_AFTER_VALUE, FORTIFY_AFTER_SIZE))
  {
...
static int
CheckPointer(unsigned char * ptr, unsigned long size, char * file, unsigned long line)
{
  struct Header * curr;
  unsigned char * ptr1;
  ...
  while (curr)
  {
    ...
    else
    {
      ...
      sprintf(st_Buffer, "         (%s,%ld,%s.%u)\n", address(ptr1), (unsigned long)curr->Size, curr->File, curr->Line);
      st_Output(st_Buffer);
...
{
  OutputFortification((unsigned char *)h + sizeof(struct Header) + FORTIFY_BEFORE_SIZE, 0, h->Size);
}
...
void FORTIFY_STORAGE _Fortify_free(uptr, file, line) void * uptr;
char *               file;
unsigned long        line;
{
  free(uptr);
...
void * FORTIFY_STORAGE _Fortify_realloc(ptr, new_size, file, line) void * ptr;
size_t                 new_size;
char *                 file;
unsigned long          line;
{
...
int FORTIFY_STORAGE _Fortify_CheckPointer(uptr, file, line) void * uptr;
char *              file;
unsigned long       line;
{
  return (1);
...
/*
 * Fortify_memcpy()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
void * FORTIFY_STORAGE
_Fortify_memcpy(void * to, void * from, size_t size, char * file, unsigned long line)
{
  if (size)
  {
    if ((from == NULL) || (to == NULL))
    {
      ...
      if (to == NULL)
        sprintf(st_Buffer + strlen(st_Buffer),
                "%s%s",
                (from == NULL) ? "" : " and ",
                "memcpy to pointer is NULL",
                file,
                line);
      strcat(st_Buffer, "\n");
      st_Output(st_Buffer);
...
/*
 * Fortify_memmove()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
void * FORTIFY_STORAGE
_Fortify_memmove(void * to, void * from, size_t size, char * file, unsigned long line)
{
  if ((from == NULL) || (to == NULL))
  {
    ...
    if (to == NULL)
      sprintf(
        st_Buffer + strlen(st_Buffer), "%s%s", (from == NULL) ? "" : " and ", "memmove to pointer is NULL", file, line);
    strcat(st_Buffer, "\n");
    st_Output(st_Buffer);
    FORTIFY_UNLOCK();
    WaitIfstdOutput();
...
/*
 * Fortify_memccpy()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
void * FORTIFY_STORAGE
_Fortify_memccpy(void * to, void * from, int c, size_t size, char * file, unsigned long line)
{
  if ((from == NULL) || (to == NULL))
  {
    ...
    if (to == NULL)
      sprintf(
        st_Buffer + strlen(st_Buffer), "%s%s", (from == NULL) ? "" : " and ", "memccpy to pointer is NULL", file, line);
    strcat(st_Buffer, "\n");
    st_Output(st_Buffer);
    FORTIFY_UNLOCK();
    WaitIfstdOutput();
...
/*
 * Fortify_memset()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
void * FORTIFY_STORAGE
...
/*
 * Fortify_memchr()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
void * FORTIFY_STORAGE
...
/*
 * Fortify_memcmp()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
int FORTIFY_STORAGE
...
/*
 * Fortify_memicmp()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
int FORTIFY_STORAGE
  ...
  if ((buffer1 == NULL) || (buffer2 == NULL))
  {
    ...
    if (buffer2 == NULL)
      sprintf(st_Buffer + strlen(st_Buffer),
              "%s%s",
              (buffer2 == NULL) ? "" : " and ",
              "memicmp second pointer is NULL",
              file,
              line);
    strcat(st_Buffer, "\n");
    st_Output(st_Buffer);
...
/*
 * Fortify_strcoll()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
int FORTIFY_STORAGE
  ...
  if ((buffer1 == NULL) || (buffer2 == NULL))
  {
    ...
    if (buffer2 == NULL)
      sprintf(st_Buffer + strlen(st_Buffer),
              "%s%s",
              (buffer2 == NULL) ? "" : " and ",
              "strcoll second pointer is NULL",
              file,
              line);
    strcat(st_Buffer, "\n");
    st_Output(st_Buffer);
...
/*
 * Fortify_strcspn()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
  ...
  if ((buffer1 == NULL) || (buffer2 == NULL))
  {
    ...
    if (buffer2 == NULL)
      sprintf(st_Buffer + strlen(st_Buffer),
              "%s%s",
              (buffer2 == NULL) ? "" : " and ",
              "strcspn second pointer is NULL",
              file,
              line);
    strcat(st_Buffer, "\n");
    st_Output(st_Buffer);
...
/*
 * Fortify_strcmp()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
int FORTIFY_STORAGE
...
/*
 * Fortify_strcmpi()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
int FORTIFY_STORAGE
  ...
  if ((buffer1 == NULL) || (buffer2 == NULL))
  {
    ...
    if (buffer2 == NULL)
      sprintf(st_Buffer + strlen(st_Buffer),
              "%s%s",
              (buffer2 == NULL) ? "" : " and ",
              "strcmpi second pointer is NULL",
              file,
              line);
    strcat(st_Buffer, "\n");
    st_Output(st_Buffer);
...
/*
 * Fortify_strncmp()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
int FORTIFY_STORAGE
  ...
  if ((buffer1 == NULL) || (buffer2 == NULL))
  {
    ...
    if (buffer2 == NULL)
      sprintf(st_Buffer + strlen(st_Buffer),
              "%s%s",
              (buffer2 == NULL) ? "" : " and ",
              "strncmp second pointer is NULL",
              file,
              line);
    strcat(st_Buffer, "\n");
    st_Output(st_Buffer);
...
/*
 * Fortify_strnicmp()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
int FORTIFY_STORAGE
  ...
  if ((buffer1 == NULL) || (buffer2 == NULL))
  {
    ...
    if (buffer2 == NULL)
      sprintf(st_Buffer + strlen(st_Buffer),
              "%s%s",
              (buffer2 == NULL) ? "" : " and ",
              "strnicmp second pointer is NULL",
              file,
              line);
    strcat(st_Buffer, "\n");
    st_Output(st_Buffer);
...
/*
 * Fortify_strchr()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
char * FORTIFY_STORAGE
...
/*
 * Fortify_strrchr()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
char * FORTIFY_STORAGE
...
/*
 * Fortify_strlwr()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
char * FORTIFY_STORAGE
...
/*
 * Fortify_strrev()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
char * FORTIFY_STORAGE
...
/*
 * Fortify_strlen()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
size_t FORTIFY_STORAGE
...
/*
 * Fortify_strcat()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
char * FORTIFY_STORAGE
...
/*
 * Fortify_strpbrk()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
char * FORTIFY_STORAGE
  ...
  if ((buffer1 == NULL) || (buffer2 == NULL))
  {
    ...
    if (buffer2 == NULL)
      sprintf(st_Buffer + strlen(st_Buffer),
              "%s%s",
              (buffer2 == NULL) ? "" : " and ",
              "strpbrk second pointer is NULL",
              file,
              line);
    strcat(st_Buffer, "\n");
    st_Output(st_Buffer);
...
/*
 * Fortify_strstr()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
char * FORTIFY_STORAGE
...
/*
 * Fortify_strtol()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
...
/*
 * Fortify_atoi()  - check if from/to is in allocated space and if so, then check if start/end is not outside allocated
 * space.
 */
int FORTIFY_STORAGE
...
/*
 * Fortify_atol()  - check if from/to is in allocated space and if so, then check if start/end is not outside allocated
 * space.
 */
long FORTIFY_STORAGE
...
/*
 * Fortify_atod()  - check if from/to is in allocated space and if so, then check if start/end is not outside allocated
 * space.
 */
...
/*
 * Fortify_strtoul()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
...
/*
 * Fortify_strtod()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
double FORTIFY_STORAGE
...
/*
 * Fortify_strset()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
char * FORTIFY_STORAGE
...
/*
 * Fortify_strnset()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
char * FORTIFY_STORAGE
...
/*
 * Fortify_strncpy()  - check if from/to is in allocated space and if so, then check if start/end is not outside
 * allocated space.
 */
