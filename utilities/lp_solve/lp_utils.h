...
#  undef allocREAL
#  undef allocLREAL
#  define allocCHAR(lp, ptr, size, clear)                    \
    (Fortify_LINE(__LINE__),                                 \
     Fortify_FILE(__FILE__),                                 \
     _Fortify_ret = allocCHAR_FORTIFY(lp, ptr, size, clear), \
     Fortify_LINE(0),                                        \
     Fortify_FILE(NULL),                                     \
     _Fortify_ret)
#  define allocMYBOOL(lp, ptr, size, clear)                    \
    (Fortify_LINE(__LINE__),                                   \
     Fortify_FILE(__FILE__),                                   \
     _Fortify_ret = allocMYBOOL_FORTIFY(lp, ptr, size, clear), \
     Fortify_LINE(0),                                          \
     Fortify_FILE(NULL),                                       \
     _Fortify_ret)
#  define allocINT(lp, ptr, size, clear)                    \
    (Fortify_LINE(__LINE__),                                \
     Fortify_FILE(__FILE__),                                \
     _Fortify_ret = allocINT_FORTIFY(lp, ptr, size, clear), \
     Fortify_LINE(0),                                       \
     Fortify_FILE(NULL),                                    \
     _Fortify_ret)
#  define allocREAL(lp, ptr, size, clear)                    \
    (Fortify_LINE(__LINE__),                                 \
     Fortify_FILE(__FILE__),                                 \
     _Fortify_ret = allocREAL_FORTIFY(lp, ptr, size, clear), \
     Fortify_LINE(0),                                        \
     Fortify_FILE(NULL),                                     \
     _Fortify_ret)
#  define allocLREAL(lp, ptr, size, clear)                    \
    (Fortify_LINE(__LINE__),                                  \
     Fortify_FILE(__FILE__),                                  \
     _Fortify_ret = allocLREAL_FORTIFY(lp, ptr, size, clear), \
     Fortify_LINE(0),                                         \
     Fortify_FILE(NULL),                                      \
     _Fortify_ret)
#endif
