...
STATIC void
write_lpcomment(void * userhandle, write_modeldata_func write_modeldata, const char * string, MYBOOL newlinebefore)
{
  write_data(userhandle, write_modeldata, "%s/* %s */\n", (newlinebefore) ? "\n" : "", string);
}

STATIC int
write_lprow(lprec *              lp,
            int                  rowno,
            void *               userhandle,
            write_modeldata_func write_modeldata,
            int                  maxlen,
            int *                idx,
            REAL *               val)
{
...
MYBOOL __WINAPI
write_lpex(lprec * lp, void * userhandle, write_modeldata_func write_modeldata)
{
  int          i, j, b, nrows = lp->rows, ncols = lp->columns, nchars, maxlen = LP_MAXLINELEN, *idx;
  MYBOOL       ok;
  REAL         a, *val;
  const char * ptr;
  ...
  for (j = 1; j <= nrows; j++)
  {
    if (((lp->names_used) && (lp->row_name[j] != NULL)) ||
        (write_lprow(lp, j, userhandle, NULL, maxlen, idx, val) == 1))
    ...
    if ((lp->orig_upbo[j]) && (lp->orig_upbo[j] < lp->infinite))
    {
      if (my_chsign(is_chsign(lp, j), lp->orig_rhs[j]) == -lp->infinite)
        ...
        write_data(userhandle,
                   write_modeldata,
                   "%+.12g %s ",
                   (lp->orig_upbo[j] - lp->orig_rhs[j]) * (is_chsign(lp, j) ? 1.0 : -1.0) /
                     (lp->scaling_used ? lp->scalars[j] : 1.0),
                   (is_chsign(lp, j)) ? ">=" : "<=");
    }
#endif
  ...
  if (SOS_count(lp) > 0)
  {
    ...
    {
      nchars = write_data(userhandle,
                          write_modeldata,
                          "%s: ",
                          (SOS->sos_list[i]->name == NULL) || (*SOS->sos_list[i]->name == 0)
                            ? "SOS"
                            : SOS->sos_list[i]->name); /* formatnumber12((double) lp->sos_list[i]->priority) */
      ...
      for (a = 0.0, j = 1; j <= SOS->sos_list[i]->size; a = SOS->sos_list[i]->weights[j], j++)
      {
        ...
        if (SOS->sos_list[i]->weights[j] == ++a)
          nchars += write_data(
            userhandle, write_modeldata, "%s%s", (j > 1) ? "," : "", get_col_name(lp, SOS->sos_list[i]->members[j]));
        else
          nchars += write_data(userhandle,
                               write_modeldata,
                               "%s%s:%.12g",
