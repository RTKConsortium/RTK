...
PRIVATE int
init_rows_cols(int n_row, int n_col, Colamd_Row Row[], Colamd_Col Col[], int A[], int p[], int stats[COLAMD_STATS]);

PRIVATE void
init_scoring(int        n_row,
             int        n_col,
             Colamd_Row Row[],
             Colamd_Col Col[],
             int        A[],
...
PRIVATE void
order_children(int n_col, Colamd_Col Col[], int p[]);
...
PRIVATE int
garbage_collection(int n_row, int n_col, Colamd_Row Row[], Colamd_Col Col[], int A[], int * pfree);

PRIVATE int
clear_mark(int n_row, Colamd_Row Row[]);

PRIVATE void
print_report(const char * method, int stats[COLAMD_STATS]);
...
PRIVATE void
debug_mark(int n_row, Colamd_Row Row[], int tag_mark, int max_mark);

PRIVATE void
debug_matrix(int n_row, int n_col, Colamd_Row Row[], Colamd_Col Col[], int A[]);

PRIVATE void
debug_structures(int n_row, int n_col, Colamd_Row Row[], Colamd_Col Col[], int A[], int n_col2);
  ...
  )
{
  ...
  for (k = 0; k < n_col2; /* 'k' is incremented below */)
  {
    ...
    debug_step++;
    debug_deg_lists(n_row, n_col, Row, Col, head, min_score, n_col2 - k, max_deg);
...
#ifndef NDEBUG
    debug_deg_lists(n_row, n_col, Row, Col, head, min_score, n_col2 - k - pivot_row_degree, max_deg);
#endif /* NDEBUG */
...
print_report(const char * method, int stats[COLAMD_STATS])
{
  ...
  switch (stats[COLAMD_STATUS])
  {
    ...
    case COLAMD_OK_BUT_JUMBLED:
      ...
      PRINTF("%s: last seen in column:                             %d", method, INDEX(i1));
    ...
    case COLAMD_ERROR_internal_error:
      ...
      PRINTF("Internal error! Please contact authors (davis@cise.ufl.edu).\n");
      break;
  }
}
