...
const std::string ImagXGeometryReader<TInputImage>::AI_VERSION_1p5 = "IMAGX:1.6.10";

template< typename TInputImage >
const std::string ImagXGeometryReader<TInputImage>::AI_VERSION_2p1 = "adaPTinsight: 2.1.0 ";
...
ImagXGeometryReader<TInputImage>::GetGeometryForAI2p1()
{
	...
	if (arcFound)
	{
		std::cout << "Found active arc '" << F.activeArcName << "' linked to geocal UID '" << F.activeGeocalUID << "'" << std::endl;
...
void ImagXGeometryReader<TInputImage>::GenerateData()
{
	...
	if(version.compare(AI_VERSION_2p1) && version.compare(AI_VERSION_1p5)) 
	{
	...
	if (!version.compare(AI_VERSION_2p1))
	{
	...
	else if (!version.compare(AI_VERSION_1p5) && m_CalibrationXMLFileName.empty() && m_RoomXMLFileName.empty())  // Read geocal from projections
	{
		gantryAngleTag = "300a|011e";   // Nozzle angle
		calibModel = GetGeometryForAI1p5();
	}
	else if (!version.compare(AI_VERSION_1p5) && !m_CalibrationXMLFileName.empty() && !m_RoomXMLFileName.empty())
	{
		gantryAngleTag = "300a|011e";   // Nozzle angle
	...
	for (unsigned int noProj = 0; noProj < m_ProjectionsFileNames.size(); noProj++)
	{
		...
		if (!version.compare(AI_VERSION_1p5))    // Using CalibModel
		{
			float gantryAngle = std::atof(value.c_str());
			addEntryToGeometry(calibModel, gantryAngle);
		}
		else if (!version.compare(AI_VERSION_2p1)) // Using flexmap
		{
	                                                 ...
	                                                 std::array<float, 3>& detTrans, std::array<float, 3>& detRot, std::array<float, 3>& srcTrans)
{
	...
	float gantryAngleRad = gantryAngleDegree*deg2rad;
...
ImagXGeometryReader<TInputImage>::interpolate(const std::vector<float>& flexAngles, bool isCW, float angleDegree)
{
	...
	InterpResultType ires = { 0, 0, 1.f, 0.f };
	if (N == 1) {
		return ires;
	}
