	...
	void I0EstimationProjectionFilter<bitShift >::ThreadedGenerateData(const OutputImageRegionType& outputRegionForThread, ThreadIdType threadId)
	{
		...
		m_mutex->Lock();
		{
			...
			if (m_nsync >= m_Nthreads) {
				...
				// RMQ 1 : there might be pixels outside the min-max region. They are supposed to be inconsistents (unused detector lines, dead pixels,...)
				...
				while ((m_histogram[m_Imax] == 0) && (m_Imax < m_NBins)) {  // Get back to zero
					++m_Imax;
				}
				...
				while ((m_histogram[m_Imin] == 0) && (m_Imin > 0)) {  // Get back to zero
					--m_Imin;
				}
...
void I0EstimationProjectionFilter<bitShift >::AfterThreadedGenerateData()
{
	...
	m_I0rls = (m_Np > 1) ? unsigned short(float(m_I0rls*m_Lambda) + float(m_I0)*(1.0-m_Lambda)) : m_I0;
