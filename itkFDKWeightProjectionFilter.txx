...
::BeforeThreadedGenerateData()
{
  ...
  for(int k=0; k<m_AngularWeightsAndRampFactor.size(); k++)
    {
...
::ThreadedGenerateData(const OutputImageRegionType& outputRegionForThread, int threadId)
{
  ...
  InputImageType::IndexType index = outputRegionForThread.GetIndex();
  ...
  for(int k=outputRegionForThread.GetIndex(2);
    ...
    for(int j=outputRegionForThread.GetIndex(1);
      ...
      for(int i=outputRegionForThread.GetIndex(0);
