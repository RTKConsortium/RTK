  ...
  foreach(FFTW OFF)
    foreach(DEBUG_RELEASE RelWithDebInfo)
      foreach(STATIC_SHARED Static Shared)
        ...
        set(CTEST_BUILD_NAME "InITKExt-Windows7-64bit-MSVC${MSVC-VERSION}-ITK${ITK_VERSION}-${STATIC_SHARED}-${DEBUG_RELEASE}-FFTW${FFTW}")
        set(CTEST_GIT_COMMAND "C:\\Program Files\\Git\\bin\\git.exe")
        set(CTEST_GIT_UPDATE_CUSTOM "${CTEST_GIT_COMMAND}" pull)
        set(CTEST_SOURCE_DIRECTORY "D:\\src\\itk\\ITKExt-${ITK_VERSION}")
        message(${CTEST_SOURCE_DIRECTORY})
        file(REMOVE_RECURSE "${CTEST_SOURCE_DIRECTORY}\\Modules\\External\\RTK")
        execute_process(COMMAND "${CTEST_GIT_COMMAND}" clone "git://github.com/SimonRit/RTK.git" "${CTEST_SOURCE_DIRECTORY}\\Modules\\External\\RTK")
        set(CTEST_BINARY_DIRECTORY "D:\\RTKExt-${MSVC_YEAR}-${STATIC_SHARED}-${FFTW}")
