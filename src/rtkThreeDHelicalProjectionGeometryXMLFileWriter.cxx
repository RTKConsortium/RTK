...
#ifndef _rtkThreeDHelicalProjectionGeometryXMLFileWriter_cxx
...
int
ThreeDHelicalProjectionGeometryXMLFileWriter::CanWriteFile(const char * name)
{
...
ThreeDHelicalProjectionGeometryXMLFileWriter::WriteFile()
{
  ...
  startWithVersion << "RTKThreeDHelicalGeometry version=\""
                   << ThreeDHelicalProjectionGeometryXMLFileReader::CurrentVersion << '"';
  ...
  WriteLocalParameter(output, indent, this->m_InputObject->GetHelixPitch(), "HelixPitch");
  WriteLocalParameter(output, indent, this->m_InputObject->GetHelixRadius(), "HelixRadius");
  ...
  for (unsigned int i = 0; i < this->m_InputObject->GetMatrices().size(); i++)
  {
    ...
    // Only the GantryAngle and the HelicalAngle are necessarily projection specific
    WriteLocalParameter(output, indent, radiansToDegrees * this->m_InputObject->GetGantryAngles()[i], "GantryAngle");
    WriteLocalParameter(output, indent, radiansToDegrees * this->m_InputObject->GetHelicalAngles()[i], "HelicalAngle");
    if (!bSIDGlobal)
      WriteLocalParameter(
  ...
  this->WriteEndElement("RTKThreeDHelicalGeometry", output);
  output << std::endl;
...
bool
ThreeDHelicalProjectionGeometryXMLFileWriter::WriteGlobalParameter(std::ofstream &             output,
...
void
ThreeDHelicalProjectionGeometryXMLFileWriter::WriteLocalParameter(std::ofstream &     output,
                                                                  const std::string & indent,
                                                                  const double &      v,
                                                                  const std::string & s)
{
